...
`S` and `P` still use standard file dialogs for saving or loading a chosen `.vizpreset` anywhere. Presets are plain text files. They store mode, palette, hue shift, complexity, intensity, speed, HUD, trails, interaction, environment reactivity, adaptive quality, and Auto Scene settings. If a preset contains an unknown value, the loader keeps the existing default for that field instead of failing the entire load.
