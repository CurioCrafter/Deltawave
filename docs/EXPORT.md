...
`analysis_timeline.csv` contains one row per rendered frame with time, resolved mode/palette/motion-style/hue/depth/object-density/mouse-depth/glow/color/scene-personality/response/stability/clarity/intensity/speed/quality, RMS/bands/flux/beat/bar/downbeat/phrase/build-tension/BPM/drop, arrangement section, section choreography values (`sectionTransform3D`, `sectionDepthMotion3D`, and `sectionMaterialShift3D`), persistent song-arc values (`songArc3D`, anticipation, impact, recovery, and continuity), persistent song identity (`songIdentity`, `songIdentityConfidence3D`, and `songIdentityContinuity3D`), cinematic camera continuity values (`cameraMotion3D` and `cameraContinuity3D`), style, adaptive profile weights, beat/section sensitivity, key confidence, scene name, scene intent, authored 2D primitive count, retained 2D primitive count, projected 3D primitive count, 3D dominance, role spread/balance/vocabulary/silhouette contrast, role crosstalk, role legibility, role motion contrast, aggregate musical structure, and total primitive count. Use it to debug whether visuals are following the music as expected, whether projected 3D is actually carrying the frame, whether the camera is easing rather than snapping, whether song sections transform the whole 3D scene instead of only adding props, whether the song identity holds through ambiguous passages instead of flickering, and whether separate musical parts look like different 3D object families with different motion languages instead of one generic swarm. Profile paths are recorded in `export_manifest.txt` and `share_manifest.json` so calibrated exports can be traced back to their source adaptation files.
