...
7. The shared 3D object layer maps modes into five profiles: Techno Machine, Crystal Storm, Neural Space, Dimensional Tunnel, and Cymatic Sculpture. It emits `Object3D` polyhedra, shards, tunnel ribs, nodes, links, plates, ribbons, and particles with x/y/z positions, rotations, scales, velocities, glow, perspective projection, depth sorting, lighting/fog shading, and mouse-driven z impulses before rendering them back into neutral Direct2D primitives.
8. `RuntimeInspector` formats current source, look, user preset library count, profile, playback, and capture metadata for live display.
9. `Direct2DRenderer` draws the frame, using trail decay when enabled, then draws the HUD, control panel, and inspector.
10. `FrameRecorder` optionally writes the same geometry to disk with matching trail persistence.
11. `FramePerformanceTracker` smooths full-frame, analysis, geometry, Direct2D, and recording costs for the HUD and live capture metadata.
12. `AnalysisTimelineWriter` records one CSV row per rendered or recorded frame, including resolved 3D depth/object/glow/personality settings and bar/downbeat/phrase/build-tension sync columns.
13. `CapturePackage` can summarize a live recording as `index.html`, `preview.bmp`, plus `capture_manifest.json`; the Windows shell also attempts a best-effort MP4 encode on recording stop.
14. `VideoEncoder` can turn deterministic exported or live-captured frames into MP4 for sharing.
15. `SharePackage` can publish the exported result as `index.html`, `preview.bmp`, plus `share_manifest.json`, including the aggregated track-intelligence summary.
16. `BatchExporter` can repeat that deterministic path across a folder of tracks and publish a root gallery manifest/page with preview and per-track intelligence columns.
17. `SupportBundle` can collect recent profile, capture, export, batch, docs, and binary metadata into `support_manifest.json` and `support_summary.txt` for troubleshooting.
