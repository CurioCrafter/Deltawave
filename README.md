...
- 3D-first scene interpretation that classifies each frame as calm, groove, tension, drop, release, melodic, industrial, dark, bright, chaotic, spacious, heavy, or minimal, then uses that intent to add bass-pressure mass, mechanical architecture, orbiting ambient depth, harmonic crystal paths, breakbeat fractures, and dark/minimal monoliths.
- Legacy screen-space geometry is now thinned and faded before the 3D scene is projected, so rings/lines act as faint context while projected 3D objects dominate live playback, captures, and offline exports.
...
- Clickable Direct2D control panel for source, source profile reset, mode, palette, hue shift, 3D depth, 3D object density, mouse depth, glow, color impact, scene personality, 3D response, complexity, curated/user presets, recording, HUD, and interaction controls.
- Persistent in-window inspector for current source, file playback progress, active look, source profile files, decoded format, and capture status. `Capture idle` means recording is off; audio loopback/file analysis can still be active.
