...
struct SectionTransformStats3D {
                                       ...
                                       const VisualSettings& settings)
{
    ...
    if (analyzerRoleSum > 0.001f || metrics.roleSeparation > 0.001f) {
        ...
        const float convergencePermission = clamp01(metrics.convergenceRole * 0.62f +
                                                    metrics.dropIntensity * 0.20f +
                                                    (metrics.phraseBoundary ? metrics.phraseConfidence * 0.18f : 0.0f) +
                                                    (metrics.downbeat ? metrics.downbeatConfidence * 0.12f : 0.0f));
        const float analyzedConvergence = clamp01(metrics.convergenceRole * 0.76f +
                                                  metrics.dropIntensity * convergencePermission * 0.24f);
        role.convergence = blendRole(role.convergence, analyzedConvergence);
    ...
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.54f +
                                            metrics.dropIntensity * 0.26f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.22f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.13f : 0.0f) +
                                            transientEnergy3D(metrics) * metrics.dropIntensity * 0.12f);
    const float separationGuard = clamp01(role.separation * 0.58f +
                                          patternClarityOf(settings) * 0.22f +
                                          std::min(activeRoles / 7.0f, 1.0f) * 0.20f);
    role.convergence = clamp01(role.convergence *
                               (0.34f + earnedConvergence * 0.74f) *
                               (1.0f - separationGuard * 0.22f));
    if (earnedConvergence < 0.22f && metrics.convergenceRole < 0.18f) {
...
Vec3 roleDistrictUnitOffset(MusicalRoleDistrict3D district)
{
    switch (district) {
    case MusicalRoleDistrict3D::Bass:
        return Vec3{0.00f, 0.34f, -0.78f};
    case MusicalRoleDistrict3D::Drums:
        return Vec3{-0.92f, 0.02f, -0.18f};
    case MusicalRoleDistrict3D::Melody:
        return Vec3{0.84f, -0.44f, 0.14f};
    case MusicalRoleDistrict3D::Harmony:
        return Vec3{0.34f, -0.24f, 0.66f};
    case MusicalRoleDistrict3D::Space:
        return Vec3{-0.18f, -0.04f, 1.08f};
    case MusicalRoleDistrict3D::Fracture:
        return Vec3{1.02f, 0.16f, -0.40f};
    case MusicalRoleDistrict3D::Shadow:
        return Vec3{-0.72f, 0.36f, 0.52f};
    }
    return Vec3{};
}
                          ...
                          double time)
{
    ...
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.56f +
                                            metrics.dropIntensity * 0.28f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.22f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.12f : 0.0f));
    const float separation = clamp01(role.separation * (1.0f - earnedConvergence * 0.16f - role.convergence * 0.10f));
    const float stageScale = minimumDimension *
                             (0.50f +
                              separation * 0.78f +
                              scenePersonalityOf(settings) * 0.14f +
                              patternClarityOf(settings) * 0.10f);
    ...
    const float convergencePull = clamp01(role.convergence *
                                          (0.16f +
                                           earnedConvergence * 0.32f +
                                           metrics.dropIntensity * 0.10f));
    const float preserveDistrict = 1.0f - convergencePull * 0.30f;
    return mix(convergence, center, std::clamp(preserveDistrict, 0.62f, 1.0f));
}
                                       ...
                                       double time)
{
    ...
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.56f +
                                            ...
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.22f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.12f : 0.0f));
    const float separation = clamp01(role.separation * (1.0f - earnedConvergence * 0.18f - role.convergence * 0.12f));
    const float stagingStrength = std::clamp(0.22f +
                                             ...
                                             0.18f,
                                             0.86f);
    const float keepPattern = std::clamp(0.36f +
                                             (1.0f - separation) * 0.08f +
                                             earnedConvergence * 0.14f +
                                             role.convergence * 0.10f +
                                             (1.0f - patternClarityOf(settings)) * 0.08f,
                                         0.38f,
                                         0.68f);
    const Vec3 convergence = roleConvergencePoint3D(role, minimumDimension);
        ...
        if (convergencePull > 0.01f) {
            const Vec3 districtRendezvous = add(convergence,
                                                scale(roleDistrictUnitOffset(district),
                                                      minimumDimension * (0.22f + districtStrength * 0.13f) *
                                                          (1.0f - earnedConvergence * 0.22f)));
            object.position = mix(object.position,
                                  districtRendezvous,
                                  convergencePull * (0.18f + districtStrength * 0.14f));
            if (object.kind == Object3DKind::Link) {
                object.target = mix(object.target, districtRendezvous, convergencePull * 0.14f);
            }
        }

        if (bridgeRole) {
            const float bridgePresence = clamp01(0.18f + earnedConvergence * 0.34f + role.convergence * 0.42f);
            object.color.a *= bridgePresence;
            object.glow *= std::clamp(0.34f + earnedConvergence * 0.26f + role.convergence * 0.42f, 0.28f, 0.94f);
            object.scale = scale(object.scale, 0.78f + earnedConvergence * 0.10f + role.convergence * 0.12f);
        } else if (convergenceRole) {
            const float eventPresence = clamp01(0.12f + earnedConvergence * 0.40f + role.convergence * 0.92f);
            object.color.a *= eventPresence;
            object.glow *= std::clamp(0.20f + earnedConvergence * 0.38f + role.convergence * 0.92f, 0.16f, 1.18f);
            object.scale = scale(object.scale, 0.68f + earnedConvergence * 0.16f + role.convergence * 0.34f);
        }
...
void applyMusicalPartKinetics3D(std::vector<Object3D>& objects,
                                const MusicRoleScene3D& role,
                                const AudioMetrics& metrics,
                                const VisualSettings& settings,
                                float minimumDimension,
                                double time)
{
    ...
    const float activeMass = role.bass + role.drums + role.melody + role.harmony +
                             role.space + role.fracture + role.shadow;
    if (activeCount < 1.2f && metrics.rms < 0.04f) {
        return;
    }
    ...
    const float stability = motionStabilityOf(settings);
    const float clarity = patternClarityOf(settings);
    const float motionGain = std::clamp(0.56f + response * 0.28f + (1.0f - stability) * 0.12f,
                                        0.52f,
                                        1.08f);
    const float readableGain = std::clamp(0.78f + clarity * 0.16f + stability * 0.08f, 0.74f, 1.0f);
    const float beatPulse = metrics.beat ? clamp01(0.08f + metrics.beatConfidence * 0.92f)
                                         : metrics.beatConfidence * 0.42f;
    const float downbeatPulse = metrics.downbeat ? clamp01(0.10f + metrics.downbeatConfidence * 0.90f)
                                                 : metrics.downbeatConfidence * 0.32f;
    const float transient = transientEnergy3D(metrics);
    const float harmonic = clamp01(metrics.harmonicEnergy * 0.58f +
                                   metrics.keyConfidence * 0.30f +
                                   averageChromaEnergy(metrics) * 0.18f);
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.50f +
                                            metrics.dropIntensity * 0.30f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.26f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.14f : 0.0f));
    const float convergencePull = clamp01(role.convergence *
                                          (0.08f + earnedConvergence * 0.34f + metrics.dropIntensity * 0.14f));
    const Vec3 convergencePoint = roleConvergencePoint3D(role, minimumDimension);
    const float harmonicPhase = (metrics.keyIndex >= 0
                                     ? static_cast<float>((metrics.keyIndex % 12 + 12) % 12) / 12.0f
                                     : metrics.spectralCentroid) *
                                    2.0f * kPi +
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        ...
        const bool bridgeRole = object.musicRole == Object3DRole::Bridge;
        const bool convergenceRole = object.musicRole == Object3DRole::Convergence;
        if (!explicitRole && !bridgeRole && !convergenceRole) {
            continue;
        }

        const Vec3 before = object.position;
        const Vec3 beforeTarget = object.target;
        const float seed = static_cast<float>(i + 1U) * 0.493f +
                           static_cast<float>(static_cast<int>(object.kind)) * 0.311f;
        const float sign = std::sin(seed * 3.7f) >= 0.0f ? 1.0f : -1.0f;

        if (!explicitRole) {
            const float eventPresence = clamp01(earnedConvergence * (0.28f + role.convergence * 0.72f));
            object.color.a *= std::clamp(0.18f + eventPresence * 0.76f, 0.12f, 0.92f);
            object.glow *= std::clamp(0.24f + eventPresence * 0.88f, 0.18f, 1.10f);
            object.scale = scale(object.scale, 0.72f + eventPresence * 0.30f);
            if (eventPresence > 0.02f) {
                const float pull = eventPresence * 0.30f;
                object.position = mix(object.position, convergencePoint, pull);
                if (object.kind == Object3DKind::Link) {
                    object.target = mix(object.target, convergencePoint, pull * 0.72f);
                }
            }
        ...
        const float roleStrength = clamp01(roleDistrictStrength(role, district));
        if (roleStrength <= 0.025f) {
            continue;
        }

        const float drive = clamp01((0.30f + activeMass * 0.12f) * roleStrength * motionGain);
        switch (district) {
        case MusicalRoleDistrict3D::Bass: {
            const float pressure = clamp01(metrics.bass * 0.64f +
                                           metrics.lowMid * 0.22f +
                                           metrics.dropIntensity * 0.32f +
                                           downbeatPulse * 0.12f);
            object.position.z -= minimumDimension * drive * (0.045f + pressure * 0.105f + beatPulse * 0.026f);
            object.position.y += minimumDimension * drive * (0.014f + pressure * 0.038f);
            object.position.x *= 1.0f - drive * pressure * 0.040f;
            object.scale.y *= 1.0f + drive * (0.080f + pressure * 0.22f);
            object.scale.z *= 1.0f + drive * (0.100f + metrics.dropIntensity * 0.24f);
            object.rotation.z *= 1.0f - drive * 0.32f;
            object.glow += pressure * drive * 0.18f;
            break;
        }
        case MusicalRoleDistrict3D::Drums: {
            const float clock = clamp01(metrics.beatConfidence * 0.56f +
                                        metrics.barConfidence * 0.30f +
                                        metrics.downbeatConfidence * 0.18f);
            const float rhythmImpact = clamp01(clock * clock + downbeatPulse * 0.18f);
            const float cell = std::max(1.0f, minimumDimension * (0.026f + clock * 0.014f));
            const int objectStep = (static_cast<int>(std::floor(std::fabs(seed) * 11.0f)) + beatStep) % 4;
            const float stepPulse = (objectStep == 0 ? 0.16f + rhythmImpact * 0.92f : rhythmImpact * 0.20f) +
                                    downbeatPulse * 0.16f;
            object.position.x = std::round(object.position.x / cell) * cell;
            object.position.z = std::round(object.position.z / (cell * 1.36f)) * (cell * 1.36f) +
                                minimumDimension * drive * stepPulse * (0.056f + clock * 0.060f);
            object.position.y += minimumDimension * drive * stepPulse * (0.032f + clock * 0.035f);
            object.scale.y *= 1.0f + drive * (0.080f + stepPulse * 0.28f);
            object.scale.x *= 1.0f - drive * 0.045f;
            object.scale.z *= 1.0f - drive * 0.030f;
            object.rotation.x = std::round(object.rotation.x * 12.0f) / 12.0f;
            object.rotation.y = std::round(object.rotation.y * 12.0f) / 12.0f;
            object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 8.0f)) * (kPi / 8.0f);
            object.glow += clock * drive * 0.12f + stepPulse * drive * (0.12f + clock * 0.10f);
            break;
        }
        case MusicalRoleDistrict3D::Melody: {
            const float tonal = clamp01(harmonic * 0.74f + metrics.melodyRole * 0.22f);
            const float chroma = chromaAt(metrics,
                                          static_cast<std::size_t>(std::max(0, metrics.keyIndex) + static_cast<int>(i % 12U)));
            object.position.y -= minimumDimension * drive * (0.036f + tonal * 0.056f + chroma * 0.018f);
            object.position.x += std::cos(harmonicPhase * 0.70f + seed) * minimumDimension * drive * (0.020f + tonal * 0.028f);
            object.position.z += std::sin(harmonicPhase + seed) * minimumDimension * drive * (0.034f + chroma * 0.060f);
            object.rotation.y += std::sin(harmonicPhase + seed * 0.5f) * drive * 0.18f;
            object.rotation.z += std::cos(harmonicPhase * 0.8f) * drive * 0.12f;
            object.scale = scale(object.scale, 1.0f + tonal * drive * 0.070f);
            object.glow += tonal * drive * 0.17f + chroma * drive * 0.08f;
            break;
        }
        case MusicalRoleDistrict3D::Harmony: {
            const float chord = clamp01(harmonic * 0.70f +
                                        metrics.harmonyRole * 0.22f +
                                        metrics.phraseConfidence * 0.16f);
            const float mirror = object.position.x >= 0.0f ? 1.0f : -1.0f;
            object.position.x = mirror * std::fabs(object.position.x) * (1.0f + chord * drive * 0.080f);
            object.position.z += minimumDimension * drive * (0.030f + chord * 0.074f);
            object.position.y -= minimumDimension * drive * chord * 0.018f;
            object.rotation.x += chord * drive * 0.12f;
            object.rotation.z *= 1.0f - drive * 0.18f;
            object.scale.x *= 1.0f + chord * drive * 0.16f;
            object.scale.z *= 1.0f + chord * drive * 0.14f;
            object.glow += chord * drive * 0.13f;
            break;
        }
        case MusicalRoleDistrict3D::Space: {
            const float width = clamp01(metrics.stereoWidth * 0.74f + role.space * 0.26f);
            object.position.x *= 1.0f + width * drive * 0.16f;
            object.position.z += minimumDimension * drive * (0.110f + width * 0.170f);
            object.position.y += std::sin(phase * 0.020f + seed) * minimumDimension * drive * 0.026f;
            object.rotation.x *= 1.0f - drive * 0.30f;
            object.rotation.z *= 1.0f - drive * 0.24f;
            object.scale.x *= 1.0f + width * drive * 0.20f;
            object.scale.z *= 1.0f + width * drive * 0.13f;
            object.color.a *= std::clamp(0.92f + width * drive * 0.10f, 0.88f, 1.06f);
            object.glow += width * drive * 0.08f;
            break;
        }
        case MusicalRoleDistrict3D::Fracture: {
            const float cut = clamp01(transient * 0.82f + metrics.fractureRole * 0.24f + beatPulse * 0.12f);
            const float stepSign = ((beatStep + static_cast<int>(i % 3U)) % 2 == 0) ? -1.0f : 1.0f;
            object.position.x += stepSign * minimumDimension * drive * (0.050f + cut * 0.080f);
            object.position.z += sign * minimumDimension * drive * (0.034f + cut * 0.082f);
            object.position.y += std::sin(phase * 0.14f + seed) * minimumDimension * drive * cut * 0.032f;
            object.rotation.y += stepSign * drive * (0.20f + cut * 0.24f);
            object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 6.0f)) * (kPi / 6.0f);
            object.scale.x *= 1.0f + cut * drive * 0.16f;
            object.scale.y *= 1.0f - cut * drive * 0.12f;
            object.glow += cut * drive * 0.22f;
            break;
        }
        case MusicalRoleDistrict3D::Shadow: {
            const float mass = clamp01(role.shadow * 0.64f +
                                       role.bass * 0.16f +
                                       (1.0f - metrics.treble) * 0.18f);
            object.position.y += minimumDimension * drive * (0.038f + mass * 0.076f);
            object.position.z += minimumDimension * drive * (0.042f + mass * 0.088f);
            object.position.x *= 1.0f - mass * drive * 0.036f;
            object.rotation.x *= 1.0f - drive * 0.38f;
            object.rotation.y *= 1.0f - drive * 0.34f;
            object.rotation.z *= 1.0f - drive * 0.56f;
            object.scale.y *= 1.0f + mass * drive * 0.28f;
            object.scale.x *= 1.0f - mass * drive * 0.048f;
            object.glow += mass * drive * 0.07f;
            object.color.a = clamp01(object.color.a * (0.98f + mass * drive * 0.10f));
            break;
        }
        }

        if (convergencePull > 0.02f && earnedConvergence > 0.18f) {
            const Vec3 roleOffset = scale(roleDistrictUnitOffset(district),
                                          minimumDimension * (0.14f + roleStrength * 0.11f) *
                                              (1.0f - earnedConvergence * 0.22f));
            object.position = mix(object.position,
                                  add(convergencePoint, roleOffset),
                                  convergencePull * (0.18f + roleStrength * 0.16f));
        }

        object.position = mix(before, object.position, readableGain);
        const Vec3 delta = subtract(object.position, before);
        if (object.kind == Object3DKind::Link) {
            object.target = add(object.target, scale(delta, 0.76f));
        }
        object.velocity = add(object.velocity, delta);

        if (object.kind == Object3DKind::Link && length(subtract(object.target, beforeTarget)) > minimumDimension * 0.42f) {
            object.target = mix(object.target, beforeTarget, 0.20f);
        }
    }
}

void applyMusicalPartComposer3D(std::vector<Object3D>& objects,
                                const MusicRoleScene3D& role,
                                const SceneInterpretation& intent,
                                const AudioMetrics& metrics,
                                const VisualSettings& settings,
                                ...
                                double time)
{
    ...
    const float activeMass = role.bass + role.drums + role.melody + role.harmony +
                             role.space + role.fracture + role.shadow;
    if (activeCount < 1.4f && metrics.rms < 0.045f) {
        return;
    }
    ...
    const float clarity = patternClarityOf(settings);
    const float stability = motionStabilityOf(settings);
    const float voiceAuthority = clamp01(role.separation * 0.58f +
                                         std::min(activeCount / 7.0f, 1.0f) * 0.22f +
                                         std::min(activeMass / 3.8f, 1.0f) * 0.14f +
                                         clarity * 0.16f +
                                         stability * 0.08f);
    const float districtHold = clamp01(voiceAuthority * (1.0f - earnedConvergence * 0.30f - role.convergence * 0.18f));
    const float backgroundYield = clamp01(voiceAuthority * (1.0f - earnedConvergence * 0.22f));
    const float bridgePermission = clamp01(earnedConvergence * 0.58f +
                                           role.convergence * 0.48f +
                                           metrics.convergenceRole * 0.26f);
    const float phraseLoom = clamp01(metrics.phraseIntensity * metrics.phraseConfidence +
                                     (metrics.phraseBoundary ? metrics.phraseConfidence * 0.32f : 0.0f));
    const Vec3 convergence = roleConvergencePoint3D(role, minimumDimension);
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        ...
        if (explicitRole) {
            ...
            const Vec3 relative = subtract(object.position, center);
            const float localKeep = std::clamp(0.64f + strength * 0.16f + clarity * 0.08f - earnedConvergence * 0.10f,
                                               0.58f,
                                               0.88f);
            const Vec3 districtTarget = add(center,
                                            add(scale(relative, localKeep),
                                                scale(roleAxis,
                                                      minimumDimension *
                                                          (0.060f + strength * 0.082f + districtHold * 0.050f) *
                                                          (1.0f - earnedConvergence * 0.34f))));
            const float hold = std::clamp(0.10f + districtHold * (0.24f + strength * 0.16f),
                                          0.08f,
                                          object.kind == Object3DKind::Link ? 0.30f : 0.46f);
            const Vec3 before = object.position;
            object.position = mix(object.position, districtTarget, hold);
            const Vec3 delta = subtract(object.position, before);
            if (object.kind == Object3DKind::Link) {
            ...
            const float rolePresence = clamp01(strength * 0.58f + voiceAuthority * 0.32f + phraseLoom * 0.10f);
            object.color.a = clamp01(object.color.a * (1.04f + rolePresence * 0.28f));
            object.glow += rolePresence * (0.055f + strength * 0.055f);

            switch (district) {
            case MusicalRoleDistrict3D::Bass:
                object.scale.y *= 1.0f + rolePresence * (0.060f + metrics.bass * 0.060f);
                object.scale.z *= 1.0f + rolePresence * (0.070f + metrics.dropIntensity * 0.090f);
                object.rotation.z *= 0.78f;
                break;
            case MusicalRoleDistrict3D::Drums:
                object.scale.x *= 0.96f;
                object.scale.y *= 1.0f + rolePresence * (0.050f + metrics.beatConfidence * 0.080f);
                object.rotation.x = std::round(object.rotation.x * 14.0f) / 14.0f;
                object.rotation.z = std::round(object.rotation.z * 16.0f) / 16.0f;
                break;
            case MusicalRoleDistrict3D::Melody:
                object.position.y -= minimumDimension * rolePresence * 0.018f;
                object.scale.z *= 1.0f + rolePresence * (0.055f + metrics.harmonicEnergy * 0.060f);
                object.rotation.y += metrics.keyConfidence * rolePresence * 0.050f;
                break;
            case MusicalRoleDistrict3D::Harmony:
                object.scale.x *= 1.0f + rolePresence * (0.070f + metrics.harmonicEnergy * 0.055f);
                object.scale.z *= 1.0f + rolePresence * (0.060f + metrics.phraseConfidence * 0.050f);
                object.rotation.z *= 0.86f;
                break;
            case MusicalRoleDistrict3D::Space:
                object.position.z += minimumDimension * rolePresence * (0.034f + metrics.stereoWidth * 0.040f);
                object.scale.x *= 1.0f + rolePresence * (0.090f + metrics.stereoWidth * 0.060f);
                object.color.a = clamp01(object.color.a * 0.96f);
                break;
            case MusicalRoleDistrict3D::Fracture:
                object.position.x += (object.position.x >= center.x ? 1.0f : -1.0f) *
                                     minimumDimension * rolePresence *
                                         (0.024f + transientEnergy3D(metrics) * 0.032f);
                object.rotation.y += rolePresence * 0.090f;
                object.scale.y *= 1.0f + rolePresence * 0.070f;
                break;
            case MusicalRoleDistrict3D::Shadow:
                object.position.y += minimumDimension * rolePresence * 0.022f;
                object.scale.y *= 1.0f + rolePresence * (0.090f + role.shadow * 0.050f);
                object.rotation.z *= 0.72f;
                object.color.a = clamp01(object.color.a * (1.02f + rolePresence * 0.08f));
                break;
            }
            continue;
        }
        ...
        if (bridgeRole || convergenceRole) {
            const float permission = clamp01(bridgePermission + (convergenceRole ? role.convergence * 0.20f : 0.0f));
            const float visibility = std::clamp(0.14f + permission * 0.78f,
                                                0.10f,
                                                convergenceRole ? 1.0f : 0.74f);
            object.color.a *= visibility;
            object.glow *= std::clamp(0.22f + permission * 0.84f, 0.16f, 1.04f);
            object.scale = scale(object.scale, std::clamp(0.74f + permission * 0.34f, 0.68f, 1.08f));
            if (permission < 0.34f) {
                object.position = mix(object.position, convergence, 0.04f + permission * 0.06f);
                if (object.kind == Object3DKind::Link) {
                    object.target = mix(object.target, convergence, 0.03f + permission * 0.05f);
                }
            }
            continue;
        }

        if (backgroundYield > 0.08f) {
            object.position.z += minimumDimension * backgroundYield * 0.16f;
            object.position.x *= 1.0f + role.space * backgroundYield * 0.035f;
            object.color.a *= std::clamp(0.74f - backgroundYield * 0.34f, 0.30f, 0.76f);
            object.glow *= std::clamp(0.66f - backgroundYield * 0.24f, 0.36f, 0.72f);
            if (object.kind != Object3DKind::Column && object.kind != Object3DKind::TunnelRib) {
                object.scale = scale(object.scale, 0.88f - backgroundYield * 0.10f);
            }
        }
    }
}
...
void applyMusicalScorePartition3D(std::vector<Object3D>& objects,
                                  const MusicRoleScene3D& role,
                                  const SceneInterpretation& intent,
                                  const AudioMetrics& metrics,
                                  const VisualSettings& settings,
                                  ...
                                  double time)
{
    ...
    const float activeMass = role.bass + role.drums + role.melody + role.harmony +
                             role.space + role.fracture + role.shadow;
    if (activeCount < 1.25f && metrics.rms < 0.040f) {
        return;
    }
    ...
    const float transient = clamp01(metrics.onset * 0.40f +
                                    metrics.spectralFlux * 0.34f +
                                    transientEnergy3D(metrics) * 0.34f);
    const float beat = metrics.beat ? clamp01(0.08f + metrics.beatConfidence * 0.92f)
                                    : metrics.beatConfidence * 0.42f;
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.54f +
                                            metrics.dropIntensity * 0.30f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.26f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.16f : 0.0f));
    const float partitionAuthority = clamp01(role.separation * 0.58f +
                                             std::min(activeCount / 7.0f, 1.0f) * 0.18f +
                                             std::min(activeMass / 3.8f, 1.0f) * 0.12f +
                                             clarity * 0.18f +
                                             stability * 0.08f +
                                             scenePersonalityOf(settings) * 0.08f);
    const float partition = clamp01(partitionAuthority *
                                    (1.0f - earnedConvergence * 0.40f - role.convergence * 0.18f));
    const float eventPermission = clamp01(earnedConvergence * 0.64f +
                                          role.convergence * 0.44f +
                                          metrics.convergenceRole * 0.26f);
    const Vec3 convergencePoint = roleConvergencePoint3D(role, minimumDimension);
    ...
    const auto roleColor = [&](MusicalRoleDistrict3D district, float alpha) {
        ColorRGBA color{1.0f, 1.0f, 1.0f, alpha};
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            color = ColorRGBA{1.0f, 0.23f, 0.10f, alpha};
            break;
        case MusicalRoleDistrict3D::Drums:
            color = ColorRGBA{0.04f, 0.92f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Melody:
            color = ColorRGBA{1.0f, 0.20f, 0.88f, alpha};
            break;
        case MusicalRoleDistrict3D::Harmony:
            color = ColorRGBA{0.36f, 0.82f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Space:
            color = ColorRGBA{0.22f, 0.40f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Fracture:
            color = ColorRGBA{1.0f, 0.96f, 0.12f, alpha};
            break;
        case MusicalRoleDistrict3D::Shadow:
            color = ColorRGBA{0.70f, 0.70f, 0.84f, alpha};
            break;
        }
        return shiftedHue(color, settings.hueShift);
    };
    const auto pulseForRole = [&](MusicalRoleDistrict3D district) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return clamp01(metrics.bass * 0.62f + metrics.lowMid * 0.22f + metrics.dropIntensity * 0.26f);
        case MusicalRoleDistrict3D::Drums:
            return clamp01(metrics.beatConfidence * 0.58f + metrics.barConfidence * 0.24f + beat * 0.18f);
        case MusicalRoleDistrict3D::Melody:
            return clamp01(harmonic * 0.72f + metrics.melodyRole * 0.20f + metrics.keyConfidence * 0.10f);
        case MusicalRoleDistrict3D::Harmony:
            return clamp01(harmonic * 0.60f + metrics.harmonyRole * 0.22f + metrics.phraseConfidence * 0.18f);
        case MusicalRoleDistrict3D::Space:
            return clamp01(metrics.stereoWidth * 0.68f + role.space * 0.30f);
        case MusicalRoleDistrict3D::Fracture:
            return clamp01(transient * 0.78f + metrics.fractureRole * 0.22f);
        case MusicalRoleDistrict3D::Shadow:
            return clamp01(role.shadow * 0.62f + (1.0f - metrics.treble) * 0.22f + intent.dark * 0.16f);
        }
        return 0.0f;
    };
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        Object3D& object = objects[i];
        MusicalRoleDistrict3D district = MusicalRoleDistrict3D::Bass;
        bool explicitRole = districtForObjectRole(object.musicRole, district);
        const bool bridgeRole = object.musicRole == Object3DRole::Bridge;
        const bool convergenceRole = object.musicRole == Object3DRole::Convergence;

        if (!explicitRole && object.musicRole == Object3DRole::Unassigned && object.kind != Object3DKind::Particle) {
            MusicalRoleDistrict3D inferred = classifyRoleDistrict3D(object, role, minimumDimension, i);
            const float inferredStrength = roleDistrictStrength(role, inferred);
            const bool structuralObject = object.kind != Object3DKind::Link;
            if (structuralObject && inferredStrength > 0.17f && partition > 0.20f) {
                object.musicRole = objectRoleForDistrict(inferred);
                district = inferred;
                explicitRole = true;
            }
        }
        ...
        if (explicitRole) {
            ...
            const float strength = clamp01(roleDistrictStrength(role, district));
            if (strength <= 0.020f) {
                object.color.a *= 0.78f;
                object.glow *= 0.72f;
                continue;
            }

            const int ordinal = roleOrdinal[slot]++;
            const float lane = static_cast<float>((ordinal % 7) - 3) / 3.0f;
            const float layer = static_cast<float>((ordinal / 7) % 5) / 4.0f;
            const float seed = static_cast<float>(i + 1U) * 0.577f +
                               static_cast<float>(ordinal + 1) * 0.331f +
                               static_cast<float>(static_cast<int>(object.kind)) * 0.193f;
            const float pulse = pulseForRole(district);
            const float hold = std::clamp(0.14f + partition * (0.24f + strength * 0.20f) -
                                              earnedConvergence * 0.08f,
                                          0.10f,
                                          0.52f);
            const Vec3 center = roleDistrictCenter3D(district, role, intent, metrics, settings, minimumDimension, time);
            Vec3 local{};
            Vec3 scaleBias{1.0f, 1.0f, 1.0f};
            float tintAmount = 0.24f + partition * 0.28f + strength * 0.12f;

            switch (district) {
            case MusicalRoleDistrict3D::Bass:
                local = Vec3{lane * minimumDimension * (0.070f + metrics.stereoWidth * 0.035f),
                             minimumDimension * (0.080f + pulse * 0.075f + layer * 0.030f),
                             -minimumDimension * (0.18f + pulse * 0.18f + layer * 0.070f)};
                scaleBias = Vec3{0.82f, 1.18f + pulse * 0.24f, 1.20f + metrics.dropIntensity * 0.24f};
                object.rotation.z *= 0.44f;
                break;
            case MusicalRoleDistrict3D::Drums: {
                const float cell = minimumDimension * (0.060f + metrics.beatConfidence * 0.012f);
                local = Vec3{std::round(lane * 4.0f) * cell,
                             minimumDimension * (lane * 0.030f),
                             minimumDimension * (-0.08f + layer * 0.42f + beat * 0.050f)};
                scaleBias = Vec3{0.62f, 1.24f + pulse * 0.22f, 0.66f};
                object.rotation.x = std::round(object.rotation.x * 12.0f) / 12.0f;
                object.rotation.y = std::round(object.rotation.y * 12.0f) / 12.0f;
                object.rotation.z = std::round(object.rotation.z * 16.0f) / 16.0f;
                break;
            ...
            case MusicalRoleDistrict3D::Melody: {
                ...
                const float angle = metrics.phrasePhase * kPi + phase * 0.018f + seed;
                local = Vec3{std::cos(angle) * minimumDimension * (0.080f + chroma * 0.075f + strength * 0.040f),
                             -minimumDimension * (0.13f + pulse * 0.095f + layer * 0.045f),
                             minimumDimension * (0.04f + chroma * 0.34f + layer * 0.18f)};
                scaleBias = Vec3{0.78f, 0.76f, 1.34f + chroma * 0.20f};
                object.rotation.y += pulse * 0.12f;
                object.rotation.z += metrics.keyConfidence * 0.09f;
                break;
            }
            case MusicalRoleDistrict3D::Harmony: {
                const float mirror = ordinal % 2 == 0 ? -1.0f : 1.0f;
                local = Vec3{mirror * minimumDimension * (0.10f + layer * 0.10f + metrics.stereoWidth * 0.045f),
                             -minimumDimension * (0.055f + harmonic * 0.045f),
                             minimumDimension * (0.18f + harmonic * 0.18f + layer * 0.24f)};
                scaleBias = Vec3{1.32f + harmonic * 0.18f, 0.78f, 1.22f + pulse * 0.16f};
                object.rotation.z *= 0.70f;
                break;
            }
            case MusicalRoleDistrict3D::Space: {
                const float angle = phase * (0.010f + metrics.stereoWidth * 0.010f) + seed;
                const bool spaceDominant = role.space > 0.54f &&
                                           role.space > role.bass + 0.18f &&
                                           role.space > role.drums + 0.18f &&
                                           role.space > role.melody + 0.16f &&
                                           role.space > role.fracture + 0.18f &&
                                           role.space > role.shadow + 0.18f;
                const float depthBase = spaceDominant ? 0.24f : 0.36f;
                const float depthSpan = spaceDominant
                                            ? 0.44f + metrics.stereoWidth * 1.08f
                                            : 0.42f + metrics.stereoWidth * 0.34f;
                const float depthOffset = spaceDominant ? metrics.stereoWidth * 0.10f
                                                        : metrics.stereoWidth * 0.16f;
                local = Vec3{std::sin(angle) * minimumDimension * (0.18f + metrics.stereoWidth * 0.22f),
                             std::cos(angle * 0.63f) * minimumDimension * (0.035f + role.space * 0.035f),
                             minimumDimension * (depthBase + layer * depthSpan + depthOffset)};
                scaleBias = Vec3{1.42f + metrics.stereoWidth * 0.24f, 0.72f, 1.26f + role.space * 0.16f};
                object.rotation.x *= 0.50f;
                object.rotation.z *= 0.54f;
                tintAmount *= 0.86f;
                break;
            }
            case MusicalRoleDistrict3D::Fracture: {
                const float side = ordinal % 2 == 0 ? -1.0f : 1.0f;
                local = Vec3{side * minimumDimension * (0.18f + layer * 0.18f + transient * 0.070f),
                             (lane + std::sin(seed) * 0.30f) * minimumDimension * (0.070f + transient * 0.065f),
                             minimumDimension * (-0.12f + layer * 0.46f + side * transient * 0.080f)};
                scaleBias = Vec3{1.22f + transient * 0.18f, 0.62f + transient * 0.10f, 0.72f + transient * 0.12f};
                object.rotation.y += side * (0.12f + transient * 0.18f);
                object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 6.0f)) * (kPi / 6.0f);
                object.glow += transient * strength * (0.44f + response * 0.30f);
                object.color.a = clamp01(object.color.a * (1.04f + transient * 0.22f));
                break;
            }
            case MusicalRoleDistrict3D::Shadow:
                local = Vec3{lane * minimumDimension * (0.070f + metrics.stereoWidth * 0.040f),
                             minimumDimension * (0.18f + pulse * 0.11f + layer * 0.050f),
                             minimumDimension * (0.16f + pulse * 0.18f + layer * 0.18f)};
                scaleBias = Vec3{0.62f, 1.46f + pulse * 0.30f, 0.70f};
                object.rotation.x *= 0.34f;
                object.rotation.y *= 0.42f;
                object.rotation.z *= 0.20f;
                break;
            }

            const Vec3 roleAxis = roleDistrictUnitOffset(district);
            const Vec3 roleTarget = add(center,
                                        add(local,
                                            scale(roleAxis,
                                                  minimumDimension *
                                                      (0.080f + strength * 0.090f + partition * 0.060f) *
                                                      (1.0f - earnedConvergence * 0.30f))));
            const Vec3 before = object.position;
            object.position = mix(object.position, roleTarget, hold);
            if (district == MusicalRoleDistrict3D::Drums) {
                const float rhythmKick = pulseForRole(district);
                const float sequencerStep = rhythmKick * rhythmKick * strength;
                const float side = lane >= 0.0f ? 1.0f : -1.0f;
                object.position.x += side * minimumDimension * sequencerStep * 0.018f;
                object.position.y += minimumDimension * sequencerStep * (0.036f + metrics.downbeatConfidence * 0.024f);
                object.position.z += minimumDimension * sequencerStep * (0.112f + metrics.barConfidence * 0.056f);
            } else if (district == MusicalRoleDistrict3D::Space) {
                const bool spaceDominant = role.space > 0.54f &&
                                           role.space > role.bass + 0.18f &&
                                           role.space > role.drums + 0.18f &&
                                           role.space > role.melody + 0.16f &&
                                           ...
                                           role.space > role.shadow + 0.18f;
                if (spaceDominant) {
                    const float depthStretch = 0.42f + metrics.stereoWidth * 1.48f;
                    object.position.z = center.z + (object.position.z - center.z) * depthStretch;
                    object.position.z += minimumDimension * (metrics.stereoWidth - 0.44f) * 0.46f;
                    object.scale.z *= 0.92f + metrics.stereoWidth * 0.18f;
                }
            }
            const Vec3 delta = subtract(object.position, before);
            if (object.kind == Object3DKind::Link) {
            ...
            object.scale = multiply(object.scale, mix(Vec3{1.0f, 1.0f, 1.0f}, scaleBias, hold * 0.78f));
            object.color = mix(object.color, roleColor(district, object.color.a), clamp01(tintAmount));
            const float hueExpression = clamp01(std::fabs(settings.hueShift) * (0.42f + colorImpactOf(settings) * 0.34f));
            if (hueExpression > 0.001f) {
                object.color = mix(object.color, shiftedHue(object.color, settings.hueShift), hueExpression);
            }
            object.color.a = clamp01(object.color.a * (1.02f + partition * 0.16f + strength * 0.08f));
            object.glow += pulse * strength * (0.10f + response * 0.08f) + partition * 0.045f;
            if (district == MusicalRoleDistrict3D::Fracture) {
                object.glow += transient * strength * (0.18f + response * 0.16f);
            }
            object.velocity = add(object.velocity, delta);
            continue;
        }
        ...
        if (bridgeRole || convergenceRole) {
            const float visibility = std::clamp(0.045f + eventPermission * 0.86f,
                                                0.035f,
                                                convergenceRole ? 0.92f : 0.66f);
            object.color.a *= visibility;
            object.glow *= std::clamp(0.12f + eventPermission * 0.88f, 0.10f, 1.04f);
            object.scale = scale(object.scale, std::clamp(0.62f + eventPermission * 0.42f, 0.58f, 1.04f));
            if (eventPermission < 0.32f) {
                object.position = mix(object.position, convergencePoint, 0.025f + eventPermission * 0.055f);
                if (object.kind == Object3DKind::Link) {
                    object.target = mix(object.target, convergencePoint, 0.020f + eventPermission * 0.045f);
                }
            }
            continue;
        }

        const float yield = clamp01(partition * (0.46f + role.space * 0.14f + role.shadow * 0.10f));
        if (yield > 0.04f) {
            object.position.z += minimumDimension * yield * (0.18f + metrics.stereoWidth * 0.08f);
            object.position.x *= 1.0f + role.space * yield * 0.040f;
            object.color.a *= std::clamp(0.62f - yield * 0.28f, 0.18f, 0.68f);
            object.glow *= std::clamp(0.56f - yield * 0.18f, 0.24f, 0.62f);
            if (object.kind != Object3DKind::TunnelRib && object.kind != Object3DKind::Column) {
                object.scale = scale(object.scale, 0.86f - yield * 0.08f);
            }
        }
    }
}

void applyMusicalDepthScoreStaging3D(std::vector<Object3D>& objects,
                                     const MusicRoleScene3D& role,
                                     const SceneInterpretation& intent,
                                     const AudioMetrics& metrics,
                                     const VisualSettings& settings,
                                     ...
                                     double time)
{
    ...
    const float activeCount = activeRoleCount3D(role);
    if (activeCount < 1.35f && metrics.rms < 0.045f) {
        return;
    }
    ...
    const float beat = metrics.beat ? clamp01(0.10f + metrics.beatConfidence * 0.90f)
                                    : metrics.beatConfidence * 0.44f;
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.54f +
                                            metrics.dropIntensity * 0.30f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.24f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.14f : 0.0f));
    const float readableAuthority = clamp01(role.separation * 0.52f +
                                            std::min(activeCount / 7.0f, 1.0f) * 0.18f +
                                            clarity * 0.16f +
                                            personality * 0.10f +
                                            response * 0.06f);
    const float staging = clamp01(readableAuthority *
                                  (1.0f - earnedConvergence * 0.42f - role.convergence * 0.18f));
    if (staging < 0.08f) {
        return;
    }

    const Vec3 convergence = roleConvergencePoint3D(role, minimumDimension);
    const float depthWindow = minimumDimension *
                              std::clamp(0.92f +
                                             role.separation * 0.28f +
                                             metrics.stereoWidth * 0.18f +
                                             intent.depthReveal * 0.14f,
                                         0.82f,
                                         1.50f);
    const float blend = std::clamp(0.070f +
                                       staging * (0.12f + clarity * 0.07f) *
                                           (0.82f + stability * 0.12f),
                                   0.060f,
                                   0.28f);

    const auto layerUnit = [&](MusicalRoleDistrict3D district) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return -0.38f - metrics.bass * 0.16f - metrics.dropIntensity * 0.10f;
        case MusicalRoleDistrict3D::Drums:
            return -0.08f + beat * 0.08f + metrics.barConfidence * 0.04f;
        case MusicalRoleDistrict3D::Melody:
            return 0.18f + harmonic * 0.16f + role.melody * 0.06f;
        case MusicalRoleDistrict3D::Harmony:
            return 0.42f + harmonic * 0.14f + metrics.phraseConfidence * 0.06f;
        case MusicalRoleDistrict3D::Space:
            return 0.74f + metrics.stereoWidth * 0.34f + role.space * 0.12f;
        case MusicalRoleDistrict3D::Fracture:
            return 0.04f + transient * 0.18f + role.fracture * 0.05f;
        case MusicalRoleDistrict3D::Shadow:
            return 0.54f + role.shadow * 0.18f + intent.dark * 0.08f;
        }
        return 0.0f;
    };
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        ...
        if (explicitRole) {
            ...
            const float layer = static_cast<float>((ordinal / 9) % 5) / 4.0f;
            const float seed = static_cast<float>(i + 1U) * 0.421f +
                               static_cast<float>(ordinal + 1) * 0.313f;
            const float targetZ = std::clamp(convergence.z + layerUnit(district) * depthWindow +
                                                 (layer - 0.5f) * minimumDimension * 0.12f,
                                             -minimumDimension * 0.92f,
                                             minimumDimension * 2.48f);
            const float localBlend = blend * std::clamp(0.72f + strength * 0.36f, 0.62f, 1.04f);
            const Vec3 before = object.position;
            object.position.z = mix(object.position.z, targetZ, localBlend);

            Vec3 scaleBias{1.0f, 1.0f, 1.0f};
            Vec3 roleMotion{};
            switch (district) {
            case MusicalRoleDistrict3D::Bass:
                scaleBias = Vec3{0.94f, 1.0f + strength * 0.10f, 1.0f + metrics.bass * 0.12f + metrics.dropIntensity * 0.12f};
                object.position.y += minimumDimension * localBlend * (0.020f + metrics.bass * 0.030f);
                roleMotion.z -= minimumDimension * metrics.bass * strength * 0.012f;
                break;
            case MusicalRoleDistrict3D::Drums:
                scaleBias = Vec3{0.90f, 1.0f + beat * 0.12f, 0.96f};
                object.position.x = mix(object.position.x,
                                        object.position.x + std::round(lane * 3.0f) * minimumDimension * 0.014f,
                                        localBlend);
                roleMotion.y += minimumDimension * beat * strength * 0.012f;
                break;
            case MusicalRoleDistrict3D::Melody:
                scaleBias = Vec3{0.96f, 0.94f, 1.0f + harmonic * 0.14f};
                object.position.y -= minimumDimension * localBlend * (0.026f + harmonic * 0.024f);
                roleMotion.x += std::sin(seed + static_cast<float>(time) * 0.020f) *
                                minimumDimension * harmonic * strength * 0.008f;
                break;
            case MusicalRoleDistrict3D::Harmony:
                scaleBias = Vec3{1.0f + harmonic * 0.12f, 0.96f, 1.0f + metrics.phraseConfidence * 0.10f};
                object.position.x = mix(object.position.x,
                                        object.position.x + (ordinal % 2 == 0 ? -1.0f : 1.0f) *
                                                              minimumDimension * harmonic * 0.018f,
                                        localBlend);
                break;
            case MusicalRoleDistrict3D::Space:
                scaleBias = Vec3{1.0f + metrics.stereoWidth * 0.16f, 0.96f, 1.0f + role.space * 0.12f};
                object.position.x *= 1.0f + localBlend * metrics.stereoWidth * 0.08f;
                object.color.a = clamp01(object.color.a * (0.96f + role.space * 0.04f));
                break;
            case MusicalRoleDistrict3D::Fracture:
                scaleBias = Vec3{1.0f + transient * 0.10f, 0.92f + transient * 0.04f, 0.94f};
                object.position.x += (lane >= 0.0f ? 1.0f : -1.0f) *
                                     minimumDimension * localBlend * (0.020f + transient * 0.024f);
                object.rotation.y += (lane >= 0.0f ? 1.0f : -1.0f) * transient * localBlend * 0.12f;
                roleMotion.x += (lane >= 0.0f ? 1.0f : -1.0f) * minimumDimension * transient * strength * 0.010f;
                break;
            case MusicalRoleDistrict3D::Shadow:
                scaleBias = Vec3{0.92f, 1.0f + role.shadow * 0.16f, 0.96f};
                object.position.y += minimumDimension * localBlend * (0.020f + role.shadow * 0.030f);
                object.rotation.x *= 0.86f;
                object.rotation.z *= 0.74f;
                break;
            }

            const float styleGain = localBlend *
                                    std::clamp(0.46f + strength * 0.34f + (1.0f - stability) * 0.18f,
                                               0.38f,
                                               0.92f);
            const float styleSide = std::sin(seed * 2.7f) >= 0.0f ? 1.0f : -1.0f;
            switch (settings.motionStyle) {
            case MotionStyle::Smooth:
                object.rotation = scale(object.rotation, 0.82f);
                roleMotion = scale(roleMotion, 0.58f);
                object.glow *= 0.98f;
                break;
            case MotionStyle::Mechanical: {
                const float cell = std::max(1.0f, minimumDimension * 0.034f);
                object.position.x = std::round(object.position.x / cell) * cell;
                object.position.z = std::round(object.position.z / (cell * 1.30f)) * (cell * 1.30f);
                object.rotation.x = std::round(object.rotation.x * 12.0f) / 12.0f;
                object.rotation.z = std::round(object.rotation.z * 16.0f) / 16.0f;
                object.scale.y *= 1.0f + beat * styleGain * 0.16f;
                break;
            }
            case MotionStyle::Liquid:
                object.position.x += std::sin(seed + static_cast<float>(time) * 0.060f) *
                                     minimumDimension * styleGain * 0.070f;
                object.position.y += std::cos(seed * 0.7f + static_cast<float>(time) * 0.045f) *
                                     minimumDimension * styleGain * 0.032f;
                object.rotation.y += std::sin(seed) * styleGain * 0.18f;
                break;
            case MotionStyle::Hyperspace:
                object.position.z += styleSide * minimumDimension * styleGain * (0.11f + role.space * 0.040f);
                object.position.x *= 1.0f + styleGain * 0.045f;
                object.rotation.y += styleSide * styleGain * 0.28f;
                object.scale.z *= 1.0f + styleGain * 0.16f;
                break;
            case MotionStyle::HeavyBass:
                object.position.y += minimumDimension * metrics.bass * styleGain * 0.060f;
                object.position.z -= minimumDimension * metrics.bass * styleGain * 0.090f;
                object.scale.y *= 1.0f + metrics.bass * styleGain * 0.12f;
                object.scale.z *= 1.0f + metrics.bass * styleGain * 0.14f;
                object.glow *= 1.0f + metrics.bass * styleGain * 0.06f;
                break;
            case MotionStyle::AmbientDrift:
                object.position.z += minimumDimension * metrics.stereoWidth * styleGain * 0.12f;
                object.position.x *= 1.0f + metrics.stereoWidth * styleGain * 0.055f;
                object.rotation.x *= 0.72f;
                object.rotation.z *= 0.76f;
                object.glow *= 0.96f + metrics.stereoWidth * styleGain * 0.05f;
                break;
            case MotionStyle::Breakbeat:
                object.position.x += styleSide * minimumDimension * transient * styleGain * 0.13f;
                object.position.z += (((ordinal + static_cast<int>(i)) % 2) == 0 ? -1.0f : 1.0f) *
                                     minimumDimension * transient * styleGain * 0.10f;
                object.rotation.y += styleSide * transient * styleGain * 0.34f;
                object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 6.0f)) *
                                    (kPi / 6.0f);
                object.glow *= 1.0f + transient * styleGain * 0.10f;
                break;
            }

            object.scale = multiply(object.scale, mix(Vec3{1.0f, 1.0f, 1.0f}, scaleBias, localBlend * 0.80f));
            object.velocity = add(object.velocity, add(subtract(object.position, before), roleMotion));
            if (object.kind == Object3DKind::Link) {
                object.target.z = mix(object.target.z, targetZ, localBlend * 0.70f);
                object.target = add(object.target, scale(subtract(object.position, before), 0.56f));
            }
            continue;
        }

        if (bridgeRole || convergenceRole) {
            const float eventVisibility = clamp01(earnedConvergence * 0.72f + role.convergence * 0.46f);
            object.color.a *= std::clamp(0.22f + eventVisibility * 0.70f, 0.16f, convergenceRole ? 0.92f : 0.72f);
            object.glow *= std::clamp(0.28f + eventVisibility * 0.72f, 0.18f, 0.94f);
            if (eventVisibility < 0.30f) {
                object.position = mix(object.position, convergence, 0.020f + staging * 0.040f);
                if (object.kind == Object3DKind::Link) {
                    object.target = mix(object.target, convergence, 0.018f + staging * 0.035f);
                }
            }
        }
    }
...
void applyMusicalRoleRepresentativeClarity3D(std::vector<Object3D>& objects,
                                             const MusicRoleScene3D& role,
                                             const SceneInterpretation& intent,
                                             const AudioMetrics& metrics,
                                             const VisualSettings& settings,
                                             ...
                                             double time)
{
    ...
    const float activeMass = role.bass + role.drums + role.melody + role.harmony +
                             role.space + role.fracture + role.shadow;
    if (activeCount < 1.2f && metrics.rms < 0.040f) {
        return;
    }
    if (settings.interactiveField && settings.interactionDepth > 0.75f) {
        return;
    }
    ...
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.56f +
                                            ...
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.16f : 0.0f) +
                                            metrics.buildTension * metrics.phraseConfidence * 0.10f);
    const float roleAuthority = clamp01(role.separation * 0.54f +
                                        std::min(activeCount / 7.0f, 1.0f) * 0.18f +
                                        std::min(activeMass / 3.8f, 1.0f) * 0.12f +
                                        clarity * 0.12f +
                                        stability * 0.06f +
                                        personality * 0.08f);
    const float partition = clamp01(roleAuthority *
                                    (1.0f - earnedConvergence * 0.34f - role.convergence * 0.18f));
    const float motionTame = std::clamp(0.66f +
                                            (1.0f - stability) * 0.18f +
                                            (1.0f - clarity) * 0.12f,
                                        0.62f,
                                        0.98f);
    const float interactionReserve = settings.interactiveField
                                         ? std::clamp(1.0f - settings.interactionDepth * 0.34f, 0.62f, 1.0f)
                                         : 1.0f;
    const float eventPermission = clamp01(earnedConvergence * 0.62f +
                                          role.convergence * 0.42f +
                                          metrics.convergenceRole * 0.28f);
    const Vec3 convergencePoint = roleConvergencePoint3D(role, minimumDimension);
    const bool darkMonolith = identity == SongSceneIdentity::DarkMonolith;
    const bool shadowDominant =
        darkMonolith &&
        role.shadow > std::max({role.bass, role.drums, role.melody, role.fracture}) - 0.06f;
    ...
    const auto roleColor = [&](MusicalRoleDistrict3D district, float alpha) {
        ...
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            color = darkMonolith ? ColorRGBA{0.42f, 0.44f, 0.54f, alpha}
                                 : ColorRGBA{1.0f, 0.19f, 0.07f, alpha};
            break;
        case MusicalRoleDistrict3D::Drums:
            color = ColorRGBA{0.04f, 0.96f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Melody:
            color = ColorRGBA{1.0f, 0.18f, 0.88f, alpha};
            break;
        case MusicalRoleDistrict3D::Harmony:
            color = ColorRGBA{0.38f, 0.82f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Space:
            color = ColorRGBA{0.18f, 0.36f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Fracture:
            color = ColorRGBA{1.0f, 0.96f, 0.10f, alpha};
            break;
        case MusicalRoleDistrict3D::Shadow:
            color = darkMonolith ? ColorRGBA{0.86f, 0.86f, 0.92f, alpha}
                                 : ColorRGBA{0.70f, 0.72f, 0.86f, alpha};
            break;
        }
        return shiftedHue(color, settings.hueShift);
    };
    const auto pulseForRole = [&](MusicalRoleDistrict3D district) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return clamp01(metrics.bass * 0.62f + metrics.lowMid * 0.22f + metrics.dropIntensity * 0.28f);
        case MusicalRoleDistrict3D::Drums:
            return clamp01(metrics.beatConfidence * 0.56f + metrics.barConfidence * 0.26f + beat * 0.18f);
        case MusicalRoleDistrict3D::Melody:
            return clamp01(harmonic * 0.72f + metrics.melodyRole * 0.18f + metrics.keyConfidence * 0.10f);
        case MusicalRoleDistrict3D::Harmony:
            return clamp01(harmonic * 0.60f + metrics.harmonyRole * 0.22f + metrics.phraseConfidence * 0.18f);
        case MusicalRoleDistrict3D::Space:
            return clamp01(metrics.stereoWidth * 0.70f + role.space * 0.26f + intent.spacious * 0.12f);
        case MusicalRoleDistrict3D::Fracture:
            return clamp01(transient * 0.78f + metrics.fractureRole * 0.22f);
        case MusicalRoleDistrict3D::Shadow:
            return clamp01(role.shadow * 0.62f + intent.dark * 0.18f + (1.0f - metrics.treble) * 0.20f);
        }
        return 0.0f;
    };

    std::array<int, 7> roleOrdinals{};
    std::array<Vec3, 7> roleCenters{};
    const std::array<MusicalRoleDistrict3D, 7> districts{
        MusicalRoleDistrict3D::Bass,
        MusicalRoleDistrict3D::Drums,
        MusicalRoleDistrict3D::Melody,
    ...
    for (MusicalRoleDistrict3D district : districts) {
        roleCenters[roleSlot(district)] =
            roleDistrictCenter3D(district, role, intent, metrics, settings, minimumDimension, time);
    }
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        ...
        if (explicitRole) {
            ...
            const float strength = clamp01(roleDistrictStrength(role, district));
            if (strength <= 0.020f) {
                object.color.a *= 0.72f;
                object.glow *= 0.70f;
                continue;
            }

            const int ordinal = roleOrdinals[slot]++;
            const float lane = static_cast<float>((ordinal % 9) - 4) / 4.0f;
            const float layer = static_cast<float>((ordinal / 9) % 6) / 5.0f;
            const float seed = static_cast<float>(i + 1U) * 0.613f +
                               static_cast<float>(ordinal + 1) * 0.337f +
                               static_cast<float>(static_cast<int>(object.kind)) * 0.211f;
            const float pulse = pulseForRole(district);
            const Vec3 center = roleCenters[slot];
            Vec3 local{};
            Vec3 scaleBias{1.0f, 1.0f, 1.0f};
            Vec3 rotationBias{};
            Vec3 roleImpulse{};
            float tintAmount = 0.24f + partition * 0.30f + strength * 0.14f;
            float glowLift = pulse * strength * (0.10f + response * 0.08f);
            float hold = std::clamp(0.16f +
                                        partition * (0.26f + strength * 0.20f) +
                                        clarity * 0.060f -
                                        earnedConvergence * 0.080f,
                                    0.14f,
                                    object.kind == Object3DKind::Link ? 0.46f : 0.68f);
            hold *= motionTame * interactionReserve;

            switch (district) {
            case MusicalRoleDistrict3D::Bass:
                local = Vec3{lane * minimumDimension * (0.065f + metrics.stereoWidth * 0.030f),
                             minimumDimension * (0.060f + pulse * 0.080f + layer * 0.030f),
                             -minimumDimension * (0.24f + pulse * 0.24f + layer * 0.085f)};
                scaleBias = Vec3{0.46f, 0.74f + pulse * 0.10f, 2.72f + metrics.dropIntensity * 0.42f};
                object.rotation.z *= 0.34f;
                object.rotation.x *= 0.68f;
                roleImpulse = Vec3{0.0f,
                                   minimumDimension * pulse * strength * 0.012f,
                                   -minimumDimension * pulse * strength * 0.030f};
                glowLift += metrics.bass * strength * 0.12f;
                break;
            case MusicalRoleDistrict3D::Drums: {
                const float cell = minimumDimension * (0.052f + metrics.beatConfidence * 0.016f);
                const float step = std::round((lane + 1.0f) * 4.0f);
                local = Vec3{(step - 4.0f) * cell,
                             minimumDimension * (lane * 0.028f + pulse * 0.035f),
                             minimumDimension * (-0.08f + layer * 0.46f + pulse * 0.090f)};
                scaleBias = Vec3{0.58f, 1.32f + pulse * 0.30f, 0.62f};
                object.rotation.x = std::round(object.rotation.x * 12.0f) / 12.0f;
                object.rotation.y = std::round(object.rotation.y * 12.0f) / 12.0f;
                object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 8.0f)) * (kPi / 8.0f);
                roleImpulse = Vec3{0.0f,
                                   minimumDimension * pulse * strength * 0.030f,
                                   minimumDimension * pulse * strength * (0.038f + metrics.barConfidence * 0.025f)};
                glowLift += pulse * strength * 0.16f;
                break;
            }
            case MusicalRoleDistrict3D::Melody: {
                const float chroma = chromaAt(metrics, static_cast<std::size_t>(std::max(0, metrics.keyIndex) + ordinal));
                const float angle = metrics.phrasePhase * kPi + phase * 0.018f + seed;
                local = Vec3{std::cos(angle) * minimumDimension * (0.080f + chroma * 0.080f + strength * 0.040f),
                             -minimumDimension * (0.15f + pulse * 0.10f + layer * 0.055f),
                             minimumDimension * (0.05f + chroma * 0.34f + layer * 0.20f)};
                scaleBias = Vec3{0.62f, 0.58f, 1.92f + chroma * 0.30f};
                rotationBias = Vec3{0.0f, pulse * 0.14f, metrics.keyConfidence * 0.10f};
                glowLift += (harmonic + chroma) * strength * 0.12f;
                break;
            }
            case MusicalRoleDistrict3D::Harmony: {
                const float mirror = ordinal % 2 == 0 ? -1.0f : 1.0f;
                local = Vec3{mirror * minimumDimension * (0.12f + layer * 0.12f + metrics.stereoWidth * 0.050f),
                             -minimumDimension * (0.055f + harmonic * 0.045f),
                             minimumDimension * (0.20f + harmonic * 0.18f + layer * 0.25f)};
                scaleBias = Vec3{1.44f + harmonic * 0.22f, 0.72f, 1.28f + pulse * 0.18f};
                object.rotation.z *= 0.66f;
                glowLift += harmonic * strength * 0.10f;
                break;
            }
            case MusicalRoleDistrict3D::Space: {
                const float angle = phase * (0.010f + metrics.stereoWidth * 0.012f) + seed;
                local = Vec3{std::sin(angle) * minimumDimension * (0.20f + metrics.stereoWidth * 0.24f),
                             std::cos(angle * 0.62f) * minimumDimension * (0.030f + role.space * 0.040f),
                             minimumDimension * (0.44f + layer * (0.54f + metrics.stereoWidth * 0.76f) +
                                                 metrics.stereoWidth * 0.18f)};
                scaleBias = Vec3{1.56f + metrics.stereoWidth * 0.26f, 0.66f, 1.34f + role.space * 0.18f};
                object.rotation.x *= 0.44f;
                object.rotation.z *= 0.48f;
                tintAmount *= 0.88f;
                object.color.a *= std::clamp(0.90f + metrics.stereoWidth * 0.08f, 0.88f, 1.04f);
                roleImpulse = Vec3{0.0f,
                                   0.0f,
                                   minimumDimension * metrics.stereoWidth * strength * (0.045f + layer * 0.58f)};
                break;
            }
            case MusicalRoleDistrict3D::Fracture: {
                const float side = ordinal % 2 == 0 ? -1.0f : 1.0f;
                local = Vec3{side * minimumDimension * (0.24f + layer * 0.22f + transient * 0.120f),
                             (lane + std::sin(seed) * 0.32f) * minimumDimension * (0.075f + transient * 0.080f),
                             minimumDimension * (-0.14f + layer * 0.50f + side * transient * 0.130f)};
                scaleBias = Vec3{1.30f + transient * 0.22f, 0.56f + transient * 0.12f, 0.64f + transient * 0.14f};
                rotationBias = Vec3{0.0f, side * (0.16f + transient * 0.22f), 0.0f};
                object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 6.0f)) * (kPi / 6.0f);
                roleImpulse = Vec3{side * minimumDimension * transient * strength * 0.060f,
                                   std::sin(seed * 1.7f) * minimumDimension * transient * strength * 0.024f,
                                   side * minimumDimension * transient * strength * 0.052f};
                hold = std::min(object.kind == Object3DKind::Link ? 0.52f : 0.74f,
                                hold + transient * 0.12f);
                glowLift += transient * strength * (0.30f + response * 0.22f);
                break;
            }
            case MusicalRoleDistrict3D::Shadow:
                local = Vec3{lane * minimumDimension * (0.060f + metrics.stereoWidth * 0.030f),
                             minimumDimension * (0.22f + pulse * 0.13f + layer * 0.060f),
                             minimumDimension * (0.18f + pulse * 0.20f + layer * 0.22f)};
                scaleBias = Vec3{0.54f, 1.62f + pulse * 0.34f, 0.60f};
                object.rotation.x *= 0.26f;
                object.rotation.y *= 0.34f;
                object.rotation.z *= 0.12f;
                glowLift += pulse * strength * 0.060f;
                break;
            }

            if (darkMonolith) {
                if (district == MusicalRoleDistrict3D::Bass) {
                    local.y *= 0.62f;
                    local.z -= minimumDimension * 0.20f;
                    scaleBias = multiply(scaleBias, Vec3{0.72f, 0.78f, 0.92f});
                    tintAmount = std::max(tintAmount, 0.58f);
                    object.color.a *= shadowDominant ? 0.42f : 0.62f;
                    object.glow *= shadowDominant ? 0.38f : 0.58f;
                    hold *= 0.72f;
                } else if (district == MusicalRoleDistrict3D::Shadow) {
                    local.x *= 0.66f;
                    local.y += minimumDimension * (0.10f + role.shadow * 0.13f);
                    local.z += minimumDimension * (0.08f + intent.dark * 0.08f);
                    scaleBias = multiply(scaleBias, Vec3{0.78f, 1.22f, 0.82f});
                    tintAmount = std::max(tintAmount, 0.72f);
                    object.color.a = clamp01(object.color.a * 1.18f + 0.040f);
                    object.glow = object.glow * 0.84f + 0.060f + role.shadow * 0.070f;
                    hold = std::min(0.78f, hold * 1.18f);
                } else if (shadowDominant && district != MusicalRoleDistrict3D::Space) {
                    object.color.a *= 0.72f;
                    object.glow *= 0.72f;
                }
            }
            ...
            const Vec3 roleTarget = add(center,
                                        add(local,
                                            scale(roleAxis,
                                                  minimumDimension *
                                                      (0.075f + strength * 0.085f + partition * 0.060f) *
                                                      (1.0f - earnedConvergence * 0.28f))));
            Vec3 boundedRoleTarget = roleTarget;
            const float forwardDepthLimit = district == MusicalRoleDistrict3D::Space ?
                                                (0.72f + metrics.stereoWidth * 0.76f) :
                                            district == MusicalRoleDistrict3D::Harmony ? 0.72f :
                                            district == MusicalRoleDistrict3D::Melody ? 0.70f :
                                            district == MusicalRoleDistrict3D::Shadow ? 0.72f : 0.58f;
            const float backwardDepthLimit = district == MusicalRoleDistrict3D::Bass ? 0.58f :
                                             district == MusicalRoleDistrict3D::Fracture ? 0.50f : 0.44f;
            boundedRoleTarget.z = center.z +
                                  std::clamp(roleTarget.z - center.z,
                                             -minimumDimension * backwardDepthLimit,
                                             minimumDimension * forwardDepthLimit);
            const Vec3 before = object.position;
            object.position = mix(object.position, boundedRoleTarget, hold);
            object.rotation = add(object.rotation, scale(rotationBias, hold));
            if (eventPermission > 0.28f && earnedConvergence > 0.18f) {
                const Vec3 rendezvous = add(convergencePoint,
                                            scale(roleAxis,
                                                  minimumDimension * (0.16f + strength * 0.10f) *
                                                      (1.0f - earnedConvergence * 0.24f)));
                object.position = mix(object.position, rendezvous, eventPermission * (0.10f + strength * 0.12f));
            }
            object.position = add(object.position, scale(roleImpulse, motionTame));
            if (district == MusicalRoleDistrict3D::Space) {
                const float stereoDepthStretch = 1.0f + metrics.stereoWidth * strength * 0.30f;
                object.position.z = center.z + (object.position.z - center.z) * stereoDepthStretch;
            }

            const Vec3 delta = subtract(object.position, before);
            if (object.kind == Object3DKind::Link) {
                object.target = add(object.target, scale(delta, 0.70f));
            }
            object.scale = multiply(object.scale, mix(Vec3{1.0f, 1.0f, 1.0f}, scaleBias, hold * 0.84f));
            object.color = mix(object.color, roleColor(district, object.color.a), clamp01(tintAmount));
            object.color.a = clamp01(object.color.a * (1.01f + partition * 0.14f + strength * 0.08f));
            object.glow = std::clamp(object.glow + glowLift + partition * 0.035f, 0.0f, 3.0f);
            object.velocity = add(object.velocity, delta);
            continue;
        }

        if (bridgeRole || convergenceRole) {
            const float visibility = std::clamp(0.035f + eventPermission * 0.86f,
                                                0.025f,
                                                convergenceRole ? 0.96f : 0.70f);
            const float darkBridgeDamp = darkMonolith && eventPermission < 0.42f ? 0.56f : 1.0f;
            object.color.a *= visibility * darkBridgeDamp;
            object.glow *= std::clamp(0.12f + eventPermission * 0.88f, 0.08f, 1.06f) * darkBridgeDamp;
            object.scale = scale(object.scale, std::clamp(0.60f + eventPermission * 0.44f, 0.56f, 1.06f));
            if (eventPermission < 0.34f) {
                object.position = mix(object.position, convergencePoint, 0.020f + eventPermission * 0.050f);
                if (object.kind == Object3DKind::Link) {
                    object.target = mix(object.target, convergencePoint, 0.018f + eventPermission * 0.040f);
                }
            }
            continue;
        }

        const float yield = clamp01(partition * (0.42f + role.space * 0.12f + role.shadow * 0.12f));
        if (yield > 0.04f) {
            object.position.z += minimumDimension * yield * (0.12f + metrics.stereoWidth * 0.08f);
            object.color.a *= std::clamp(0.66f - yield * 0.30f, 0.18f, 0.70f);
            object.glow *= std::clamp(0.58f - yield * 0.20f, 0.22f, 0.64f);
            if (darkMonolith && object.kind == Object3DKind::TunnelRib) {
                object.color = mix(object.color, ColorRGBA{0.24f, 0.26f, 0.34f, object.color.a}, 0.72f);
                object.color.a *= 0.46f;
                object.glow *= 0.40f;
                object.scale = multiply(object.scale, Vec3{0.62f, 0.68f, 0.72f});
            } else if (object.kind != Object3DKind::TunnelRib && object.kind != Object3DKind::Column) {
                object.scale = scale(object.scale, 0.88f - yield * 0.08f);
            }
        }
    }

    if (eventPermission > 0.34f && activeCount >= 2.5f) {
        Object3D knot = makeObject3D(Object3DKind::Cage,
                                     convergencePoint,
                                     Vec3{minimumDimension * (0.060f + eventPermission * 0.070f),
                                          minimumDimension * (0.050f + eventPermission * 0.060f),
                                          minimumDimension * (0.070f + eventPermission * 0.090f)},
                                     Vec3{phase * 0.014f,
                                          metrics.beatPhase * kPi * 0.20f,
                                          metrics.phrasePhase * kPi},
                                     roleColor(MusicalRoleDistrict3D::Harmony, 0.10f + eventPermission * 0.18f),
                                     0.16f + eventPermission * 0.42f);
        knot.musicRole = Object3DRole::Convergence;
        objects.push_back(knot);

        int spokes = 0;
        for (MusicalRoleDistrict3D district : districts) {
            const float strength = roleDistrictStrength(role, district);
            if (strength <= 0.18f) {
                continue;
            }

            const Vec3 from = mix(roleCenters[roleSlot(district)],
                                  convergencePoint,
                                  eventPermission * (0.12f + strength * 0.10f));
            const Vec3 to = add(convergencePoint,
                                scale(roleDistrictUnitOffset(district),
                                      minimumDimension * (0.10f + strength * 0.055f) *
                                          (1.0f - eventPermission * 0.20f)));
            Object3D spoke = makeObject3D(Object3DKind::Link,
                                          from,
                                          Vec3{1.0f, 1.0f, 1.0f},
                                          Vec3{},
                                          roleColor(district, 0.032f + eventPermission * 0.060f),
                                          0.030f + eventPermission * 0.11f + strength * 0.030f);
            spoke.target = to;
            spoke.musicRole = Object3DRole::Bridge;
            spoke.velocity = scale(subtract(to, from), 0.014f + eventPermission * 0.018f);
...
void applyStereoSpaceDepthFinalizer3D(std::vector<Object3D>& objects,
                                      const MusicRoleScene3D& role,
                                      const AudioMetrics& metrics,
                                      const VisualSettings& settings,
                                      float minimumDimension)
{
    if (objects.empty() ||
        ...
        (settings.interactiveField && settings.interactionDepth > 0.75f)) {
        return;
    }
    ...
    int count = 0;
    for (const Object3D& object : objects) {
        if (object.musicRole != Object3DRole::Space) {
            continue;
        }
    ...
    centerZ /= static_cast<float>(count);

    const float stereoStretch = std::clamp(1.0f +
                                               (metrics.stereoWidth - 0.25f) *
                                                   role.space *
                                                   (0.22f + depth3DOf(settings) * 0.08f),
                                           1.0f,
                                           1.34f);
    const float stereoDrift = minimumDimension *
                              (metrics.stereoWidth - 0.25f) *
                              role.space *
                              0.30f;
    for (Object3D& object : objects) {
        ...
        object.position.z = centerZ + (object.position.z - centerZ) * stereoStretch + stereoDrift;
        const float deltaZ = object.position.z - previousZ;
        object.velocity.z += deltaZ;
        if (object.kind == Object3DKind::Link) {
            const float previousTargetZ = object.target.z;
            object.target.z = centerZ + (object.target.z - centerZ) * stereoStretch + stereoDrift;
            object.velocity.z += (object.target.z - previousTargetZ) * 0.42f;
        }
    }
}

void applyDarkMonolithFinalDiscipline3D(std::vector<Object3D>& objects,
                                        const MusicRoleScene3D& role,
                                        const SceneInterpretation& intent,
                                        const AudioMetrics& metrics,
                                        const VisualSettings& settings,
                                        ...
                                        double time)
{
    const bool darkBassTransition = identity == SongSceneIdentity::BassPressure &&
    ...
    if (objects.empty() || (identity != SongSceneIdentity::DarkMonolith && !darkBassTransition)) {
        return;
    }

    const float phase = static_cast<float>(time);
    const float shadowAuthority = clamp01(role.shadow * 0.56f +
                                          intent.shadow * 0.24f +
                                          intent.dark * 0.16f +
                                          intent.minimal * 0.12f);
    const bool dominantShadowCue = role.shadow > 0.56f &&
                                   role.shadow > std::max(role.drums, role.melody) + 0.10f &&
                                   role.shadow > role.fracture + 0.08f;
    const bool darkMinimalCue = intent.dark > 0.40f &&
                                intent.minimal > 0.24f &&
                                intent.shadow > 0.30f;
    const bool balancedLayeredScene = activeRoleCount3D(role) >= 4.5f && role.shadow < 0.82f;
    if (!darkBassTransition && balancedLayeredScene) {
        return;
    }
    if (!darkBassTransition &&
        (shadowAuthority < 0.46f || (!dominantShadowCue && !darkMinimalCue))) {
        return;
    }
    const float convergenceEvent = clamp01(metrics.convergenceRole * 0.54f +
                                           metrics.dropIntensity * 0.22f +
                                           (metrics.phraseBoundary ? metrics.phraseConfidence * 0.18f : 0.0f));
    const float tunnelKeep = std::clamp(0.08f + convergenceEvent * 0.16f - shadowAuthority * 0.04f,
                                        0.055f,
                                        0.24f);
    const ColorRGBA coldVoid{0.22f, 0.25f, 0.32f, 1.0f};
    const ColorRGBA coldStone = shiftedHue(ColorRGBA{0.62f, 0.66f, 0.76f, 1.0f}, settings.hueShift * 0.12f);
    const ColorRGBA paleStone = shiftedHue(ColorRGBA{0.86f, 0.86f, 0.92f, 1.0f}, settings.hueShift * 0.16f);

    for (Object3D& object : objects) {
        const bool tunnel = object.kind == Object3DKind::TunnelRib;
        const bool bassPressure = object.musicRole == Object3DRole::Bass &&
                                  object.kind != Object3DKind::Column &&
                                  object.kind != Object3DKind::Anchor;
        const bool transitionBassFoundation = darkBassTransition &&
                                              object.musicRole == Object3DRole::Bass;
        const bool eventGlue = object.musicRole == Object3DRole::Bridge ||
                               object.musicRole == Object3DRole::Convergence;
        const bool shadowMonument = object.musicRole == Object3DRole::Shadow &&
                                    (object.kind == Object3DKind::Column ||
        ...
        const bool nonMonumentSurface = object.kind == Object3DKind::WaveSurface ||
                                        object.kind == Object3DKind::Plate ||
                                        object.kind == Object3DKind::Ribbon ||
                                        object.kind == Object3DKind::Cage ||
                                        object.kind == Object3DKind::Polyhedron ||
                                        object.kind == Object3DKind::Shard;

        if (tunnel || bassPressure || transitionBassFoundation) {
            const float keep = transitionBassFoundation
                                   ? std::min(tunnelKeep, 0.050f + convergenceEvent * 0.050f)
                                   : tunnelKeep;
            object.color = mix(object.color, withAlpha(coldVoid, object.color.a), transitionBassFoundation ? 0.93f : 0.86f);
            object.color.a = clamp01(object.color.a * keep);
            object.glow *= std::clamp(keep + (transitionBassFoundation ? 0.030f : 0.080f),
                                      transitionBassFoundation ? 0.055f : 0.10f,
                                      transitionBassFoundation ? 0.18f : 0.32f);
            object.scale = multiply(object.scale,
                                    transitionBassFoundation ? Vec3{0.34f, 0.46f, 0.42f}
                                                            : (tunnel ? Vec3{0.34f, 0.38f, 0.52f}
                                                                      : Vec3{0.50f, 0.58f, 0.64f}));
            object.position.z += minimumDimension * (0.22f + shadowAuthority * (transitionBassFoundation ? 0.24f : 0.16f));
            object.position.y *= transitionBassFoundation ? 0.58f : 0.78f;
            object.velocity = scale(object.velocity, transitionBassFoundation ? 0.24f : 0.36f);
            continue;
        }

        if (!shadowMonument && (warmPressureColor || nonMonumentSurface)) {
            const float keep = std::clamp(0.14f + convergenceEvent * 0.12f, 0.12f, 0.30f);
            object.color = mix(object.color, withAlpha(coldVoid, object.color.a), warmPressureColor ? 0.90f : 0.72f);
            object.color.a = clamp01(object.color.a * keep);
            object.glow *= std::clamp(keep + 0.08f, 0.14f, 0.36f);
            object.scale = multiply(object.scale, Vec3{0.56f, 0.62f, 0.64f});
            object.position.z += minimumDimension * (0.18f + shadowAuthority * 0.10f);
            object.velocity = scale(object.velocity, 0.46f);
            continue;
        }

        if (eventGlue && convergenceEvent < 0.58f) {
            object.color = mix(object.color, withAlpha(coldVoid, object.color.a), 0.70f);
            object.color.a *= std::clamp(0.16f + convergenceEvent * 0.28f, 0.12f, 0.34f);
            object.glow *= std::clamp(0.18f + convergenceEvent * 0.30f, 0.14f, 0.44f);
            object.scale = scale(object.scale, 0.72f);
            continue;
        }

        if (shadowMonument) {
            const ColorRGBA monumentTone = withAlpha(warmPressureColor ? mix(coldStone, paleStone, 0.34f)
                                                                        : paleStone,
                                                     object.color.a);
            object.color = warmPressureColor ? monumentTone
                                             : mix(object.color, monumentTone, 0.72f);
            object.color.a = clamp01(object.color.a * (warmPressureColor ? 1.06f : 1.22f) + 0.035f);
            object.glow = std::clamp(object.glow * (warmPressureColor ? 0.46f : 0.78f) +
                                         shadowAuthority * (warmPressureColor ? 0.060f : 0.11f),
                                     0.0f,
                                     warmPressureColor ? 0.78f : 1.45f);
            object.scale = multiply(object.scale, Vec3{0.76f, 1.18f + shadowAuthority * 0.18f, 0.82f});
            object.rotation.x *= 0.42f;
    ...
    const int gates = std::max(3,
                               static_cast<int>(std::round(5.0f *
                                                           (0.70f + shadowAuthority * 0.34f) *
                                                           (darkBassTransition ? 1.32f : 1.0f))));
    for (int i = 0; i < gates; ++i) {
        const float unit = gates > 1 ? static_cast<float>(i) / static_cast<float>(gates - 1) : 0.0f;
        const float lane = unit - 0.5f;
        Object3D gate = makeObject3D(Object3DKind::Column,
                                     Vec3{lane * minimumDimension * (0.28f + metrics.stereoWidth * 0.06f),
                                          minimumDimension * (0.22f + shadowAuthority * 0.10f),
                                          minimumDimension * (0.12f + unit * 0.42f + shadowAuthority * 0.10f)},
                                     Vec3{minimumDimension * (0.018f + intent.minimal * 0.010f),
                                          minimumDimension * (0.42f + shadowAuthority * 0.32f + (i % 2 == 0 ? 0.08f : 0.0f)),
                                          minimumDimension * (0.024f + intent.shadow * 0.014f)},
                                     Vec3{0.018f + intent.tension * 0.030f,
                                          lane * 0.055f,
                                          phase * 0.0025f},
                                     withAlpha(paleStone, 0.20f + shadowAuthority * 0.20f),
                                     0.10f + shadowAuthority * 0.20f);
        gate.musicRole = Object3DRole::Shadow;
        gate.velocity = Vec3{0.0f,
                             minimumDimension * shadowAuthority * 0.010f,
                             minimumDimension * shadowAuthority * 0.012f};
        objects.push_back(gate);
    }
}

void applySongIdentitySignatureAccents3D(std::vector<Object3D>& objects,
                                         SongSceneIdentity identity,
                                         const SceneInterpretation& intent,
                                         const MusicRoleScene3D& role,
                                         const AudioMetrics& metrics,
                                         ...
                                         double time)
{
    ...
    const float density = std::clamp(objectDensity3DOf(settings), 0.35f, 1.0f);
    const float personality = scenePersonalityOf(settings);
    ...
    const float alpha = std::clamp(0.12f + personality * 0.08f + colorImpact * 0.07f, 0.11f, 0.32f);
    const float glow = std::clamp(0.10f + personality * 0.08f + lightingGlowOf(settings) * 0.08f, 0.08f, 0.34f);
    const bool reserveInspectionDepth = settings.interactiveField && settings.interactionDepth > 0.82f;
    const float stability = motionStabilityOf(settings);
    const float clarity = patternClarityOf(settings);
    if (stability > 0.96f && clarity > 0.96f) {
        return;
    }
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.54f +
                                            metrics.dropIntensity * 0.30f +
    ...
    const float roleFocus = clamp01(role.separation * 0.56f +
                                    std::min(activeRoles / 7.0f, 1.0f) * 0.18f +
                                    clarity * 0.20f -
                                    earnedConvergence * 0.18f);
    const Vec3 convergencePoint = roleConvergencePoint3D(role, minimumDimension);
    const float accentTame = std::clamp(0.72f +
                                            (1.0f - stability) * 0.18f +
                                            (1.0f - clarity) * 0.16f,
                                        0.62f,
                                        1.0f);
    const float accentReach = std::clamp((0.58f +
                                              personality * 0.08f +
                                              (1.0f - stability) * 0.14f +
                                              (1.0f - clarity) * 0.10f -
                                              earnedConvergence * 0.05f) *
                                             accentTame,
                                         0.50f,
                                         0.78f);

    const auto push = [&](Object3DKind kind,
                          Object3DRole role,
                          Vec3 position,
                          Vec3 scaleValue,
                          Vec3 rotation,
                          ...
                          float objectGlow,
                          Vec3 velocity = Vec3{}) {
        Object3D object = makeObject3D(kind, position, scaleValue, rotation, color, objectGlow);
        object.musicRole = role;
        object.velocity = velocity;
        const float seed = static_cast<float>(objects.size() + 1U) * 0.731f +
                           static_cast<float>(static_cast<int>(kind)) * 0.277f;
        const float sign = std::sin(seed * 2.3f) >= 0.0f ? 1.0f : -1.0f;
        switch (settings.motionStyle) {
        case MotionStyle::Smooth:
            object.rotation = scale(object.rotation, 0.74f);
            object.velocity = scale(object.velocity, 0.54f);
            object.glow *= 0.92f;
            break;
        case MotionStyle::Mechanical: {
            const float cell = std::max(1.0f, minimumDimension * 0.030f);
            object.position.x = std::round(object.position.x / cell) * cell;
            object.position.z = std::round(object.position.z / (cell * 1.25f)) * (cell * 1.25f);
            object.rotation.z = std::round(object.rotation.z * 8.0f) / 8.0f;
            object.glow *= 1.04f;
            break;
        }
        case MotionStyle::Liquid:
            object.position.x += std::sin(seed + phase * 0.070f) * minimumDimension * 0.012f;
            object.position.y += std::cos(seed * 0.7f + phase * 0.050f) * minimumDimension * 0.008f;
            object.rotation.y += std::sin(seed) * 0.070f;
            break;
        case MotionStyle::Hyperspace:
            object.position.z += sign * minimumDimension * 0.018f;
            object.rotation.y += sign * 0.120f;
            object.scale.z *= 1.06f;
            break;
        case MotionStyle::HeavyBass:
            object.position.y += minimumDimension * metrics.bass * 0.012f;
            object.position.z -= minimumDimension * metrics.bass * 0.018f;
            object.scale.y *= 1.0f + metrics.bass * 0.055f;
            object.scale.z *= 1.0f + metrics.bass * 0.060f;
            break;
        case MotionStyle::AmbientDrift:
            object.position.z += minimumDimension * stereo * 0.020f;
            object.rotation.x *= 0.62f;
            object.rotation.z *= 0.66f;
            object.glow *= 0.96f;
            break;
        case MotionStyle::Breakbeat:
            object.position.x += sign * minimumDimension * transient * 0.020f;
            object.rotation.y += sign * transient * 0.130f;
            object.rotation.z += std::round(metrics.beatPhase * 8.0f) * 0.030f;
            object.glow *= 1.0f + transient * 0.08f;
            break;
        }
        object.scale = scale(object.scale, 0.86f + accentTame * 0.14f);
        object.velocity = scale(object.velocity, 0.78f + accentTame * 0.22f);
        object.color.a = clamp01(object.color.a * (0.84f + accentTame * 0.16f));
        object.glow *= 0.78f + accentTame * 0.22f;
        objects.push_back(object);
        return object.position;
    };
    const auto roleColor = [&](int index, float a, float mixAmount = 0.0f) {
        ColorRGBA color = colors[static_cast<std::size_t>(std::clamp(index, 0, 4))];
        if (mixAmount > 0.0f) {
            color = mix(color, colors[4], mixAmount);
        }
        return withAlpha(shiftedHue(color, settings.hueShift * 0.70f), a);
    };
    const auto roleStrength = [&](MusicalRoleDistrict3D district, float identityCue = 0.0f) {
        return clamp01(roleDistrictStrength(role, district) * 0.80f + identityCue * 0.20f);
    };
    const auto accentCount = [&](int baseCount, float strength, float densityBias = 1.0f) {
        if (strength < 0.12f && densityBias < 0.72f) {
            return 0;
        }
        const int shapedBase = std::max(1,
                                        static_cast<int>(std::round(static_cast<float>(baseCount) *
                                                                    (0.50f + strength * 0.70f))));
        return scaledCount(shapedBase, density * densityBias * accentTame);
    };
    const auto districtPoint = [&](Vec3 local, MusicalRoleDistrict3D district, float strength, float mergeBias = 0.0f) {
        const Vec3 rawCenter = roleDistrictCenter3D(district, role, intent, metrics, settings, minimumDimension, time);
        const Vec3 center = mix(convergencePoint, rawCenter, accentReach);
        const float preserveDistrict = std::clamp(0.64f + roleFocus * 0.16f * accentTame + strength * 0.08f -
                                                      earnedConvergence * (0.16f + mergeBias * 0.20f),
                                                  0.52f,
                                                  0.86f);
        return mix(convergencePoint, add(center, scale(local, accentReach)), preserveDistrict);
    };

    switch (identity) {
    case SongSceneIdentity::CalmSpace: {
        const float spaceStrength = roleStrength(MusicalRoleDistrict3D::Space, intent.calm * 0.55f + stereo * 0.34f);
        const int shells = accentCount(3, spaceStrength, 0.54f);
        for (int i = 0; i < shells; ++i) {
            const float unit = shells > 1 ? static_cast<float>(i) / static_cast<float>(shells - 1) : 0.0f;
            push(Object3DKind::DepthPlane,
                 Object3DRole::Space,
                 districtPoint(Vec3{(unit - 0.5f) * minimumDimension * (0.10f + stereo * 0.06f),
                                    -minimumDimension * (0.02f + unit * 0.016f),
                                    minimumDimension * (-0.22f + unit * 0.72f + spaceStrength * 0.10f)},
                               MusicalRoleDistrict3D::Space,
                               spaceStrength),
                 Vec3{minimumDimension * (0.32f + unit * 0.12f),
                      minimumDimension * (0.13f + intent.calm * 0.04f + spaceStrength * 0.035f),
                      minimumDimension * 0.018f},
                 Vec3{0.42f + unit * 0.08f, phase * 0.0015f, unit * 0.06f},
                 roleColor(0, alpha * (0.52f + spaceStrength * 0.32f + unit * 0.14f), 0.34f),
                 glow * (0.42f + spaceStrength * 0.22f));
        }
        break;
    }
    case SongSceneIdentity::BassPressure: {
        const float bassStrength = roleStrength(MusicalRoleDistrict3D::Bass,
                                                metrics.bass * 0.60f + metrics.dropIntensity * 0.36f);
        const int slabs = accentCount(5, bassStrength, 0.56f + metrics.bass * 0.10f);
        for (int i = 0; i < slabs; ++i) {
            const float unit = slabs > 1 ? static_cast<float>(i) / static_cast<float>(slabs - 1) : 0.0f;
            const float lane = unit * 2.0f - 1.0f;
            const float pulse = clamp01(metrics.bass * 0.68f + metrics.dropIntensity * 0.32f);
            push(i % 2 == 0 ? Object3DKind::DepthPlane : Object3DKind::TunnelRib,
                 Object3DRole::Bass,
                 districtPoint(Vec3{lane * minimumDimension * (0.10f + bassStrength * 0.05f),
                                    minimumDimension * (0.02f + pulse * 0.05f),
                                    -minimumDimension * (0.26f + unit * 0.34f + pulse * 0.16f)},
                               MusicalRoleDistrict3D::Bass,
                               bassStrength,
                               metrics.dropIntensity * 0.42f),
                 Vec3{minimumDimension * (0.26f + pulse * 0.10f),
                      minimumDimension * (0.12f + pulse * 0.10f),
                      minimumDimension * (0.026f + pulse * 0.034f + bassStrength * 0.010f)},
                 Vec3{0.54f + pulse * 0.08f, lane * 0.10f, phase * 0.006f},
                 withAlpha(shiftedHue(i % 2 == 0 ? ColorRGBA{1.0f, 0.14f, 0.07f, 1.0f}
                                                  : ColorRGBA{1.0f, 0.08f, 0.42f, 1.0f},
                                      settings.hueShift),
                           alpha * (0.66f + bassStrength * 0.34f) + pulse * 0.08f),
                 glow + pulse * 0.24f + bassStrength * 0.08f,
                 Vec3{0.0f,
                      minimumDimension * pulse * bassStrength * 0.012f,
                      -minimumDimension * pulse * (0.018f + bassStrength * 0.024f)});
        }
        break;
    }
    case SongSceneIdentity::TechnoArchitecture: {
        const float drumStrength = roleStrength(MusicalRoleDistrict3D::Drums,
                                                beat * 0.52f + metrics.barConfidence * 0.32f);
        const int pillars = accentCount(7, drumStrength, 0.58f + beat * 0.10f);
        std::vector<Vec3> anchors;
        anchors.reserve(static_cast<std::size_t>(pillars));
        for (int i = 0; i < pillars; ++i) {
            const float unit = pillars > 1 ? static_cast<float>(i) / static_cast<float>(pillars - 1) : 0.0f;
            const float lane = unit * 2.0f - 1.0f;
            anchors.push_back(push(i % 3 == 0 ? Object3DKind::Cage : Object3DKind::Column,
                                   Object3DRole::Drums,
                                   districtPoint(Vec3{lane * minimumDimension * (0.30f + drumStrength * 0.08f),
                                                      minimumDimension * (0.04f + beat * 0.06f),
                                                      minimumDimension * (-0.18f + unit * 0.50f + beat * 0.08f)},
                                                 MusicalRoleDistrict3D::Drums,
                                                 drumStrength,
                                                 metrics.downbeatConfidence * 0.24f),
                                   Vec3{minimumDimension * (0.020f + beat * 0.010f),
                                        minimumDimension * (0.20f + beat * 0.14f + drumStrength * 0.06f),
                                        minimumDimension * 0.020f},
                                   Vec3{0.0f, 0.0f, std::round((metrics.barPhase + unit) * 8.0f) / 8.0f},
                                   i % 2 == 0 ? roleColor(0, alpha + beat * 0.08f)
                                              : withAlpha(ColorRGBA{1.0f, 0.10f, 0.70f, 1.0f}, alpha + beat * 0.08f),
                                   glow + beat * 0.16f + drumStrength * 0.08f,
                                   Vec3{0.0f,
                                        minimumDimension * beat * drumStrength * 0.020f,
                                        minimumDimension * beat * drumStrength * 0.020f}));
        }
        const std::size_t linkStride = roleFocus > 0.58f && earnedConvergence < 0.32f ? 3U : 2U;
        for (std::size_t i = 1U; i < anchors.size(); i += linkStride) {
            Object3D link = makeObject3D(Object3DKind::Link,
                                         anchors[i - 1U],
                                         Vec3{1.0f, 1.0f, 1.0f},
                                         Vec3{},
                                         roleColor(0, alpha * (0.28f + drumStrength * 0.18f)),
                                         glow * 0.42f + beat * 0.06f);
            link.target = anchors[i];
            link.musicRole = Object3DRole::Drums;
            objects.push_back(link);
        }
        break;
    }
    case SongSceneIdentity::AmbientOrbit: {
        const float spaceStrength = roleStrength(MusicalRoleDistrict3D::Space, stereo * 0.58f + intent.orbital * 0.24f);
        const float harmonyStrength = roleStrength(MusicalRoleDistrict3D::Harmony, harmonic * 0.36f);
        const int orbiters = accentCount(6, std::max(spaceStrength, harmonyStrength), 0.58f + stereo * 0.14f);
        for (int i = 0; i < orbiters; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, orbiters));
            const float angle = unit * 2.0f * kPi + phase * (0.006f + stereo * 0.008f);
            const float radius = minimumDimension * (0.28f + stereo * 0.18f + (i % 3) * 0.06f);
            const bool harmonicSurface = i % 3 == 0;
            const MusicalRoleDistrict3D district = harmonicSurface ? MusicalRoleDistrict3D::Harmony
                                                                   : MusicalRoleDistrict3D::Space;
            const float strength = harmonicSurface ? harmonyStrength : spaceStrength;
            push(i % 3 == 0 ? Object3DKind::WaveSurface : Object3DKind::Orbiter,
                 i % 3 == 0 ? Object3DRole::Harmony : Object3DRole::Space,
                 districtPoint(Vec3{std::cos(angle) * radius * (0.72f + strength * 0.28f),
                                    std::sin(angle * 0.68f) * radius * 0.28f,
                                    minimumDimension * (0.12f + unit * (0.68f + stereo * 0.40f))},
                               district,
                               strength),
                 Vec3{minimumDimension * (i % 3 == 0 ? 0.11f : 0.020f),
                      minimumDimension * (i % 3 == 0 ? 0.055f : 0.020f),
                      minimumDimension * (i % 3 == 0 ? 0.018f : 0.020f)},
                 Vec3{0.28f + stereo * 0.10f, angle * 0.10f, phase * 0.005f},
                 roleColor(3, alpha * (0.58f + strength * 0.28f) + stereo * 0.06f, 0.18f),
                 glow + stereo * 0.08f + strength * 0.07f,
                 Vec3{std::sin(angle) * minimumDimension * stereo * 0.006f,
                      0.0f,
                      std::cos(angle) * minimumDimension * stereo * 0.010f});
        }
        for (int i = 0; i < (reserveInspectionDepth || spaceStrength < 0.26f ? 0 : 2); ++i) {
            const float side = i == 0 ? -1.0f : 1.0f;
            const float depth = i == 0
                                    ? -minimumDimension * (0.18f + stereo * 0.28f)
                                    : minimumDimension * (1.08f + stereo * 1.12f);
            push(Object3DKind::DepthPlane,
                 Object3DRole::Space,
                 districtPoint(Vec3{side * minimumDimension * (0.24f + stereo * 0.18f),
                                    side * minimumDimension * 0.035f,
                                    depth},
                               MusicalRoleDistrict3D::Space,
                               spaceStrength),
                 Vec3{minimumDimension * (0.20f + stereo * 0.16f),
                      minimumDimension * (0.08f + stereo * 0.06f),
                      minimumDimension * (0.018f + stereo * 0.010f)},
                 Vec3{0.34f + stereo * 0.12f, side * 0.10f, phase * 0.004f},
                 roleColor(3, alpha * (0.62f + spaceStrength * 0.30f) + stereo * 0.08f, 0.24f),
                 glow + stereo * 0.10f + spaceStrength * 0.08f,
                 Vec3{side * minimumDimension * stereo * 0.006f,
                      0.0f,
                      side * minimumDimension * stereo * 0.012f});
        }
        break;
    }
    case SongSceneIdentity::MelodicCrystal: {
        const float melodyStrength = roleStrength(MusicalRoleDistrict3D::Melody,
                                                  harmonic * 0.58f + metrics.keyConfidence * 0.26f);
        const int crystals = accentCount(7, melodyStrength, 0.56f + harmonic * 0.12f);
        for (int i = 0; i < crystals; ++i) {
            const float unit = crystals > 1 ? static_cast<float>(i) / static_cast<float>(crystals - 1) : 0.0f;
            const float note = chromaAt(metrics, static_cast<std::size_t>(std::max(0, metrics.keyIndex) + i));
            const float angle = unit * kPi + phase * 0.010f;
            push(i % 3 == 0 ? Object3DKind::Cage : Object3DKind::Shard,
                 Object3DRole::Melody,
                 districtPoint(Vec3{std::cos(angle) * minimumDimension * (0.10f + note * 0.12f),
                                    -minimumDimension * (0.14f + harmonic * 0.10f + unit * 0.035f),
                                    minimumDimension * (0.02f + unit * 0.50f + harmonic * 0.18f)},
                               MusicalRoleDistrict3D::Melody,
                               melodyStrength),
                 Vec3{minimumDimension * (0.020f + note * 0.016f),
                      minimumDimension * (0.082f + harmonic * 0.060f + melodyStrength * 0.030f),
                      minimumDimension * (0.018f + note * 0.018f)},
                 Vec3{0.26f + harmonic * 0.10f, angle * 0.16f, phase * 0.010f + unit},
                 i % 2 == 0 ? withAlpha(ColorRGBA{1.0f, 0.18f, 0.92f, 1.0f},
                                         alpha * (0.62f + melodyStrength * 0.30f) + harmonic * 0.08f)
                            : withAlpha(ColorRGBA{0.10f, 0.94f, 1.0f, 1.0f},
                                        alpha * (0.62f + melodyStrength * 0.30f) + harmonic * 0.08f),
                 glow + harmonic * 0.18f + melodyStrength * 0.08f,
                 Vec3{0.0f,
                      -minimumDimension * harmonic * melodyStrength * 0.010f,
                      minimumDimension * harmonic * (0.010f + melodyStrength * 0.010f)});
        }
        break;
    }
    case SongSceneIdentity::BreakbeatFracture: {
        const float fractureStrength = roleStrength(MusicalRoleDistrict3D::Fracture,
                                                    transient * 0.58f + staggeredCutEnergy3D(metrics) * 0.30f);
        const int cuts = accentCount(7, fractureStrength, 0.58f + transient * 0.14f);
        for (int i = 0; i < cuts; ++i) {
            const float unit = cuts > 1 ? static_cast<float>(i) / static_cast<float>(cuts - 1) : 0.0f;
            const float side = i % 2 == 0 ? -1.0f : 1.0f;
            const float lane = unit * 2.0f - 1.0f;
            push(i % 2 == 0 ? Object3DKind::Plate : Object3DKind::Shard,
                 Object3DRole::Fracture,
                 districtPoint(Vec3{side * minimumDimension * (0.24f + transient * 0.14f),
                                    lane * minimumDimension * (0.07f + transient * 0.05f),
                                    minimumDimension * (-0.14f + unit * 0.60f + side * transient * 0.14f)},
                               MusicalRoleDistrict3D::Fracture,
                               fractureStrength,
                               transient * 0.18f),
                 Vec3{minimumDimension * (0.12f + transient * 0.08f),
                      minimumDimension * (0.020f + transient * 0.024f),
                      minimumDimension * (0.018f + transient * 0.020f)},
                 Vec3{0.18f + transient * 0.20f,
                      side * (0.48f + transient * 0.30f),
                      std::round((metrics.beatPhase + unit) * 6.0f) / 6.0f},
                 i % 2 == 0 ? withAlpha(ColorRGBA{1.0f, 0.95f, 0.10f, 1.0f},
                                         alpha * (0.60f + fractureStrength * 0.32f) + transient * 0.10f)
                            : roleColor(2, alpha * (0.60f + fractureStrength * 0.32f) + transient * 0.08f),
                 glow + transient * 0.24f + fractureStrength * 0.09f,
                 Vec3{side * minimumDimension * transient * fractureStrength * 0.030f,
                      minimumDimension * transient * 0.006f,
                      side * minimumDimension * transient * (0.012f + fractureStrength * 0.022f)});
        }
        break;
    }
    case SongSceneIdentity::DarkMonolith: {
        const float shadowStrength = roleStrength(MusicalRoleDistrict3D::Shadow,
                                                  intent.shadow * 0.54f + intent.dark * 0.28f);
        const int gates = accentCount(5, shadowStrength, 0.54f + intent.shadow * 0.12f);
        for (int i = 0; i < gates; ++i) {
            const float unit = gates > 1 ? static_cast<float>(i) / static_cast<float>(gates - 1) : 0.0f;
            const float lane = unit * 2.0f - 1.0f;
            push(i == gates / 2 ? Object3DKind::Anchor : Object3DKind::Column,
                 Object3DRole::Shadow,
                 districtPoint(Vec3{lane * minimumDimension * (0.16f + intent.minimal * 0.06f),
                                    minimumDimension * (0.16f + intent.shadow * 0.07f),
                                    minimumDimension * (-0.08f + unit * 0.48f + intent.dark * 0.10f)},
                               MusicalRoleDistrict3D::Shadow,
                               shadowStrength),
                 Vec3{minimumDimension * (0.016f + intent.shadow * 0.008f),
                      minimumDimension * (0.30f + intent.shadow * 0.30f + shadowStrength * 0.10f),
                      minimumDimension * (0.020f + intent.minimal * 0.010f)},
                 Vec3{0.014f + intent.tension * 0.03f, lane * 0.04f, phase * 0.0015f},
                 withAlpha(ColorRGBA{0.78f, 0.80f, 0.90f, 1.0f},
                           alpha * (0.62f + shadowStrength * 0.30f) + intent.shadow * 0.06f),
                 glow * 0.62f + intent.shadow * 0.08f + shadowStrength * 0.06f);
        }
        break;
    }
    }

    if (earnedConvergence > 0.38f && !reserveInspectionDepth) {
        struct ConvergingPart {
            MusicalRoleDistrict3D district;
            ColorRGBA color;
            float strength;
        };
        const std::array<ConvergingPart, 7> parts{{
            {MusicalRoleDistrict3D::Bass, ColorRGBA{1.0f, 0.18f, 0.06f, 1.0f}, role.bass},
            {MusicalRoleDistrict3D::Drums, ColorRGBA{0.06f, 0.92f, 1.0f, 1.0f}, role.drums},
            {MusicalRoleDistrict3D::Melody, ColorRGBA{1.0f, 0.18f, 0.86f, 1.0f}, role.melody},
            {MusicalRoleDistrict3D::Harmony, ColorRGBA{0.36f, 0.78f, 1.0f, 1.0f}, role.harmony},
            {MusicalRoleDistrict3D::Space, ColorRGBA{0.20f, 0.36f, 1.0f, 1.0f}, role.space},
            {MusicalRoleDistrict3D::Fracture, ColorRGBA{1.0f, 0.92f, 0.10f, 1.0f}, role.fracture},
            {MusicalRoleDistrict3D::Shadow, ColorRGBA{0.72f, 0.74f, 0.90f, 1.0f}, role.shadow}
        }};

        std::vector<std::pair<Vec3, ColorRGBA>> convergingAnchors;
        convergingAnchors.reserve(parts.size());
        for (const ConvergingPart& part : parts) {
            if (part.strength < 0.20f) {
                continue;
            }
            const Vec3 center = roleDistrictCenter3D(part.district,
                                                     role,
                                                     intent,
                                                     metrics,
                                                     settings,
                                                     minimumDimension,
                                                     time);
            const Vec3 axis = roleDistrictUnitOffset(part.district);
            convergingAnchors.push_back({add(center, scale(axis, minimumDimension * (0.040f + part.strength * 0.050f))),
                                         part.color});
        }

        if (convergingAnchors.size() >= 2U) {
            Object3D knot = makeObject3D(Object3DKind::Cage,
                                         convergencePoint,
                                         Vec3{minimumDimension * (0.060f + earnedConvergence * 0.080f),
                                              minimumDimension * (0.050f + role.convergence * 0.070f),
                                              minimumDimension * (0.070f + earnedConvergence * 0.090f)},
                                         Vec3{phase * 0.018f,
                                              metrics.beatPhase * kPi * 0.12f,
                                              metrics.phrasePhase * kPi},
                                         withAlpha(shiftedHue(colors[0], settings.hueShift),
                                                   alpha * (0.40f + earnedConvergence * 0.44f)),
                                         glow + earnedConvergence * 0.32f);
            knot.musicRole = Object3DRole::Convergence;
            knot.velocity = Vec3{0.0f,
                                 minimumDimension * earnedConvergence * 0.010f,
                                 -minimumDimension * earnedConvergence * 0.016f};
            objects.push_back(knot);

            const std::size_t stride = earnedConvergence > 0.62f ? 1U : 2U;
            for (std::size_t i = 0U; i < convergingAnchors.size(); i += stride) {
                Object3D link = makeObject3D(Object3DKind::Link,
                                             convergingAnchors[i].first,
                                             Vec3{1.0f, 1.0f, 1.0f},
                                             Vec3{},
                                             withAlpha(shiftedHue(convergingAnchors[i].second, settings.hueShift),
                                                       alpha * (0.16f + earnedConvergence * 0.20f)),
                                             glow * 0.38f + earnedConvergence * 0.12f);
                link.target = mix(convergingAnchors[i].first, convergencePoint, 0.38f + earnedConvergence * 0.40f);
                link.musicRole = Object3DRole::Convergence;
                link.velocity = scale(subtract(link.target, link.position), 0.010f + earnedConvergence * 0.018f);
                objects.push_back(link);
            }
        }
    }
...
void applyMusicalCounterpointConvergence3D(std::vector<Object3D>& objects,
                                           const MusicRoleScene3D& role,
                                           const SectionNarrative3D& narrative,
                                           const AudioMetrics& metrics,
                                           const VisualSettings& settings,
                                           ...
                                           double time)
{
    ...
    const float stability = motionStabilityOf(settings);
    const float response = response3DOf(settings);
    const float authority = clamp01(role.separation * 0.46f +
                                    std::min(activeRoles / 7.0f, 1.0f) * 0.22f +
                                    clarity * 0.18f +
                                    response * 0.14f);
    if (authority < 0.12f || metrics.rms < 0.025f) {
        return;
    }
    ...
    const float beat = metrics.beat ? clamp01(0.08f + metrics.beatConfidence * 0.92f)
                                    : metrics.beatConfidence * 0.42f;
    const float earnedConvergence = clamp01(role.convergence * 0.46f +
                                            metrics.convergenceRole * 0.34f +
                                            narrative.drop * 0.24f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.18f : 0.0f));
    const float motionBlend = std::clamp((0.030f + authority * 0.080f + response * 0.035f) *
                                             (0.80f + stability * 0.24f),
                                         0.030f,
                                         0.155f);
    const float stage = std::max(1.0f, minimumDimension);
    const Vec3 convergencePoint = roleConvergencePoint3D(role, minimumDimension);

    std::array<Vec3, 7> centers{};
    std::array<int, 7> counts{};
    const auto slot = [](MusicalRoleDistrict3D district) {
        return static_cast<std::size_t>(district);
    };
    for (const Object3D& object : objects) {
        MusicalRoleDistrict3D district = MusicalRoleDistrict3D::Bass;
        if (!districtForObjectRole(object.musicRole, district) || object.kind == Object3DKind::Link) {
            continue;
        }
        const std::size_t index = slot(district);
        centers[index] = add(centers[index], object.position);
        ++counts[index];
    }
    for (std::size_t i = 0; i < centers.size(); ++i) {
        if (counts[i] > 0) {
            centers[i] = scale(centers[i], 1.0f / static_cast<float>(counts[i]));
        }
    }

    const auto driveFor = [&](MusicalRoleDistrict3D district) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return clamp01(metrics.bass * 0.62f + metrics.lowMid * 0.22f + metrics.dropIntensity * 0.26f);
        case MusicalRoleDistrict3D::Drums:
            return clamp01(beat * 0.60f + metrics.barConfidence * 0.24f + metrics.downbeatConfidence * 0.16f);
        case MusicalRoleDistrict3D::Melody:
            return clamp01(harmonic * 0.74f + metrics.melodyRole * 0.16f + narrative.build * 0.10f);
        case MusicalRoleDistrict3D::Harmony:
            return clamp01(harmonic * 0.58f + metrics.phraseConfidence * 0.24f + narrative.release * 0.18f);
        case MusicalRoleDistrict3D::Space:
            return clamp01(metrics.stereoWidth * 0.70f + narrative.breakdown * 0.24f + role.space * 0.12f);
        case MusicalRoleDistrict3D::Fracture:
            return clamp01(transient * 0.78f + metrics.fractureRole * 0.14f + narrative.drop * 0.14f);
        case MusicalRoleDistrict3D::Shadow:
            return clamp01(role.shadow * 0.58f + (1.0f - metrics.treble) * 0.24f + narrative.breakdown * 0.18f);
        }
        return 0.0f;
    };
    const auto colorFor = [&](MusicalRoleDistrict3D district, float alpha) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return withAlpha(mix(colors[1], ColorRGBA{1.0f, 0.18f, 0.05f, 1.0f}, 0.62f), alpha);
        case MusicalRoleDistrict3D::Drums:
            return withAlpha(mix(colors[0], ColorRGBA{0.04f, 0.90f, 1.0f, 1.0f}, 0.58f), alpha);
        case MusicalRoleDistrict3D::Melody:
            return withAlpha(mix(colors[2], ColorRGBA{1.0f, 0.20f, 0.86f, 1.0f}, 0.52f), alpha);
        case MusicalRoleDistrict3D::Harmony:
            return withAlpha(mix(colors[3], ColorRGBA{0.46f, 0.92f, 1.0f, 1.0f}, 0.48f), alpha);
        case MusicalRoleDistrict3D::Space:
            return withAlpha(mix(colors[4], ColorRGBA{0.12f, 0.38f, 1.0f, 1.0f}, 0.52f), alpha);
        case MusicalRoleDistrict3D::Fracture:
            return withAlpha(mix(colors[2], ColorRGBA{1.0f, 0.92f, 0.08f, 1.0f}, 0.66f), alpha);
        case MusicalRoleDistrict3D::Shadow:
            return withAlpha(mix(colors[4], ColorRGBA{0.70f, 0.72f, 0.86f, 1.0f}, 0.42f), alpha);
        }
        return withAlpha(colors[0], alpha);
    };

    std::array<int, 7> ordinals{};
    for (std::size_t i = 0; i < objects.size(); ++i) {
        Object3D& object = objects[i];
        MusicalRoleDistrict3D district = MusicalRoleDistrict3D::Bass;
        if (!districtForObjectRole(object.musicRole, district) || object.kind == Object3DKind::Link) {
        ...
        const std::size_t index = slot(district);
        const int ordinal = ordinals[index]++;
        const float strength = clamp01(roleDistrictStrength(role, district));
        if (strength <= 0.035f) {
            continue;
        }

        const float drive = driveFor(district);
        const float seed = static_cast<float>(i + 1U) * 0.571f +
                           static_cast<float>(ordinal + 1) * 0.337f +
                           static_cast<float>(static_cast<int>(object.kind)) * 0.149f;
        const float side = std::sin(seed * 2.9f) >= 0.0f ? 1.0f : -1.0f;
        const float lane = static_cast<float>((ordinal % 9) - 4) / 4.0f;
        const float localBlend = motionBlend *
                                 std::clamp(0.70f + strength * 0.32f + drive * 0.18f, 0.64f, 1.14f);
        const Vec3 before = object.position;
        Vec3 target = object.position;
        Vec3 scaleBias{1.0f, 1.0f, 1.0f};
        Vec3 rotationBias{};
        Vec3 semanticVelocity{};

        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            target.z -= stage * strength * (0.035f + drive * 0.095f + narrative.drop * 0.055f);
            target.y += stage * strength * (0.012f + drive * 0.036f);
            target.x = mix(target.x, centers[index].x, 0.045f + drive * 0.035f);
            scaleBias = Vec3{1.0f - drive * 0.030f,
                             1.0f + drive * 0.130f,
                             1.0f + drive * 0.155f + narrative.drop * 0.080f};
            rotationBias.z -= object.rotation.z * 0.14f;
            semanticVelocity.z -= stage * strength * (0.50f + drive * 0.34f + narrative.drop * 0.16f);
            semanticVelocity.y += stage * strength * (0.050f + drive * 0.070f);
            break;
        case MusicalRoleDistrict3D::Drums: {
            const float step = std::round(clamp01(metrics.beatPhase + lane * 0.06f) * 8.0f);
            const float cell = stage * (0.026f + role.drums * 0.010f);
            target.x = std::round(target.x / cell) * cell + side * beat * stage * 0.018f;
            target.y += beat * stage * strength * 0.032f;
            target.z += ((static_cast<int>(step) + ordinal) % 2 == 0 ? 1.0f : -0.60f) *
                        stage * strength * (0.018f + drive * 0.055f);
            scaleBias.y = 1.0f + beat * strength * 0.155f;
            rotationBias.z += std::round(metrics.beatPhase * 8.0f) * 0.018f;
            semanticVelocity.x += side * stage * strength * beat * 0.130f;
            semanticVelocity.y += stage * strength * (0.085f + beat * 0.130f);
            semanticVelocity.z += ((static_cast<int>(step) + ordinal) % 2 == 0 ? 1.0f : -0.55f) *
                                  stage * strength * (0.060f + drive * 0.110f);
            break;
        }
        case MusicalRoleDistrict3D::Melody: {
            const float harmonicPhase = (metrics.keyIndex >= 0
                                        ...
                                        metrics.phrasePhase * kPi +
                                        phase * 0.026f;
            target.x += std::cos(harmonicPhase + seed) * stage * strength * (0.020f + drive * 0.038f);
            target.y -= stage * strength * (0.030f + drive * 0.058f + narrative.build * 0.030f);
            target.z += std::sin(harmonicPhase * 0.72f + seed * 0.4f) * stage * strength * 0.036f +
                        stage * strength * (narrative.build * 0.052f + narrative.release * 0.070f);
            scaleBias.z = 1.0f + drive * strength * 0.125f;
            rotationBias.y += std::sin(harmonicPhase + seed) * drive * 0.095f;
            semanticVelocity.x += std::cos(harmonicPhase + seed) * stage * strength * (0.080f + drive * 0.120f);
            semanticVelocity.y -= stage * strength * (0.260f + drive * 0.240f + narrative.build * 0.080f);
            semanticVelocity.z += stage * strength * (0.120f + drive * 0.150f + narrative.release * 0.080f);
            break;
        }
        case MusicalRoleDistrict3D::Harmony:
            target.x = (target.x >= centers[index].x ? 1.0f : -1.0f) *
                       std::fabs(target.x - centers[index].x) *
                           (1.0f + drive * strength * 0.060f) +
                       centers[index].x;
            target.y -= stage * strength * (narrative.release * 0.026f + harmonic * 0.014f);
            target.z += stage * strength * (0.020f + drive * 0.068f + narrative.release * 0.060f);
            scaleBias.x = 1.0f + drive * strength * 0.115f;
            scaleBias.z = 1.0f + metrics.phraseConfidence * strength * 0.105f;
            rotationBias.x += drive * 0.055f;
            semanticVelocity.x += (target.x >= centers[index].x ? 1.0f : -1.0f) *
                                  stage * strength * (0.100f + drive * 0.110f);
            semanticVelocity.z += stage * strength * (0.170f + drive * 0.170f + narrative.release * 0.100f);
            break;
        case MusicalRoleDistrict3D::Space:
            target.x = centers[index].x + (target.x - centers[index].x) *
                                           (1.0f + drive * strength * 0.155f);
            target.y += std::sin(phase * 0.014f + seed) * stage * strength * 0.018f;
            target.z += stage * strength * (0.085f + drive * 0.135f + narrative.breakdown * 0.160f);
            scaleBias.x = 1.0f + drive * strength * 0.175f;
            scaleBias.z = 1.0f + narrative.breakdown * strength * 0.110f;
            rotationBias.x -= object.rotation.x * 0.080f;
            semanticVelocity.x += (target.x >= centers[index].x ? 1.0f : -1.0f) *
                                  stage * strength * (0.180f + drive * 0.180f);
            semanticVelocity.z += stage * strength * (0.390f + drive * 0.360f + narrative.breakdown * 0.160f);
            break;
        case MusicalRoleDistrict3D::Fracture:
            target.x += side * stage * strength * (0.070f + drive * 0.125f);
            target.y += std::sin(seed * 3.4f + metrics.beatPhase * kPi) * stage * strength * (0.014f + drive * 0.030f);
            target.z += side * stage * strength * (0.030f + drive * 0.090f + narrative.drop * 0.044f);
            scaleBias.y = 1.0f + drive * strength * 0.185f;
            rotationBias.y += side * drive * 0.180f;
            rotationBias.z += side * narrative.drop * 0.090f;
            semanticVelocity.x += side * stage * strength * (0.700f + drive * 0.520f);
            semanticVelocity.z += side * stage * strength * (0.180f + drive * 0.260f + narrative.drop * 0.100f);
            break;
        case MusicalRoleDistrict3D::Shadow:
            target.x = mix(target.x, centers[index].x, 0.035f + drive * 0.030f);
            target.y += stage * strength * (0.030f + drive * 0.068f);
            target.z += stage * strength * (0.030f + narrative.breakdown * 0.100f + drive * 0.046f);
            scaleBias.x = 1.0f - drive * 0.035f;
            scaleBias.y = 1.0f + drive * strength * 0.160f;
            rotationBias.x -= object.rotation.x * 0.140f;
            rotationBias.z -= object.rotation.z * 0.180f;
            semanticVelocity.y += stage * strength * (0.210f + drive * 0.220f + narrative.breakdown * 0.080f);
            semanticVelocity.z += stage * strength * (0.150f + drive * 0.180f);
            break;
        }

        const float convergencePull = clamp01(earnedConvergence * (0.12f + strength * 0.12f) *
                                              (1.0f - role.separation * 0.26f));
        target = mix(target, convergencePoint, convergencePull);
        object.position = mix(object.position, target, localBlend);
        object.scale = multiply(object.scale, mix(Vec3{1.0f, 1.0f, 1.0f}, scaleBias, localBlend * 0.86f));
        object.rotation = add(object.rotation, scale(rotationBias, localBlend));
        object.glow += drive * strength * localBlend * (0.22f + response * 0.16f);
        const Vec3 delta = subtract(object.position, before);
        object.velocity = add(add(scale(object.velocity, 0.62f + stability * 0.16f),
                                  semanticVelocity),
                              scale(delta, 1.0f + drive * 0.55f));
    }

    if (earnedConvergence < 0.30f || activeRoles < 2.5f) {
        return;
    }
    ...
    const std::array<MusicalRoleDistrict3D, 7> order{
        MusicalRoleDistrict3D::Bass,
        MusicalRoleDistrict3D::Drums,
        MusicalRoleDistrict3D::Melody,
        MusicalRoleDistrict3D::Harmony,
    ...
    std::vector<ActiveRole> active;
    active.reserve(order.size());
    for (MusicalRoleDistrict3D district : order) {
        const std::size_t index = slot(district);
        const float strength = clamp01(roleDistrictStrength(role, district));
        if (counts[index] > 0 && strength > 0.18f) {
            active.push_back(ActiveRole{district, centers[index], strength});
        }
    }
    if (active.size() < 2U) {
        return;
    ...
    const int arcLimit = std::min<int>(static_cast<int>(active.size()), earnedConvergence > 0.62f ? 6 : 4);
    for (int i = 0; i < arcLimit; ++i) {
        const ActiveRole& fromRole = active[static_cast<std::size_t>(i)];
        const ActiveRole& toRole = active[static_cast<std::size_t>((i + 1) % static_cast<int>(active.size()))];
        const float pairStrength = clamp01((fromRole.strength + toRole.strength) * 0.5f);
        const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, arcLimit - 1));
        const Vec3 from = mix(fromRole.center, convergencePoint, earnedConvergence * 0.18f);
        const Vec3 to = mix(toRole.center, convergencePoint, earnedConvergence * 0.18f);
        const Vec3 middle = add(mix(mix(from, to, 0.5f), convergencePoint, 0.28f + earnedConvergence * 0.24f),
                                Vec3{std::sin(phase * 0.018f + unit * kPi) * stage * (0.035f + pairStrength * 0.035f),
                                     -stage * (0.050f + harmonic * 0.040f + unit * 0.018f),
                                     stage * (0.055f + metrics.stereoWidth * 0.050f + pairStrength * 0.035f)});
        const ColorRGBA arcColor = shiftedHue(mix(colorFor(fromRole.district, 1.0f),
                                                  colorFor(toRole.district, 1.0f),
                                                  0.5f),
                                             settings.hueShift * 0.66f);
        const float alpha = 0.055f + earnedConvergence * 0.090f + pairStrength * 0.045f;
        const float glow = 0.070f + earnedConvergence * 0.180f + pairStrength * 0.130f;

        Object3D linkA = makeObject3D(Object3DKind::Link,
                                      from,
                                      Vec3{1.0f, 1.0f, 1.0f},
                                      Vec3{},
                                      withAlpha(arcColor, alpha),
                                      glow);
        linkA.target = middle;
        linkA.musicRole = Object3DRole::Convergence;
        linkA.velocity = scale(subtract(middle, from), 0.018f + earnedConvergence * 0.022f);
        objects.push_back(linkA);

        Object3D linkB = makeObject3D(Object3DKind::Link,
                                      middle,
                                      Vec3{1.0f, 1.0f, 1.0f},
                                      Vec3{},
                                      withAlpha(arcColor, alpha * 0.92f),
                                      glow * 0.92f);
        linkB.target = to;
        linkB.musicRole = Object3DRole::Convergence;
        linkB.velocity = scale(subtract(to, middle), 0.018f + earnedConvergence * 0.022f);
        objects.push_back(linkB);

        Object3D bead = makeObject3D(i % 2 == 0 ? Object3DKind::Orbiter : Object3DKind::Shard,
                                     middle,
                                     Vec3{stage * (0.012f + earnedConvergence * 0.010f),
                                          stage * (0.010f + pairStrength * 0.009f),
                                          stage * (0.012f + earnedConvergence * 0.012f)},
                                     Vec3{phase * 0.010f + unit,
                                          metrics.phrasePhase * kPi + unit * 0.4f,
                                          phase * 0.014f},
                                     withAlpha(arcColor, 0.10f + earnedConvergence * 0.14f),
                                     glow + 0.08f);
        bead.musicRole = Object3DRole::Convergence;
        bead.velocity = scale(subtract(convergencePoint, middle), 0.012f + earnedConvergence * 0.018f);
        objects.push_back(bead);
    }
}

void addSongIdentitySceneGrammarFinalizer3D(std::vector<Object3D>& objects,
                                            SongSceneIdentity identity,
                                            const SceneInterpretation& intent,
                                            const AudioMetrics& metrics,
                                            const VisualSettings& settings,
                                            ...
                                            double time)
{
    ...
    const float beat = metrics.beat ? clamp01(0.08f + metrics.beatConfidence * 0.92f)
                                    : metrics.beatConfidence * 0.42f;
    const float stage = std::max(1.0f, minimumDimension);
    const float grammarScale = std::clamp(0.70f + personality * 0.20f + clarity * 0.10f,
                                          0.68f,
                                          1.08f);
    const float grammarResponse = std::clamp(0.82f + response * 0.18f, 0.82f, 1.18f);
    ...
    const auto push = [&](Object3DKind kind,
                          ...
                          ColorRGBA color,
                          float glow,
                          Vec3 velocity = Vec3{}) -> Vec3 {
        Object3D object = makeObject3D(kind,
                                       position,
                                       scale(scaleValue, grammarScale),
                                       rotation,
                                       shiftedHue(color, settings.hueShift * 0.66f),
                                       glow * grammarResponse);
        object.musicRole = role;
        object.velocity = scale(velocity, grammarResponse);
        objects.push_back(object);
        return position;
    };
    const auto link = [&](Vec3 from, Vec3 to, Object3DRole role, ColorRGBA color, float glow) {
        Object3D object = makeObject3D(Object3DKind::Link,
                                       from,
                                       Vec3{1.0f, 1.0f, 1.0f},
                                       Vec3{},
                                       shiftedHue(color, settings.hueShift * 0.66f),
        ...
        object.target = to;
        object.musicRole = role;
        object.velocity = scale(subtract(to, from), 0.010f + glow * 0.010f);
        objects.push_back(object);
    };

    switch (identity) {
    case SongSceneIdentity::CalmSpace: {
        const int rooms = scaledCount(3, density * 0.38f);
        std::vector<Vec3> anchors;
        anchors.reserve(static_cast<std::size_t>(rooms));
        for (int i = 0; i < rooms; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, rooms - 1));
            anchors.push_back(push(Object3DKind::DepthPlane,
                                   Object3DRole::Space,
                                   Vec3{std::sin(phase * 0.006f + unit * kPi) * stage * 0.035f,
                                        (unit - 0.5f) * stage * 0.075f,
                                        stage * (0.10f + unit * 0.76f)},
                                   Vec3{stage * (0.34f + unit * 0.08f),
                                        stage * (0.18f + intent.calm * 0.06f),
                                        stage * 0.014f},
                                   Vec3{0.46f + unit * 0.08f, phase * 0.004f, phase * 0.005f + unit},
                                   withAlpha(mix(colors[4], colors[2], unit * 0.35f), 0.11f + intent.calm * 0.16f),
                                   0.10f + intent.calm * 0.20f,
                                   Vec3{0.0f, 0.0f, stage * 0.018f}));
        }
        for (std::size_t i = 1; i < anchors.size(); ++i) {
            link(anchors[i - 1U], anchors[i], Object3DRole::Harmony, withAlpha(colors[3], 0.07f), 0.08f);
        }
        break;
    }
    case SongSceneIdentity::BassPressure: {
        const int shells = scaledCount(5, density * (0.48f + metrics.bass * 0.30f + metrics.dropIntensity * 0.18f));
        std::vector<Vec3> spine;
        spine.reserve(static_cast<std::size_t>(shells));
        for (int i = 0; i < shells; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, shells - 1));
            const float compression = std::pow(1.0f - unit, 1.7f) * (metrics.bass * 0.18f + metrics.dropIntensity * 0.24f);
            const float radius = stage * (0.20f + unit * 0.17f + metrics.bass * 0.08f - compression * 0.04f);
            spine.push_back(push(Object3DKind::TunnelRib,
                                 Object3DRole::Bass,
                                 Vec3{0.0f,
                                      stage * (0.10f + metrics.bass * 0.07f + compression * 0.05f),
                                      stage * (-0.54f + unit * (1.14f + metrics.dropIntensity * 0.22f) - compression)},
                                 Vec3{radius,
                                      radius * (0.40f + metrics.lowMid * 0.18f),
                                      0.82f + metrics.bass * 0.70f + metrics.dropIntensity * 0.26f},
                                 Vec3{compression * 0.22f, phase * 0.010f, metrics.beatPhase * kPi * 0.18f + unit},
                                 withAlpha(mix(colors[1], ColorRGBA{1.0f, 0.10f, 0.04f, 1.0f}, 0.52f),
                                           0.28f + metrics.bass * 0.24f),
                                 0.34f + metrics.bass * 0.74f + metrics.dropIntensity * 0.32f,
                                 Vec3{0.0f,
                                      stage * metrics.bass * 0.045f,
                                      -stage * (0.34f + metrics.dropIntensity * 0.24f)}));
        }
        for (std::size_t i = 1; i < spine.size(); ++i) {
            link(spine[i - 1U], spine[i], Object3DRole::Bass, withAlpha(colors[1], 0.11f + metrics.bass * 0.06f), 0.12f);
        }
        for (int i = 0; i < 3; ++i) {
            const float side = i == 1 ? 0.0f : (i == 0 ? -1.0f : 1.0f);
            push(Object3DKind::WaveSurface,
                 Object3DRole::Bass,
                 Vec3{side * stage * (0.18f + metrics.stereoWidth * 0.06f),
                      stage * (0.18f + metrics.bass * 0.05f),
                      stage * (-0.30f + static_cast<float>(i) * 0.20f)},
                 Vec3{stage * (0.28f + metrics.bass * 0.10f),
                      stage * (0.070f + metrics.lowMid * 0.040f),
                      stage * (0.018f + metrics.dropIntensity * 0.016f)},
                 Vec3{0.62f, side * 0.18f, metrics.beatPhase * kPi + static_cast<float>(i) * 0.30f},
                 withAlpha(mix(colors[1], colors[3], 0.20f), 0.15f + metrics.bass * 0.18f),
                 0.20f + metrics.bass * 0.34f,
                 Vec3{0.0f, stage * metrics.bass * 0.020f, -stage * metrics.bass * 0.16f});
        }
        break;
    }
    case SongSceneIdentity::TechnoArchitecture: {
        const int lanes = scaledCount(5, density * (0.46f + beat * 0.20f + intent.architecture * 0.18f));
        const int steps = 4;
        std::vector<Vec3> rails;
        rails.reserve(static_cast<std::size_t>(lanes * steps));
        const int activeStep = std::clamp(static_cast<int>(std::floor(clamp01(metrics.barPhase) * steps)), 0, steps - 1);
        for (int lane = 0; lane < lanes; ++lane) {
            const float laneUnit = static_cast<float>(lane) / static_cast<float>(std::max(1, lanes - 1));
            const float x = (laneUnit - 0.5f) * stage * (0.62f + metrics.stereoWidth * 0.18f);
            for (int step = 0; step < steps; ++step) {
                const float stepUnit = static_cast<float>(step) / static_cast<float>(std::max(1, steps - 1));
                const float pulse = step == activeStep ? beat : beat * 0.24f;
                rails.push_back(push(step == activeStep ? Object3DKind::Cage : Object3DKind::Column,
                                     Object3DRole::Drums,
                                     Vec3{x,
                                          stage * (-0.08f + (lane % 2) * 0.045f),
                                          stage * (-0.20f + stepUnit * 0.78f + pulse * 0.08f)},
                                     Vec3{stage * (0.014f + pulse * 0.010f),
                                          stage * (0.12f + pulse * 0.10f),
                                          stage * (0.014f + pulse * 0.010f)},
                                     Vec3{0.0f, laneUnit * 0.08f, activeStep * kPi * 0.10f},
                                     withAlpha(lane % 2 == 0 ? ColorRGBA{0.04f, 0.92f, 1.0f, 1.0f}
                                                             : ColorRGBA{1.0f, 0.10f, 0.62f, 1.0f},
                                               0.18f + pulse * 0.24f),
                                     0.16f + pulse * 0.44f + intent.architecture * 0.16f,
                                     Vec3{stage * pulse * 0.035f, stage * pulse * 0.040f, stage * pulse * 0.060f}));
            }
        }
        for (std::size_t i = 1; i < rails.size(); ++i) {
            if (i % static_cast<std::size_t>(steps) != 0U) {
                link(rails[i - 1U], rails[i], Object3DRole::Drums, withAlpha(colors[i % 5U], 0.085f + beat * 0.06f), 0.10f);
            }
        }
        break;
    }
    case SongSceneIdentity::AmbientOrbit: {
        const int bodies = scaledCount(7, density * (0.42f + metrics.stereoWidth * 0.30f + intent.spacious * 0.18f));
        std::vector<Vec3> orbit;
        orbit.reserve(static_cast<std::size_t>(bodies));
        for (int i = 0; i < bodies; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, bodies));
            const float layer = static_cast<float>(i % 4) / 3.0f;
            const float angle = unit * 2.0f * kPi + phase * (0.010f + metrics.stereoWidth * 0.014f);
            const float radius = stage * (0.22f + layer * 0.12f + metrics.stereoWidth * 0.12f);
            orbit.push_back(push(i % 3 == 0 ? Object3DKind::WaveSurface : Object3DKind::Orbiter,
                                 Object3DRole::Space,
                                 Vec3{std::cos(angle) * radius * (0.82f + metrics.stereoWidth * 0.44f),
                                      std::sin(angle * 0.66f) * radius * 0.40f,
                                      stage * (0.18f + layer * 0.48f + metrics.stereoWidth * 0.18f)},
                                 Vec3{stage * (0.018f + layer * 0.020f + metrics.stereoWidth * 0.010f),
                                      stage * (0.018f + layer * 0.016f),
                                      stage * (0.014f + layer * 0.010f)},
                                 Vec3{0.34f + layer * 0.16f, angle * 0.14f, phase * 0.010f + unit},
                                 withAlpha(mix(colors[4], ColorRGBA{0.10f, 0.46f, 1.0f, 1.0f}, 0.42f),
                                           0.17f + metrics.stereoWidth * 0.16f),
                                 0.16f + intent.spacious * 0.30f + metrics.stereoWidth * 0.14f,
                                 Vec3{std::sin(angle) * stage * metrics.stereoWidth * 0.028f,
                                      0.0f,
                                      stage * (0.16f + metrics.stereoWidth * 0.14f)}));
        }
        for (std::size_t i = 0; i < orbit.size(); i += 2) {
            link(orbit[i], orbit[(i + 3U) % orbit.size()], Object3DRole::Harmony, withAlpha(colors[(i + 1U) % 5U], 0.070f), 0.08f);
        }
        break;
    }
    case SongSceneIdentity::MelodicCrystal: {
        const int notes = scaledCount(8, density * (0.42f + harmonic * 0.28f));
        std::vector<Vec3> melody;
        melody.reserve(static_cast<std::size_t>(notes));
        for (int i = 0; i < notes; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, notes - 1));
            const float chroma = chromaAt(metrics, i);
            const float lane = unit * 2.0f - 1.0f;
            melody.push_back(push(i % 3 == 0 ? Object3DKind::Cage : Object3DKind::Shard,
                                  i % 3 == 0 ? Object3DRole::Harmony : Object3DRole::Melody,
                                  Vec3{lane * stage * (0.24f + metrics.stereoWidth * 0.08f),
                                       -stage * (0.16f + harmonic * 0.070f) +
                                           std::sin(unit * kPi + phase * 0.016f) * stage * 0.035f,
                                       stage * (0.04f + chroma * 0.30f + unit * 0.18f)},
                                  Vec3{stage * (0.026f + chroma * 0.024f),
                                       stage * (0.082f + harmonic * 0.050f),
                                       stage * (0.016f + chroma * 0.014f)},
                                  Vec3{unit * 0.50f, phase * 0.024f + chroma * kPi, metrics.phrasePhase * kPi + lane * 0.26f},
                                  withAlpha(i % 3 == 0 ? ColorRGBA{1.0f, 0.18f, 0.92f, 1.0f}
                                                       : ColorRGBA{0.14f, 0.96f, 1.0f, 1.0f},
                                            0.25f + harmonic * 0.20f + chroma * 0.12f),
                                  0.28f + harmonic * 0.48f + chroma * 0.20f,
                                  Vec3{std::cos(unit * kPi) * stage * harmonic * 0.050f,
                                       -stage * harmonic * 0.170f,
                                       stage * harmonic * 0.120f}));
        }
        for (std::size_t i = 1; i < melody.size(); ++i) {
            link(melody[i - 1U], melody[i], Object3DRole::Harmony, withAlpha(colors[(i + 2U) % 5U], 0.10f + harmonic * 0.08f), 0.11f);
        }
        break;
    }
    case SongSceneIdentity::BreakbeatFracture: {
        const int cuts = scaledCount(8, density * (0.42f + transient * 0.34f + metrics.onset * 0.12f));
        std::vector<Vec3> anchors;
        anchors.reserve(static_cast<std::size_t>(cuts));
        const float quant = std::floor(clamp01(metrics.beatPhase) * 8.0f) / 8.0f;
        for (int i = 0; i < cuts; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, cuts - 1));
            const float side = i % 2 == 0 ? -1.0f : 1.0f;
            const float stagger = static_cast<float>((i * 5 + 3) % 11) / 10.0f;
            anchors.push_back(push(i % 2 == 0 ? Object3DKind::Plate : Object3DKind::Shard,
                                   Object3DRole::Fracture,
                                   Vec3{(unit - 0.5f) * stage * (0.88f + metrics.stereoWidth * 0.20f) +
                                            side * transient * stage * 0.06f,
                                        (stagger - 0.5f) * stage * (0.28f + transient * 0.10f),
                                        stage * (-0.12f + stagger * 0.62f + quant * 0.18f)},
                                   Vec3{stage * (0.028f + transient * 0.038f),
                                        stage * (0.094f + metrics.onset * 0.085f),
                                        stage * (0.014f + transient * 0.022f)},
                                   Vec3{0.42f + stagger * 0.30f, quant * kPi + side * transient * 0.26f, unit * kPi},
                                   withAlpha(i % 3 == 0 ? ColorRGBA{1.0f, 0.42f, 0.10f, 1.0f}
                                                        : ColorRGBA{0.92f, 1.0f, 0.12f, 1.0f},
                                             0.23f + transient * 0.30f),
                                   0.24f + transient * 0.66f + metrics.onset * 0.18f,
                                   Vec3{side * stage * (0.34f + transient * 0.38f),
                                        0.0f,
                                        side * stage * (0.16f + transient * 0.22f)}));
        }
        for (std::size_t i = 2; i < anchors.size(); i += 3) {
            link(anchors[i - 2U], anchors[i], Object3DRole::Fracture, withAlpha(colors[(i + 2U) % 5U], 0.08f + transient * 0.08f), 0.10f);
        }
        break;
    }
    case SongSceneIdentity::DarkMonolith: {
        const int monoliths = scaledCount(4, density * (0.34f + intent.shadow * 0.18f));
        ...
        for (int i = 0; i < monoliths; ++i) {
            ...
            const float height = 0.34f + intent.shadow * 0.20f + metrics.bass * 0.10f;
            const Vec3 position{(unit - 0.5f) * stage * (0.50f + metrics.stereoWidth * 0.14f),
                                stage * (0.12f + intent.dark * 0.05f),
                                stage * (-0.06f + unit * 0.38f)};
            tops.push_back(add(position, Vec3{0.0f, stage * height, 0.0f}));
            push(Object3DKind::Column,
                 Object3DRole::Shadow,
                 position,
                 Vec3{stage * (0.024f + intent.minimal * 0.010f),
                      stage * height,
                      stage * (0.038f + intent.shadow * 0.018f)},
                 Vec3{0.035f + intent.tension * 0.060f, (unit - 0.5f) * 0.09f, phase * 0.006f},
                 withAlpha(mix(colors[3], colors[4], 0.68f), 0.20f + intent.shadow * 0.22f),
                 0.16f + intent.shadow * 0.38f,
                 Vec3{0.0f, stage * intent.shadow * 0.095f, stage * intent.dark * 0.050f});
        }
        for (std::size_t i = 1; i < tops.size(); ++i) {
            link(tops[i - 1U], tops[i], Object3DRole::Shadow, withAlpha(colors[4], 0.065f + intent.shadow * 0.06f), 0.08f);
        }
        push(Object3DKind::Anchor,
             Object3DRole::Shadow,
             Vec3{0.0f, -stage * 0.02f, stage * (0.18f + intent.dark * 0.10f)},
             Vec3{stage * 0.020f, stage * 0.020f, stage * 0.020f},
             Vec3{0.0f, phase * 0.010f, phase * 0.014f},
             withAlpha(colors[4], 0.24f + intent.shadow * 0.16f),
             0.18f + intent.shadow * 0.28f,
             Vec3{0.0f, stage * intent.shadow * 0.050f, stage * 0.040f});
        break;
    }
    }
}
                                            ...
                                            const AudioMetrics& metrics)
{
    ...
    const float expressedHueShift = wrapUnit(settings.hueShift * (1.0f + impact * 0.35f));
    const float accentHue = wrapUnit(expressedHueShift +
                                     harmonicSteer * impact +
                                     phraseSteer * impact +
                                     metrics.spectralCentroid * 0.06f +
                                     metrics.dropIntensity * 0.05f);

    colors = shiftedPalette(colors, expressedHueShift);
    const float energy = clamp01(metrics.rms * 1.35f +
...
struct SongIdentitySelection3D {
    SongSceneIdentity identity = SongSceneIdentity::CalmSpace;
...
struct RoleSceneStats3D {
    ...
    float roleCrosstalk = 0.0f;
    float districtSpread = 0.0f;
    float roleBalance = 0.0f;
    float roleVocabulary = 0.0f;
    float roleSilhouetteContrast = 0.0f;
    float roleLegibility = 0.0f;
    float roleMotionContrast = 0.0f;
    float musicalStructure = 0.0f;
};

struct CameraContinuityStats3D {
    float motion = 0.0f;
...
std::string_view songSceneIdentityName(SongSceneIdentity identity)
{
    switch (identity) {
    case SongSceneIdentity::CalmSpace:
        return "Calm Space";
    case SongSceneIdentity::BassPressure:
        return "Bass Pressure";
    case SongSceneIdentity::TechnoArchitecture:
        return "Techno Architecture";
    case SongSceneIdentity::AmbientOrbit:
        return "Ambient Orbit";
    case SongSceneIdentity::MelodicCrystal:
        return "Melodic Crystal";
    case SongSceneIdentity::BreakbeatFracture:
        return "Breakbeat Fracture";
    case SongSceneIdentity::DarkMonolith:
        return "Dark Monolith";
    }
    return "Unknown Song Identity";
}
                                       ...
                                       VisualMode mode)
{
    ...
    if (mode == VisualMode::SpectralOrigami &&
        metrics.fractureRole > 0.56f &&
        metrics.roleSeparation > 0.48f &&
        metrics.fractureRole > metrics.bassRole + 0.10f) {
        return SongSceneIdentity::BreakbeatFracture;
    }
    ...
    if ((mode == VisualMode::ChromaKaleidoscope ||
        ...
        metrics.melodyRole + metrics.harmonyRole > 0.54f &&
        metrics.harmonyRole > metrics.bassRole + 0.10f &&
    ...
    if (mode == VisualMode::SpectralOrigami &&
        metrics.fractureRole > 0.46f &&
        metrics.fractureRole > metrics.bassRole + 0.10f &&
        (staggeredCut > 0.22f || transient > 0.18f || metrics.spectralFlux > 0.12f)) {
        return SongSceneIdentity::BreakbeatFracture;
    }
    ...
    if (resonanceOrCathedral &&
        metrics.shadowRole > 0.48f &&
        metrics.dropIntensity < 0.50f &&
        metrics.shadowRole > metrics.melodyRole + 0.18f &&
        (metrics.keyMode == MusicalMode::Minor || metrics.harmonicEnergy > 0.20f || intent.dark > 0.24f)) {
        return SongSceneIdentity::DarkMonolith;
    }
    const bool lockedTechnoCue = technoScore > 0.50f &&
                                 metrics.style == AudioStyle::Techno &&
                                 metrics.styleConfidence > 0.62f &&
                                 beatRegularity > 0.42f &&
                                 metrics.dropIntensity < 0.46f &&
                                 (metrics.drumRole <= 0.001f ||
                                  metrics.drumRole > metrics.bassRole + 0.12f ||
                                  metrics.bass < 0.62f);
    if (lockedTechnoCue) {
        return SongSceneIdentity::TechnoArchitecture;
    }
...
float songSceneIdentityCue3D(SongSceneIdentity identity,
                             const SceneInterpretation& intent,
                             const AudioMetrics& metrics)
{
    const float styleWeight = std::clamp(metrics.styleConfidence, 0.0f, 1.0f);
    ...
    const float harmonic = clamp01(metrics.harmonicEnergy * 0.62f +
                                   ...
                                   averageChromaEnergy(metrics) * 0.16f);
    const float melodyRole = std::max(metrics.melodyRole, metrics.harmonyRole);
    const float roleConfidence = clamp01(metrics.roleSeparation * 0.62f +
                                         std::max({metrics.bassRole,
                                                   metrics.drumRole,
                                                   melodyRole,
                                                   metrics.spaceRole,
                                                   metrics.fractureRole,
                                                   metrics.shadowRole}) *
                                             0.42f);
    const bool audible = metrics.rms > 0.035f || metrics.peak > 0.08f;

    switch (identity) {
    case SongSceneIdentity::CalmSpace:
        return clamp01((1.0f - metrics.rms * 6.0f) * 0.44f +
                       intent.calm * 0.38f +
                       (metrics.style == AudioStyle::Silence ? 0.28f + styleWeight * 0.16f : 0.0f) -
                       metrics.beatConfidence * 0.18f);
    case SongSceneIdentity::BassPressure:
        return clamp01(intent.mass * 0.36f +
                       intent.heavy * 0.24f +
                       intent.drop * 0.24f +
                       lowWeight * 0.30f +
                       metrics.bassRole * roleConfidence * 0.42f +
                       metrics.convergenceRole * 0.12f +
                       metrics.dropIntensity * 0.24f +
                       (metrics.style == AudioStyle::BassHeavy ? 0.20f + styleWeight * 0.12f : 0.0f));
    case SongSceneIdentity::TechnoArchitecture:
        return clamp01(intent.architecture * 0.36f +
                       intent.industrial * 0.24f +
                       intent.groove * 0.18f +
                       beatRegularity * 0.30f +
                       metrics.drumRole * 0.16f +
                       metrics.drumRole * roleConfidence * 0.46f +
                       (metrics.style == AudioStyle::Techno ? 0.22f + styleWeight * 0.12f : 0.0f) -
                       staggeredCut * 0.10f);
    case SongSceneIdentity::AmbientOrbit:
        return clamp01(intent.orbital * 0.32f +
                       intent.spacious * 0.34f +
                       metrics.stereoWidth * 0.26f +
                       metrics.spaceRole * roleConfidence * 0.46f +
                       harmonic * 0.12f +
                       (metrics.style == AudioStyle::Ambient || metrics.style == AudioStyle::Wide ? 0.20f + styleWeight * 0.10f : 0.0f) -
                       transient * 0.14f -
                       metrics.dropIntensity * 0.16f);
    case SongSceneIdentity::MelodicCrystal:
        return clamp01(intent.crystal * 0.34f +
                       intent.melodic * 0.30f +
                       intent.bright * 0.12f +
                       harmonic * 0.32f +
                       (metrics.melodyRole + metrics.harmonyRole) * 0.24f +
                       melodyRole * roleConfidence * 0.46f -
                       metrics.dropIntensity * 0.12f -
                       metrics.bass * 0.10f);
    case SongSceneIdentity::BreakbeatFracture:
        return clamp01(intent.fracture * 0.36f +
                       intent.chaotic * 0.24f +
                       transient * 0.34f +
                       staggeredCut * 0.34f +
                       metrics.fractureRole * 0.24f +
                       metrics.fractureRole * roleConfidence * 0.48f +
                       metrics.spectralFlux * 0.16f -
                       metrics.bass * 0.08f);
    case SongSceneIdentity::DarkMonolith:
        return clamp01(intent.shadow * 0.38f +
                       intent.dark * 0.26f +
                       intent.minimal * 0.18f +
                       metrics.shadowRole * 0.26f +
                       metrics.shadowRole * roleConfidence * 0.46f +
                       (1.0f - std::clamp((metrics.highMid + metrics.treble) * 3.0f, 0.0f, 1.0f)) * (audible ? 0.16f : 0.04f) +
                       (metrics.keyMode == MusicalMode::Minor ? metrics.keyConfidence * 0.14f : 0.0f) -
                       (metrics.style == AudioStyle::Ambient ? 0.18f : 0.0f));
    }
    return 0.0f;
}

SongIdentitySelection3D updateSongIdentityMemory3D(SongIdentityMemory& memory,
                                                   SongSceneIdentity candidate,
                                                   const SceneInterpretation& intent,
                                                   const AudioMetrics& metrics,
                                                   double time)
{
    const float candidateCue = songSceneIdentityCue3D(candidate, intent, metrics);
    const bool firstFrame = memory.lastTimeSeconds < 0.0;
    float dt = firstFrame ? 1.0f / 60.0f : static_cast<float>(time - memory.lastTimeSeconds);
    const bool independentFrame = !firstFrame && dt <= 0.0001f;
    if (firstFrame || independentFrame || dt > 8.0f) {
        memory.lastTimeSeconds = time;
        memory.identity = static_cast<int>(candidate);
        memory.confidence = candidateCue;
        memory.continuity = 1.0f;
        return SongIdentitySelection3D{candidate, candidateCue, 1.0f};
    }
    dt = std::clamp(dt, 1.0f / 120.0f, 1.0f);

    const SongSceneIdentity current = static_cast<SongSceneIdentity>(std::clamp(memory.identity, 0, 6));
    const float currentCue = songSceneIdentityCue3D(current, intent, metrics);
    const bool sameIdentity = candidate == current;
    const bool strongDrop = metrics.dropIntensity > 0.62f ||
                            (metrics.section == ArrangementSection::Drop && metrics.sectionConfidence > 0.68f);
    const bool phraseEvent = metrics.phraseBoundary && metrics.phraseConfidence > 0.56f;
    const bool strongCut = candidate == SongSceneIdentity::BreakbeatFracture &&
    ...
    const bool explicitRoleScene = candidate == SongSceneIdentity::BassPressure ||
                                   ...
                                   candidate == SongSceneIdentity::BreakbeatFracture ||
                                   candidate == SongSceneIdentity::DarkMonolith;
    const bool strongRoleIdentity = explicitRoleScene &&
    ...
    const SongSceneIdentity resolved = static_cast<SongSceneIdentity>(std::clamp(memory.identity, 0, 6));
    const float resolvedCue = resolved == candidate ? candidateCue : currentCue;
    const bool heldAmbiguousIdentity = !sameIdentity && !shouldSwitch && resolved == current;
    const float riseRate = shouldSwitch ? 2.2f : (resolved == candidate ? 1.25f : 0.58f);
    const float confidenceAlpha = std::clamp(1.0f - std::exp(-dt * riseRate),
                                             shouldSwitch ? 0.16f : 0.04f,
                                             shouldSwitch ? 0.54f : 0.28f);
    const float confidenceTarget = std::max(resolvedCue, heldAmbiguousIdentity ? currentCue * 0.86f : resolvedCue);
    memory.confidence += (confidenceTarget - memory.confidence) * confidenceAlpha;
    if (!sameIdentity && !shouldSwitch) {
        memory.confidence = std::max(memory.confidence * 0.98f, currentCue * 0.84f);
    }
    const float continuityTarget = shouldSwitch ? 0.34f :
                                   (sameIdentity ? 1.0f : (heldAmbiguousIdentity ? 0.82f : 0.62f));
    const float continuityAlpha = std::clamp(1.0f - std::exp(-dt * (shouldSwitch ? 3.0f : 1.1f)), 0.08f, 0.46f);
    memory.continuity += (continuityTarget - memory.continuity) * continuityAlpha;
    memory.lastTimeSeconds = time;

    return SongIdentitySelection3D{
        resolved,
...
float cameraPoseDistance3D(const Camera3D& camera,
                           const CameraMotionMemory& memory,
                           float minimumDimension)
{
    const float invDimension = 1.0f / std::max(1.0f, minimumDimension);
    const float centerDx = (camera.center.x - memory.centerX) * invDimension;
    const float centerDy = (camera.center.y - memory.centerY) * invDimension;
    const float depthDelta = (camera.cameraDistance - memory.cameraDistance) * invDimension;
    const float yawDelta = (camera.yaw - memory.yaw) * 4.4f;
    const float pitchDelta = (camera.pitch - memory.pitch) * 4.8f;
    const float rollDelta = (camera.roll - memory.roll) * 5.2f;
    return std::sqrt(centerDx * centerDx +
                     centerDy * centerDy +
...
void storeCameraMotionMemory(CameraMotionMemory& memory, const Camera3D& camera, double time)
{
    memory.lastTimeSeconds = time;
    memory.centerX = camera.center.x;
    memory.centerY = camera.center.y;
    memory.cameraDistance = camera.cameraDistance;
    memory.yaw = camera.yaw;
    memory.pitch = camera.pitch;
    memory.roll = camera.roll;
}

CameraContinuityStats3D applyCameraContinuity3D(Camera3D& camera,
                                                CameraMotionMemory& memory,
                                                const AudioMetrics& metrics,
                                                const VisualSettings& settings,
                                                const MusicSceneArc3D& arc,
                                                float minimumDimension,
                                                double time)
{
    float dt = 1.0f / 60.0f;
    if (memory.lastTimeSeconds >= 0.0) {
        dt = static_cast<float>(time - memory.lastTimeSeconds);
    }

    const bool centerJumped = memory.lastTimeSeconds >= 0.0 &&
                              (std::fabs(camera.center.x - memory.centerX) > minimumDimension * 0.78f ||
                               std::fabs(camera.center.y - memory.centerY) > minimumDimension * 0.78f);
    if (memory.lastTimeSeconds < 0.0 ||
        dt <= 0.0001f ||
        dt > 8.0f ||
        centerJumped) {
        storeCameraMotionMemory(memory, camera, time);
        return CameraContinuityStats3D{0.0f, 1.0f};
    }
    dt = std::clamp(dt, 1.0f / 120.0f, 0.75f);

    const float rawMotion = cameraPoseDistance3D(camera, memory, minimumDimension);
    if (rawMotion <= 0.0005f) {
        storeCameraMotionMemory(memory, camera, time);
        return CameraContinuityStats3D{0.0f, 1.0f};
    }
    ...
    const float stability = motionStabilityOf(settings);
    const float clarity = patternClarityOf(settings);
    const float impact = clamp01(std::max(arc.impact, metrics.dropIntensity) +
                                 (metrics.downbeat ? metrics.downbeatConfidence * 0.10f : 0.0f));
    const float responseSeconds = std::clamp(0.055f +
                                                 stability * 0.18f +
                                                 clarity * 0.060f -
                                                 impact * 0.090f -
                                                 metrics.beatConfidence * 0.020f,
                                             0.045f,
                                             0.32f);
    float alpha = 1.0f - std::exp(-dt / responseSeconds);
    const float minimumAlpha = std::clamp(0.050f +
                                              impact * 0.16f +
                                              (1.0f - stability) * 0.12f +
                                              metrics.dropIntensity * 0.06f,
                                          0.045f,
                                          0.42f);
    alpha = std::clamp(alpha, minimumAlpha, 0.94f);

    const CameraMotionMemory previous = memory;
    camera.center.x = previous.centerX + (camera.center.x - previous.centerX) * alpha;
    camera.center.y = previous.centerY + (camera.center.y - previous.centerY) * alpha;
    camera.cameraDistance = previous.cameraDistance + (camera.cameraDistance - previous.cameraDistance) * alpha;
    camera.yaw = previous.yaw + (camera.yaw - previous.yaw) * alpha;
    camera.pitch = previous.pitch + (camera.pitch - previous.pitch) * alpha;
    camera.roll = previous.roll + (camera.roll - previous.roll) * alpha;

    const float appliedMotion = cameraPoseDistance3D(camera, previous, minimumDimension);
    storeCameraMotionMemory(memory, camera, time);
    return CameraContinuityStats3D{
        appliedMotion,
        clamp01(1.0f - appliedMotion / std::max(0.001f, rawMotion))
    };
}
                                  ...
                                  float minimumDimension)
{
    ...
    for (const Object3D& object : objects) {
        ...
        if (index >= 0) {
            ...
            velocitySums[roleSlot] = add(velocitySums[roleSlot], scale(object.velocity, mass));
    ...
    const float unassignedMass = std::max(0.0f, totalVisualMass - explicitRoleMass - bridgeMass);
    stats.roleCrosstalk = clamp01((bridgeMass + unassignedMass * 0.32f) / std::max(1.0f, totalVisualMass));
    ...
    for (std::size_t a = 0; a < activeRoleSlots.size(); ++a) {
        for (std::size_t b = a + 1U; b < activeRoleSlots.size(); ++b) {
            ...
            const Vec3 leftVelocity = scale(velocitySums[left], 1.0f / leftMass);
            const Vec3 rightVelocity = scale(velocitySums[right], 1.0f / rightMass);
            motionDistance += std::clamp(length(subtract(leftVelocity, rightVelocity)) /
                                             std::max(1.0f, minimumDimension * 0.045f),
                                         0.0f,
                                         1.0f);
            ++rolePairs;
        }
    ...
    if (rolePairs > 0) {
        ...
        stats.roleMotionContrast = motionDistance * invPairs;
    } else if (activeRoleSlots.size() == 1U) {
        stats.roleVocabulary = 1.0f;
        stats.roleSilhouetteContrast = 1.0f;
        stats.roleMotionContrast = 1.0f;
    }
    stats.roleLegibility = clamp01(stats.explicitRoleShare * 0.36f +
                                   clamp01(stats.districtSpread * 0.82f) * 0.24f +
                                   stats.roleVocabulary * 0.20f +
                                   stats.roleSilhouetteContrast * 0.14f +
                                   stats.roleBalance * 0.10f -
                                   stats.roleCrosstalk * 0.28f);
    stats.musicalStructure = clamp01(stats.roleLegibility * 0.44f +
                                     stats.roleMotionContrast * 0.24f +
                                     stats.roleVocabulary * 0.12f +
                                     stats.roleSilhouetteContrast * 0.10f +
                                     (1.0f - stats.roleCrosstalk) * 0.08f +
                                     stats.roleBalance * 0.06f -
                                     stats.bridgeShare * (0.06f + role.convergence * 0.10f));
    return stats;
}
...
ColorRGBA shade3DColor(ColorRGBA color, float depthUnit, float glow, float light, float lightingGlow)
{
    ...
    const float chromaFloor = 0.08f + lightingGlow * 0.08f;
    color.r = clamp01(color.r * (value + chromaFloor) + glow * lightingGlow * 0.05f);
    color.g = clamp01(color.g * (value + chromaFloor) + glow * lightingGlow * 0.05f);
    color.b = clamp01(color.b * (value + chromaFloor) + glow * lightingGlow * 0.05f);
                      ...
                      bool closed = false)
{
    ...
    frame.polylines.push_back(Polyline{
        ...
        width * 1.35f,
        ColorRGBA{color.r * 0.32f, color.g * 0.32f, color.b * 0.32f, color.a * 0.06f},
        closed
    });
                          ...
                          bool closed)
{
    ...
    frame.polylines.push_back(Polyline{
        std::move(shadow),
        width * 1.25f,
        ColorRGBA{color.r * 0.32f, color.g * 0.32f, color.b * 0.32f, color.a * 0.05f},
        closed
    });
    frame.polylines.push_back(Polyline{
        std::move(projected),
                                                 ...
                                                 float time)
{
    ...
    if (interaction.pressed && depthStrength > 0.001f) {
        const float sceneDolly = std::min(width, height) *
                                 (0.010f + depthStrength * 0.018f) *
                                 readability *
                                 std::clamp(0.70f + interaction.strength * 0.22f + interaction.velocity * 0.16f,
                                            0.70f,
                                            1.12f);
        for (Object3D& object : objects) {
            object.position.z -= sceneDolly;
            object.velocity.z -= sceneDolly * 0.44f;
            if (object.kind == Object3DKind::Link) {
                object.target.z -= sceneDolly * 0.72f;
            }
        }
        stats.focus += (sceneDolly / std::max(1.0f, std::min(width, height))) *
    ...
    for (Object3D& object : objects) {
        ...
        const Vec3 parallaxShift{
            ...
            std::clamp(-normalizedY * verticalLimit * parallaxSign * influence,
                       -verticalLimit,
                       verticalLimit),
            std::clamp((-nearWeight * focusLimit * 1.18f + farWeight * focusLimit * 0.52f) *
                           influence *
                           (0.88f + ripple * 0.12f),
                       -focusLimit,
                       focusLimit * 0.64f)
        ...
        if (object.kind == Object3DKind::Link) {
            ...
            const Vec3 targetShift{
                ...
                std::clamp(-normalizedY * verticalLimit * 0.72f * targetParallaxSign * influence,
                           -verticalLimit * 0.72f,
                           verticalLimit * 0.72f),
                std::clamp((-(1.0f - targetDepthUnit) * focusLimit * 1.18f +
                            targetDepthUnit * focusLimit * 0.52f) *
                               influence * 0.72f,
                           -focusLimit * 0.72f,
                           focusLimit * 0.48f)
            };
        ...
        if (isExplicitMusicalPartRole(object.musicRole)) {
            const float surfaceDamp = object.kind == Object3DKind::DepthPlane ||
                                      ...
                                      object.kind == Object3DKind::Plate
                                          ? 0.035f
                                          : 0.015f;
            object.color.a *= std::clamp(1.03f - roleCrowding * (0.055f + surfaceDamp), 0.82f, 1.06f);
            object.glow *= std::clamp(1.00f - roleCrowding * 0.055f, 0.84f, 1.04f);
        } else if (object.musicRole == Object3DRole::Bridge || object.musicRole == Object3DRole::Convergence) {
            object.color.a *= std::clamp(0.44f + role.convergence * 0.30f, 0.30f, 0.82f);
            object.glow *= std::clamp(0.46f + role.convergence * 0.34f, 0.34f, 0.84f);
        } else {
            object.color.a *= std::clamp(0.68f - roleCrowding * 0.22f + role.convergence * 0.10f, 0.32f, 0.76f);
            object.glow *= std::clamp(0.62f - roleCrowding * 0.18f + role.convergence * 0.10f, 0.34f, 0.72f);
        }
    }
}

Object3DRole songIdentityRoleForObject3D(SongSceneIdentity identity, Object3DKind kind, std::size_t ordinal)
{
    switch (identity) {
    case SongSceneIdentity::CalmSpace:
        if (kind == Object3DKind::DepthPlane || kind == Object3DKind::WaveSurface || kind == Object3DKind::Orbiter) {
            return Object3DRole::Space;
        }
        if (kind == Object3DKind::Link) {
            return Object3DRole::Harmony;
        }
        return ordinal % 3U == 0U ? Object3DRole::Harmony : Object3DRole::Space;
    case SongSceneIdentity::BassPressure:
        if (kind == Object3DKind::Link) {
            return Object3DRole::Convergence;
        }
        if (kind == Object3DKind::Anchor || kind == Object3DKind::DepthPlane) {
            return Object3DRole::Bass;
        }
        return kind == Object3DKind::Plate || kind == Object3DKind::Shard ? Object3DRole::Fracture : Object3DRole::Bass;
    case SongSceneIdentity::TechnoArchitecture:
        if (kind == Object3DKind::TunnelRib) {
            return Object3DRole::Bass;
        }
        if (kind == Object3DKind::Link || kind == Object3DKind::Column || kind == Object3DKind::Cage ||
            kind == Object3DKind::Plate) {
            return Object3DRole::Drums;
        }
        return ordinal % 4U == 0U ? Object3DRole::Harmony : Object3DRole::Drums;
    case SongSceneIdentity::AmbientOrbit:
        if (kind == Object3DKind::Link) {
            return Object3DRole::Harmony;
        }
        if (kind == Object3DKind::Cage || kind == Object3DKind::Node) {
            return Object3DRole::Harmony;
        }
        return Object3DRole::Space;
    case SongSceneIdentity::MelodicCrystal:
        if (kind == Object3DKind::Link || kind == Object3DKind::Cage || kind == Object3DKind::WaveSurface) {
            return Object3DRole::Harmony;
        }
        if (kind == Object3DKind::DepthPlane || kind == Object3DKind::Orbiter) {
            return ordinal % 2U == 0U ? Object3DRole::Space : Object3DRole::Harmony;
        }
        return Object3DRole::Melody;
    case SongSceneIdentity::BreakbeatFracture:
        if (kind == Object3DKind::Column || kind == Object3DKind::Cage) {
            return Object3DRole::Drums;
        }
        if (kind == Object3DKind::Link) {
            return Object3DRole::Fracture;
        }
        return kind == Object3DKind::TunnelRib ? Object3DRole::Bass : Object3DRole::Fracture;
    case SongSceneIdentity::DarkMonolith:
        if (kind == Object3DKind::TunnelRib || (kind == Object3DKind::Column && ordinal % 5U == 0U)) {
            return Object3DRole::Bass;
        }
        if (kind == Object3DKind::Link) {
            return Object3DRole::Bridge;
        }
        return Object3DRole::Shadow;
    }
    return Object3DRole::Unassigned;
                                ...
                                double time)
{
    ...
    for (std::size_t i = firstIdentityObject; i < objects.size(); ++i) {
        ...
        objects[i].musicRole = songIdentityRoleForObject3D(identity, objects[i].kind, i - firstIdentityObject);
    }
}

void addSongIdentityHeroArchitecture3D(std::vector<Object3D>& objects,
                                       SongSceneIdentity identity,
                                       const SceneInterpretation& intent,
                                       const AudioMetrics& metrics,
                                       const std::array<ColorRGBA, 5>& colors,
                                       ...
                                       double time)
{
    const float phase = static_cast<float>(time);
    const float transient = transientEnergy3D(metrics);
    const float beat = metrics.beat ? std::max(metrics.beatConfidence, 0.24f) : metrics.beatConfidence * 0.42f;
    const float harmonic = clamp01(metrics.harmonicEnergy * 0.62f +
                                   metrics.keyConfidence * 0.26f +
                                   averageChromaEnergy(metrics) * 0.20f);
    const float stereo = clamp01(metrics.stereoWidth);
    const float stereoSpread = 0.76f + stereo * 0.72f;
    const float phrase = clamp01(metrics.phraseIntensity * 0.58f + metrics.phraseConfidence * 0.28f);
    const float heroGain = std::clamp(0.78f + personality * 0.18f + std::min(response, 1.8f) * 0.06f,
                                      0.74f,
                                      1.08f);

    const auto roleVelocity = [&](Object3DRole role, float strength, float seed) {
        switch (role) {
        case Object3DRole::Bass:
            return Vec3{0.0f,
                        minimumDimension * (0.006f + metrics.bass * 0.020f) * strength,
                        -minimumDimension * (0.018f + metrics.dropIntensity * 0.040f) * strength};
        case Object3DRole::Drums:
            return Vec3{std::sin(seed + metrics.beatPhase * kPi) * minimumDimension * beat * 0.018f * strength,
                        0.0f,
                        minimumDimension * beat * 0.026f * strength};
        case Object3DRole::Melody:
            return Vec3{std::cos(seed + metrics.phrasePhase * kPi) * minimumDimension * harmonic * 0.018f * strength,
                        -minimumDimension * harmonic * 0.020f * strength,
                        std::sin(seed * 0.7f) * minimumDimension * harmonic * 0.016f * strength};
        case Object3DRole::Harmony:
            return Vec3{std::sin(seed * 0.6f) * minimumDimension * harmonic * 0.012f * strength,
                        0.0f,
                        minimumDimension * phrase * 0.016f * strength};
        case Object3DRole::Space:
            return Vec3{std::sin(seed + phase * 0.014f) * minimumDimension * stereo * 0.012f * strength,
                        std::cos(seed * 0.5f + phase * 0.010f) * minimumDimension * 0.010f * strength,
                        minimumDimension * (0.014f + stereo * 0.018f) * strength};
        case Object3DRole::Fracture:
            return Vec3{std::sin(seed * 3.0f) * minimumDimension * transient * 0.038f * strength,
                        0.0f,
                        std::cos(seed * 2.0f) * minimumDimension * transient * 0.030f * strength};
        case Object3DRole::Shadow:
            return Vec3{0.0f,
                        minimumDimension * (0.006f + intent.shadow * 0.018f) * strength,
                        minimumDimension * (0.010f + intent.dark * 0.018f) * strength};
        case Object3DRole::Bridge:
        case Object3DRole::Convergence:
        case Object3DRole::Unassigned:
            break;
    ...
    const auto pushHeroObject = [&](Object3DKind kind,
                                    Object3DRole role,
                                    Vec3 position,
                                    Vec3 scaleValue,
                                    Vec3 rotation,
        ...
        Object3D object = makeObject3D(kind,
                                       position,
                                       scale(scaleValue, heroGain),
                                       rotation,
                                       color,
                                       glow);
        object.musicRole = role;
        object.velocity = roleVelocity(role, strength, position.x * 0.007f + position.z * 0.005f + phase);
        objects.push_back(object);
        return position;
    };
    const auto pushHeroLink = [&](Vec3 from, Vec3 to, Object3DRole role, ColorRGBA color, float glow) {
        Object3D link = makeObject3D(Object3DKind::Link,
                                     from,
                                     Vec3{1.0f, 1.0f, 1.0f},
                                     Vec3{},
        ...
        link.target = to;
        link.musicRole = role;
        link.velocity = roleVelocity(role, 0.5f, from.x * 0.004f + to.z * 0.004f + phase);
        objects.push_back(link);
    };

    switch (identity) {
    case SongSceneIdentity::CalmSpace: {
        const int rooms = scaledCount(4, density * 0.46f);
        std::vector<Vec3> roomAnchors;
        roomAnchors.reserve(static_cast<std::size_t>(rooms));
        for (int i = 0; i < rooms; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, rooms - 1));
            Vec3 center{std::sin(unit * kPi + phase * 0.010f) * minimumDimension * 0.045f,
                        (unit - 0.5f) * minimumDimension * 0.060f,
                        minimumDimension * (0.02f + unit * (0.92f + intent.calm * 0.18f))};
            roomAnchors.push_back(pushHeroObject(Object3DKind::DepthPlane,
                                                 Object3DRole::Space,
                                                 center,
                                                 Vec3{minimumDimension * (0.30f + unit * 0.10f),
                                                      minimumDimension * (0.18f + intent.calm * 0.08f),
                                                      minimumDimension * 0.018f},
                                                 Vec3{0.48f + unit * 0.08f,
                                                      phase * 0.004f,
                                                      phase * 0.006f + unit * 0.3f},
                                                 withAlpha(mix(colors[4], colors[2], unit * 0.42f),
                                                           0.10f + intent.calm * 0.18f),
                                                 0.10f + intent.calm * 0.25f,
                                                 0.62f + intent.calm * 0.20f));
        }
        for (std::size_t i = 1; i < roomAnchors.size(); ++i) {
            pushHeroLink(roomAnchors[i - 1U],
                         roomAnchors[i],
                         Object3DRole::Harmony,
                         withAlpha(colors[3], 0.060f + intent.calm * 0.060f),
                         0.06f + intent.calm * 0.12f);
        }
        break;
    }
    case SongSceneIdentity::BassPressure: {
        const int ribs = scaledCount(8, density * (0.70f + metrics.bass * 0.34f + metrics.dropIntensity * 0.18f));
        std::vector<Vec3> spine;
        spine.reserve(static_cast<std::size_t>(ribs));
        for (int i = 0; i < ribs; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, ribs - 1));
            const float compression = std::pow(1.0f - unit, 1.55f) *
                                      (0.16f + metrics.bass * 0.18f + metrics.dropIntensity * 0.26f);
            const float radius = minimumDimension *
                                 (0.24f + unit * 0.24f + metrics.bass * 0.08f - compression * 0.05f);
            Vec3 center{0.0f,
                        minimumDimension * (0.060f + metrics.bass * 0.045f),
                        minimumDimension * (-0.62f + unit * (1.46f + metrics.dropIntensity * 0.34f) - compression)};
            spine.push_back(pushHeroObject(Object3DKind::TunnelRib,
                                           Object3DRole::Bass,
                                           center,
                                           Vec3{radius,
                                                radius * (0.42f + metrics.lowMid * 0.18f),
                                                0.74f + metrics.bass * 0.80f + metrics.dropIntensity * 0.34f},
                                           Vec3{compression * 0.22f,
                                                phase * 0.012f,
                                                metrics.beatPhase * kPi * 0.18f + unit * 0.36f},
                                           withAlpha(mix(colors[1], colors[0], unit * 0.25f),
                                                     0.27f + metrics.bass * 0.24f),
                                           0.34f + metrics.bass * 0.82f + metrics.dropIntensity * 0.44f,
                                           0.80f + metrics.bass * 0.34f));
        }
        const int weights = scaledCount(6, density * (0.56f + metrics.bass * 0.28f));
        for (int i = 0; i < weights; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, weights - 1));
            const float side = i % 2 == 0 ? -1.0f : 1.0f;
            pushHeroObject(Object3DKind::Column,
                           Object3DRole::Bass,
                           Vec3{side * minimumDimension * (0.21f + unit * 0.10f) * stereoSpread,
                                minimumDimension * (0.16f + metrics.bass * 0.08f),
                                minimumDimension * (-0.34f + unit * 0.68f - metrics.dropIntensity * 0.16f)},
                           Vec3{minimumDimension * (0.040f + metrics.bass * 0.020f),
                                minimumDimension * (0.34f + metrics.bass * 0.30f + metrics.dropIntensity * 0.14f),
                                minimumDimension * (0.048f + metrics.bass * 0.020f)},
                           Vec3{0.04f,
                                side * (0.12f + metrics.bass * 0.08f),
                                phase * 0.010f + side * metrics.beatPhase * 0.16f},
                           withAlpha(mix(colors[1], colors[3], 0.36f), 0.25f + metrics.bass * 0.26f),
                           0.30f + metrics.bass * response * 0.58f,
                           0.72f + metrics.bass * 0.28f);
        }
        for (std::size_t i = 1; i < spine.size(); i += 2) {
            pushHeroLink(spine[i - 1U],
                         spine[i],
                         Object3DRole::Convergence,
                         withAlpha(colors[1], 0.10f + metrics.dropIntensity * 0.11f),
                         0.12f + metrics.dropIntensity * 0.28f);
        }
        break;
    }
    case SongSceneIdentity::TechnoArchitecture: {
        const int lanes = scaledCount(5, density * (0.64f + intent.architecture * 0.30f));
        const int steps = scaledCount(6, density * (0.58f + beat * 0.20f));
        std::vector<Vec3> gateAnchors;
        gateAnchors.reserve(static_cast<std::size_t>(lanes * steps));
        const float currentStep = std::floor(clamp01(metrics.barPhase) * static_cast<float>(std::max(1, steps)));
        for (int lane = 0; lane < lanes; ++lane) {
            const float u = static_cast<float>(lane) / static_cast<float>(std::max(1, lanes - 1));
            const float x = (u - 0.5f) * minimumDimension * 0.70f * stereoSpread;
            for (int step = 0; step < steps; ++step) {
                const float v = static_cast<float>(step) / static_cast<float>(std::max(1, steps - 1));
                const float active = (step == static_cast<int>(currentStep)) ? beat : beat * 0.24f;
                Vec3 position{x,
                              minimumDimension * (-0.12f + (lane % 2) * 0.055f),
                              minimumDimension * (-0.24f + v * 0.92f + active * 0.08f)};
                gateAnchors.push_back(pushHeroObject((step + lane) % 3 == 0 ? Object3DKind::Cage : Object3DKind::Column,
                                                     Object3DRole::Drums,
                                                     position,
                                                     Vec3{minimumDimension * (0.014f + active * 0.014f),
                                                          minimumDimension * (0.13f + active * 0.12f +
                                                                              intent.architecture * 0.05f),
                                                          minimumDimension * (0.016f + active * 0.012f)},
                                                     Vec3{0.0f,
                                                          (lane - lanes / 2) * 0.025f,
                                                          currentStep * 0.20f + active * 0.12f},
                                                     withAlpha(colors[(lane + step) % 5], 0.18f + active * 0.24f),
                                                     0.16f + active * 0.52f + intent.architecture * 0.20f,
                                                     0.70f + beat * 0.24f));
            }
        }
        for (std::size_t i = 1; i < gateAnchors.size(); ++i) {
            if (i % static_cast<std::size_t>(std::max(1, steps)) == 0U) {
                continue;
            }
            pushHeroLink(gateAnchors[i - 1U],
                         gateAnchors[i],
                         Object3DRole::Drums,
                         withAlpha(colors[i % 5U], 0.08f + beat * 0.10f),
                         0.10f + beat * 0.18f);
        }
        pushHeroObject(Object3DKind::Plate,
                       Object3DRole::Drums,
                       Vec3{0.0f, minimumDimension * 0.015f, minimumDimension * (0.22f + beat * 0.05f)},
                       Vec3{minimumDimension * (0.34f + intent.architecture * 0.10f),
                            minimumDimension * (0.030f + beat * 0.030f),
                            minimumDimension * (0.028f + beat * 0.020f)},
                       Vec3{0.18f, 0.0f, currentStep * kPi * 0.25f},
                       withAlpha(colors[0], 0.14f + beat * 0.20f),
                       0.16f + beat * 0.34f,
                       0.68f + beat * 0.26f);
        break;
    }
    case SongSceneIdentity::AmbientOrbit: {
        const int bodies = scaledCount(10, density * (0.54f + intent.spacious * 0.36f));
        std::vector<Vec3> orbitAnchors;
        orbitAnchors.reserve(static_cast<std::size_t>(bodies));
        for (int i = 0; i < bodies; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, bodies));
            const float layer = static_cast<float>(i % 4) / 3.0f;
            const float angle = unit * 2.0f * kPi +
                                phase * (0.012f + stereo * 0.016f) +
                                metrics.phrasePhase * kPi * 0.32f;
            const float radius = minimumDimension * (0.22f + layer * 0.12f + stereo * 0.10f);
            Vec3 position{std::cos(angle) * radius * stereoSpread,
                          std::sin(angle * 0.68f) * radius * 0.44f,
                          minimumDimension * (0.02f + layer * 0.52f + intent.spacious * 0.16f)};
            orbitAnchors.push_back(pushHeroObject(i % 4 == 0 ? Object3DKind::WaveSurface : Object3DKind::Orbiter,
                                                  Object3DRole::Space,
                                                  position,
                                                  Vec3{minimumDimension * (0.018f + layer * 0.022f + stereo * 0.010f),
                                                       minimumDimension * (0.018f + layer * 0.018f),
                                                       minimumDimension * (0.012f + layer * 0.010f)},
                                                  Vec3{0.34f + layer * 0.16f,
                                                       angle * 0.16f,
                                                       phase * 0.012f + unit},
                                                  withAlpha(colors[(i + 4) % 5],
                                                            0.17f + stereo * 0.18f + intent.spacious * 0.10f),
                                                  0.16f + intent.spacious * 0.34f + stereo * 0.16f,
                                                  0.66f + intent.spacious * 0.28f));
        }
        for (std::size_t i = 0; i < orbitAnchors.size(); i += 2) {
            pushHeroLink(orbitAnchors[i],
                         orbitAnchors[(i + 3U) % orbitAnchors.size()],
                         Object3DRole::Harmony,
                         withAlpha(colors[(i + 1U) % 5U], 0.065f + intent.spacious * 0.080f),
                         0.07f + intent.spacious * 0.15f);
        }
        break;
    }
    case SongSceneIdentity::MelodicCrystal: {
        const int notes = scaledCount(14 + static_cast<int>(std::round(harmonic * 5.0f)),
                                      density * (0.62f + harmonic * 0.28f + intent.crystal * 0.20f));
        std::vector<Vec3> melodyPath;
        melodyPath.reserve(static_cast<std::size_t>(notes));
        for (int i = 0; i < notes; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, notes - 1));
            const float chroma = chromaAt(metrics, i);
            const float angle = unit * 1.45f * kPi + metrics.phrasePhase * kPi * 0.58f;
            Vec3 position{(unit - 0.5f) * minimumDimension * 0.74f * stereoSpread,
                          -minimumDimension * (0.10f + chroma * 0.22f + harmonic * 0.08f) +
                              std::sin(angle) * minimumDimension * 0.045f,
                          minimumDimension * (0.00f + chroma * 0.58f + std::sin(angle * 0.6f) * 0.10f +
                                              harmonic * 0.10f)};
            const Object3DRole role = i % 4 == 0 ? Object3DRole::Harmony : Object3DRole::Melody;
            melodyPath.push_back(pushHeroObject(i % 5 == 0 ? Object3DKind::Cage :
                                                (i % 2 == 0 ? Object3DKind::Shard : Object3DKind::Node),
                                                role,
                                                position,
                                                Vec3{minimumDimension * (0.018f + chroma * 0.030f),
                                                     minimumDimension * (0.034f + harmonic * 0.045f),
                                                     minimumDimension * (0.014f + chroma * 0.020f)},
                                                Vec3{angle * 0.14f,
                                                     phase * 0.038f + chroma * kPi,
                                                     harmonic * kPi + metrics.keyConfidence * 0.25f},
                                                withAlpha(colors[(i + 2) % 5],
                                                          0.24f + chroma * 0.26f + harmonic * 0.12f),
                                                0.22f + chroma * response * 0.44f + harmonic * 0.38f,
                                                0.68f + harmonic * 0.32f));
        }
        for (std::size_t i = 1; i < melodyPath.size(); ++i) {
            pushHeroLink(melodyPath[i - 1U],
                         melodyPath[i],
                         i % 3U == 0U ? Object3DRole::Harmony : Object3DRole::Melody,
                         withAlpha(colors[(i + 3U) % 5U], 0.09f + harmonic * 0.15f),
                         0.10f + harmonic * 0.24f);
        }
        break;
    }
    case SongSceneIdentity::BreakbeatFracture: {
        const int cuts = scaledCount(16, density * (0.68f + transient * 0.44f + metrics.onset * 0.18f));
        std::vector<Vec3> cutAnchors;
        cutAnchors.reserve(static_cast<std::size_t>(cuts));
        const float quant = std::floor(clamp01(metrics.beatPhase) * 8.0f) / 8.0f;
        for (int i = 0; i < cuts; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, cuts - 1));
            const float stagger = static_cast<float>((i * 5 + 3) % 11) / 10.0f;
            const float side = i % 2 == 0 ? -1.0f : 1.0f;
            Vec3 position{(unit - 0.5f) * minimumDimension * 0.88f * stereoSpread +
                              side * transient * minimumDimension * 0.035f,
                          (stagger - 0.5f) * minimumDimension * (0.34f + transient * 0.12f),
                          minimumDimension * (-0.20f + stagger * 0.78f + quant * 0.16f)};
            cutAnchors.push_back(pushHeroObject(i % 3 == 0 ? Object3DKind::Plate : Object3DKind::Shard,
                                                Object3DRole::Fracture,
                                                position,
                                                Vec3{minimumDimension * (0.025f + transient * 0.038f),
                                                     minimumDimension * (0.070f + metrics.onset * 0.090f),
                                                     minimumDimension * (0.014f + transient * 0.024f)},
                                                Vec3{0.40f + stagger * 0.36f,
                                                     quant * kPi + side * transient * 0.22f,
                                                     std::floor((unit + quant) * 8.0f) * kPi * 0.16f},
                                                withAlpha(colors[(i + 3) % 5], 0.22f + transient * 0.32f),
                                                0.23f + transient * 0.70f + metrics.onset * 0.24f,
                                                0.68f + transient * 0.34f));
        }
        for (std::size_t i = 2; i < cutAnchors.size(); i += 3) {
            pushHeroLink(cutAnchors[i - 2U],
                         cutAnchors[i],
                         Object3DRole::Fracture,
                         withAlpha(colors[(i + 2U) % 5U], 0.075f + transient * 0.12f),
                         0.08f + transient * 0.22f);
        }
        pushHeroObject(Object3DKind::Cage,
                       Object3DRole::Drums,
                       Vec3{-minimumDimension * 0.22f, -minimumDimension * 0.02f, minimumDimension * (0.18f + quant * 0.22f)},
                       Vec3{minimumDimension * 0.050f,
                            minimumDimension * (0.18f + beat * 0.10f),
                            minimumDimension * 0.050f},
                       Vec3{0.0f, quant * kPi, phase * 0.020f},
                       withAlpha(colors[0], 0.16f + beat * 0.16f),
                       0.14f + beat * 0.30f,
                       0.58f + beat * 0.24f);
        break;
    }
    case SongSceneIdentity::DarkMonolith: {
        const int monoliths = scaledCount(12, density * (0.50f + intent.shadow * 0.28f + intent.minimal * 0.12f));
        std::vector<Vec3> crowns;
        crowns.reserve(static_cast<std::size_t>(monoliths));
        for (int i = 0; i < monoliths; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, monoliths - 1));
            const float x = (unit - 0.5f) * minimumDimension * (0.52f + stereo * 0.08f);
            const float height = 0.34f + intent.shadow * 0.24f + metrics.bass * 0.08f + (i % 2 == 0 ? 0.08f : 0.0f);
            Vec3 base{x,
                      minimumDimension * (0.13f + intent.dark * 0.05f),
                      minimumDimension * (-0.08f + unit * 0.44f + intent.minimal * 0.10f)};
            crowns.push_back(add(base, Vec3{0.0f, minimumDimension * height, 0.0f}));
            pushHeroObject(Object3DKind::Column,
                           Object3DRole::Shadow,
                           base,
                           Vec3{minimumDimension * (0.028f + intent.minimal * 0.012f),
                                minimumDimension * height,
                                minimumDimension * (0.040f + intent.shadow * 0.024f)},
                           Vec3{0.03f + intent.tension * 0.06f,
                                (unit - 0.5f) * 0.08f,
                                phase * 0.004f},
                           withAlpha(mix(colors[4], colors[3], 0.24f), 0.20f + intent.shadow * 0.24f),
                           0.15f + intent.shadow * 0.48f + metrics.bass * 0.10f,
                           0.72f + intent.shadow * 0.28f);
        }
        for (std::size_t i = 1; i < crowns.size(); ++i) {
            pushHeroLink(crowns[i - 1U],
                         crowns[i],
                         Object3DRole::Bridge,
                         withAlpha(colors[4], 0.055f + intent.shadow * 0.070f),
                         0.07f + intent.shadow * 0.16f);
        }
        pushHeroObject(Object3DKind::DepthPlane,
                       Object3DRole::Shadow,
                       Vec3{0.0f, minimumDimension * 0.11f, minimumDimension * 0.38f},
                       Vec3{minimumDimension * (0.38f + intent.minimal * 0.10f),
                            minimumDimension * (0.22f + intent.dark * 0.08f),
                            minimumDimension * 0.024f},
                       Vec3{0.64f, phase * 0.003f, phase * 0.004f},
                       withAlpha(colors[4], 0.08f + intent.shadow * 0.15f),
                       0.10f + intent.shadow * 0.24f,
                       0.62f + intent.shadow * 0.20f);
        pushHeroObject(Object3DKind::Anchor,
                       Object3DRole::Shadow,
                       Vec3{0.0f, minimumDimension * 0.02f, minimumDimension * (0.12f + intent.dark * 0.18f)},
                       Vec3{minimumDimension * (0.025f + intent.minimal * 0.012f),
                            minimumDimension * (0.025f + intent.minimal * 0.012f),
                            minimumDimension * 0.025f},
                       Vec3{0.0f, phase * 0.008f, phase * 0.012f},
                       withAlpha(colors[4], 0.22f + intent.shadow * 0.22f),
                       0.16f + intent.shadow * 0.38f,
                       0.74f + intent.shadow * 0.24f);
        break;
    }
    }
}
...
void applySongIdentityMaterialDirection3D(std::vector<Object3D>& objects,
                                          SongSceneIdentity identity,
                                          const AudioMetrics& metrics,
                                          const VisualSettings& settings,
                                          float minimumDimension,
                                          double time)
{
    ...
    const float phase = static_cast<float>(time);
    const float colorGain = std::clamp(0.42f + colorImpactOf(settings) * 0.46f + scenePersonalityOf(settings) * 0.12f,
                                       0.38f,
                                       0.92f);
    const float harmonic = clamp01(metrics.harmonicEnergy * 0.58f +
                                   metrics.keyConfidence * 0.26f +
                                   averageChromaEnergy(metrics) * 0.20f);
    const float transient = transientEnergy3D(metrics);
    const float beat = metrics.beat ? std::max(metrics.beatConfidence, 0.18f) : metrics.beatConfidence * 0.42f;
    const float analyzerRoleSum = metrics.bassRole +
                                  metrics.drumRole +
                                  metrics.melodyRole +
                                  metrics.harmonyRole +
    ...
    const float identityFocus = layeredRoleScene ? 0.58f : 1.0f;
    const float dimFloor = layeredRoleScene ? 0.74f : 0.18f;

    for (std::size_t i = 0; i < objects.size(); ++i) {
        Object3D& object = objects[i];
        const float unit = objects.size() > 1U ? static_cast<float>(i) / static_cast<float>(objects.size() - 1U) : 0.0f;
        const float sign = std::sin(static_cast<float>(i + 1U) * 2.137f) >= 0.0f ? 1.0f : -1.0f;
        const auto tint = [&](ColorRGBA target, float amount) {
            target.a = object.color.a;
            object.color = mix(object.color, target, clamp01(amount * colorGain * identityFocus));
            object.color.a = clamp01(object.color.a);
        };
        const auto dim = [&](float amount) {
            const float keep = std::clamp(std::max(amount, dimFloor), 0.18f, 1.0f);
            object.color.a *= keep;
            object.glow *= std::clamp(keep + 0.08f, 0.22f, 1.0f);
            object.scale = scale(object.scale, 0.84f + keep * 0.16f);
        };
        const bool materialSurface = object.kind == Object3DKind::DepthPlane ||
                                     object.kind == Object3DKind::WaveSurface ||
                                     object.kind == Object3DKind::Plate ||
        ...
        switch (identity) {
        case SongSceneIdentity::CalmSpace: {
            const bool spaceForm = object.musicRole == Object3DRole::Space ||
                                   object.musicRole == Object3DRole::Harmony ||
                                   object.kind == Object3DKind::DepthPlane ||
                                   object.kind == Object3DKind::Orbiter ||
                                   object.kind == Object3DKind::WaveSurface;
            if (spaceForm) {
                tint(i % 2U == 0U ? ColorRGBA{0.30f, 0.74f, 1.0f, object.color.a}
                                   : ColorRGBA{0.70f, 0.86f, 1.0f, object.color.a},
                     0.58f);
                object.color.a = clamp01(object.color.a * 1.10f + (materialSurface ? 0.040f : 0.018f));
                object.glow += 0.055f + metrics.stereoWidth * 0.075f;
                if (object.kind == Object3DKind::DepthPlane || object.kind == Object3DKind::WaveSurface) {
                    object.scale.x *= 1.18f;
                    object.scale.z *= 0.82f;
                    object.position.z += minimumDimension * (0.050f + unit * 0.035f);
                }
            } else {
                dim(0.50f);
                object.position.z += minimumDimension * 0.16f;
            }
            break;
        }
        case SongSceneIdentity::BassPressure: {
            const bool bassForm = object.musicRole == Object3DRole::Bass ||
                                  object.musicRole == Object3DRole::Convergence ||
                                  object.kind == Object3DKind::TunnelRib;
            if (bassForm) {
                tint(i % 3U == 0U ? ColorRGBA{1.0f, 0.20f, 0.08f, object.color.a}
                                   : ColorRGBA{1.0f, 0.08f, 0.44f, object.color.a},
                     0.62f);
                object.scale.y *= 1.04f + metrics.bass * 0.10f;
                object.scale.z *= 1.06f + metrics.dropIntensity * 0.14f;
                object.position.z -= minimumDimension * metrics.bass * 0.040f;
                object.color.a = clamp01(object.color.a * 1.08f + 0.035f);
                object.glow += metrics.bass * 0.16f + metrics.dropIntensity * 0.12f;
            } else {
                dim(0.62f);
            }
            break;
        ...
        case SongSceneIdentity::TechnoArchitecture: {
            const bool drumForm = object.musicRole == Object3DRole::Drums ||
                                  object.kind == Object3DKind::Column ||
                                  object.kind == Object3DKind::Cage ||
                                  object.kind == Object3DKind::Link;
            if (drumForm) {
                tint(i % 2U == 0U ? ColorRGBA{0.05f, 0.92f, 1.0f, object.color.a}
                                   : ColorRGBA{1.0f, 0.10f, 0.68f, object.color.a},
                     0.60f);
                object.rotation.z = std::round(object.rotation.z * 10.0f) / 10.0f;
                object.scale.y *= 1.08f + beat * 0.09f;
                object.color.a = clamp01(object.color.a * 1.06f + 0.020f);
                object.glow += beat * 0.12f;
            } else {
                dim(0.58f);
            }
            break;
        ...
        case SongSceneIdentity::AmbientOrbit: {
            const bool orbitForm = object.musicRole == Object3DRole::Space ||
                                   object.musicRole == Object3DRole::Harmony ||
                                   object.kind == Object3DKind::Orbiter ||
                                   object.kind == Object3DKind::WaveSurface ||
                                   object.kind == Object3DKind::DepthPlane;
            if (orbitForm) {
                tint(i % 3U == 0U ? ColorRGBA{0.12f, 0.48f, 1.0f, object.color.a}
                                   : ColorRGBA{0.55f, 0.95f, 1.0f, object.color.a},
                     0.64f);
                object.position.x *= 1.08f + metrics.stereoWidth * 0.08f;
                object.position.z += minimumDimension * (0.055f + metrics.stereoWidth * 0.080f);
                object.color.a = clamp01(object.color.a * 1.10f + 0.025f);
                object.glow += metrics.stereoWidth * 0.11f + 0.035f;
            } else {
                dim(0.45f);
                object.position.z += minimumDimension * 0.20f;
            }
            break;
        }
        case SongSceneIdentity::MelodicCrystal: {
            const bool crystalForm = object.musicRole == Object3DRole::Melody ||
                                     object.kind == Object3DKind::Shard ||
                                     object.kind == Object3DKind::Node ||
                                     object.kind == Object3DKind::Cage ||
                                     object.kind == Object3DKind::Ribbon ||
                                     (object.kind == Object3DKind::Link &&
                                      (object.musicRole == Object3DRole::Harmony ||
                                       object.musicRole == Object3DRole::Melody));
            if (crystalForm) {
                const ColorRGBA crystal = i % 3U == 0U ? ColorRGBA{1.0f, 0.18f, 0.92f, object.color.a}
                                        : (i % 3U == 1U ? ColorRGBA{0.12f, 0.96f, 1.0f, object.color.a}
                                                        : ColorRGBA{1.0f, 0.92f, 0.20f, object.color.a});
                tint(crystal, 0.76f);
                object.position.x = object.position.x * 0.28f +
                                    std::sin(unit * kPi * 2.0f + phase * 0.012f) * minimumDimension * 0.13f;
                object.position.y -= minimumDimension * (0.052f + harmonic * 0.050f);
                object.position.z = object.position.z * 0.78f +
                                    minimumDimension * (0.08f +
                                                        std::sin(unit * kPi + phase * 0.018f) * harmonic * 0.045f);
                object.scale = scale(object.scale, 1.34f + harmonic * 0.22f);
                if (object.kind == Object3DKind::Shard || object.kind == Object3DKind::Cage) {
                    object.scale.y *= 1.46f;
                    object.scale.z *= 0.82f;
                }
                object.color.a = clamp01(object.color.a * 1.20f + 0.060f);
                object.glow += 0.12f + harmonic * 0.22f;
            } else {
                dim(0.14f);
                object.position.z += minimumDimension * 0.24f;
            }
            break;
        }
        case SongSceneIdentity::BreakbeatFracture: {
            const bool fractureForm = object.musicRole == Object3DRole::Fracture ||
                                      object.kind == Object3DKind::Plate ||
                                      object.kind == Object3DKind::Shard;
            if (fractureForm) {
                const ColorRGBA cutColor = i % 3U == 0U ? ColorRGBA{1.0f, 0.38f, 0.12f, object.color.a}
                                          : (i % 3U == 1U ? ColorRGBA{0.92f, 1.0f, 0.14f, object.color.a}
                                                          : ColorRGBA{0.08f, 0.90f, 1.0f, object.color.a});
                tint(cutColor, 0.76f);
                object.position.x = object.position.x * 0.64f + sign * minimumDimension * (0.120f + transient * 0.080f);
                object.position.z = object.position.z * 0.78f - minimumDimension * (0.050f + transient * 0.055f);
                object.rotation.z = std::round((object.rotation.z + metrics.beatPhase * kPi) / (kPi / 6.0f)) * (kPi / 6.0f);
                object.scale.x *= 1.34f + transient * 0.22f;
                object.scale.y *= 1.18f + transient * 0.26f;
                object.color.a = clamp01(object.color.a * 1.14f + 0.040f);
                object.glow += 0.08f + transient * 0.20f;
            } else {
                dim(0.20f);
                object.position.z += minimumDimension * 0.18f;
            }
            break;
        }
        case SongSceneIdentity::DarkMonolith: {
            const bool shadowForm = object.musicRole == Object3DRole::Shadow ||
                                    object.musicRole == Object3DRole::Bass ||
                                    object.kind == Object3DKind::Column ||
                                    object.kind == Object3DKind::Anchor ||
                                    object.kind == Object3DKind::DepthPlane;
            if (shadowForm) {
                tint(i % 2U == 0U ? ColorRGBA{0.92f, 0.86f, 0.78f, object.color.a}
                                   : ColorRGBA{0.78f, 0.70f, 0.86f, object.color.a},
                     0.58f);
                object.scale.y *= 1.08f + metrics.bass * 0.08f;
                object.scale.x *= 0.94f;
                object.position.y += minimumDimension * 0.022f;
                object.position.z += minimumDimension * 0.035f;
                object.color.a = clamp01(object.color.a * 1.10f + 0.028f);
                object.glow += 0.045f + metrics.bass * 0.08f;
            } else {
                dim(0.30f);
                object.position.z += minimumDimension * 0.26f;
            }
            break;
        }
        }
    ...
    const auto pushFinalObject = [&](Object3DKind kind,
                                     Object3DRole role,
                                     Vec3 position,
                                     Vec3 scaleValue,
                                     Vec3 rotation,
                                     ColorRGBA color,
                                     float glow) -> Vec3 {
        Object3D object = makeObject3D(kind, position, scaleValue, rotation, color, glow);
        object.musicRole = role;
        objects.push_back(object);
        return position;
    };
    const auto pushFinalLink = [&](Vec3 from, Vec3 to, Object3DRole role, ColorRGBA color, float glow) {
        Object3D link = makeObject3D(Object3DKind::Link,
                                     from,
                                     Vec3{1.0f, 1.0f, 1.0f},
                                     Vec3{},
    ...
    if (identity == SongSceneIdentity::MelodicCrystal) {
        std::array<Vec3, 5> crystalAnchors{};
        for (int i = 0; i < 5; ++i) {
            const float unit = static_cast<float>(i) / 4.0f;
            const float lane = unit * 2.0f - 1.0f;
            const float chroma = chromaAt(metrics, i);
            const ColorRGBA crystal = i % 3 == 0 ? ColorRGBA{1.0f, 0.18f, 0.92f, 0.64f}
                                    : (i % 3 == 1 ? ColorRGBA{0.10f, 0.96f, 1.0f, 0.62f}
                                                   : ColorRGBA{1.0f, 0.92f, 0.16f, 0.60f});
            crystalAnchors[static_cast<std::size_t>(i)] =
                pushFinalObject(i % 2 == 0 ? Object3DKind::Shard : Object3DKind::Cage,
                                i % 3 == 0 ? Object3DRole::Harmony : Object3DRole::Melody,
                                Vec3{lane * minimumDimension * (0.13f + harmonic * 0.035f),
                                     -minimumDimension * (0.16f + harmonic * 0.055f) +
                                         std::sin(unit * kPi + phase * 0.018f) * minimumDimension * 0.028f,
                                     minimumDimension * (0.06f + chroma * 0.20f + unit * 0.12f)},
                                Vec3{minimumDimension * (0.034f + chroma * 0.020f),
                                     minimumDimension * (0.090f + harmonic * 0.050f),
                                     minimumDimension * (0.018f + chroma * 0.014f)},
                                Vec3{unit * 0.52f,
                                     phase * 0.026f + chroma * kPi,
                                     metrics.phrasePhase * kPi + lane * 0.28f},
                                crystal,
                                0.42f + harmonic * 0.48f + chroma * 0.16f);
        }
        for (std::size_t i = 1; i < crystalAnchors.size(); ++i) {
            pushFinalLink(crystalAnchors[i - 1U],
                          crystalAnchors[i],
                          Object3DRole::Harmony,
                          ColorRGBA{0.52f, 0.98f, 1.0f, 0.20f + harmonic * 0.16f},
                          0.16f + harmonic * 0.26f);
        }
    } else if (identity == SongSceneIdentity::BreakbeatFracture) {
        const float quant = std::floor(clamp01(metrics.beatPhase) * 8.0f) / 8.0f;
        for (int i = 0; i < 6; ++i) {
            const float unit = static_cast<float>(i) / 5.0f;
            const float side = i % 2 == 0 ? -1.0f : 1.0f;
            const ColorRGBA cutColor = i % 3 == 0 ? ColorRGBA{1.0f, 0.38f, 0.12f, 0.58f}
                                      : (i % 3 == 1 ? ColorRGBA{0.92f, 1.0f, 0.12f, 0.56f}
                                                     : ColorRGBA{0.08f, 0.90f, 1.0f, 0.54f});
            pushFinalObject(i % 2 == 0 ? Object3DKind::Plate : Object3DKind::Shard,
                            Object3DRole::Fracture,
                            Vec3{side * minimumDimension * (0.12f + unit * 0.16f),
                                 (unit - 0.5f) * minimumDimension * 0.16f,
                                 minimumDimension * (-0.10f + quant * 0.18f + unit * 0.10f)},
                            Vec3{minimumDimension * (0.038f + transient * 0.030f),
                                 minimumDimension * (0.14f + metrics.onset * 0.080f),
                                 minimumDimension * (0.016f + transient * 0.018f)},
                            Vec3{0.42f + unit * 0.24f,
                                 quant * kPi + side * 0.28f,
                                 std::floor((unit + quant) * 8.0f) * kPi * 0.16f},
                            cutColor,
                            0.32f + transient * 0.54f + metrics.onset * 0.16f);
        }
    }
}
...
SectionTransformStats3D applySectionNarrativeComposition3D(std::vector<Object3D>& objects,
                                                           const SectionNarrative3D& narrative,
                                                           const AudioMetrics& metrics,
                                                           const VisualSettings& settings,
                                                           float minimumDimension,
                                                           double time)
{
    SectionTransformStats3D stats;
    if (objects.empty() || narrative.intensity <= 0.025f) {
        return stats;
    }
    ...
    const float clarity = patternClarityOf(settings);
    const float stability = motionStabilityOf(settings);
    const float active = clamp01(narrative.intensity);
    const float build = narrative.build * (0.78f + clarity * 0.22f);
    const float drop = narrative.drop * (0.74f + response3DOf(settings) * 0.26f);
    const float groove = narrative.groove * (0.70f + stability * 0.30f);
    const float breakdown = narrative.breakdown * (0.74f + clarity * 0.18f + metrics.stereoWidth * 0.08f);
    const float release = narrative.release * (0.78f + stability * 0.12f + clarity * 0.10f);
    const float sectionProgress = clamp01(metrics.sectionProgress);
    ...
    float depthMotion = 0.0f;
    float materialShift = 0.0f;
    float transformMass = 0.0f;
    const float invDimension = 1.0f / std::max(1.0f, minimumDimension);

    const auto averageScale = [](Vec3 value) {
        return (std::fabs(value.x) + std::fabs(value.y) + std::fabs(value.z)) / 3.0f;
    };

    const auto transformPoint = [&](Vec3 point, float seed, float depthUnit) {
        const float lane = std::sin(seed * 2.1f) >= 0.0f ? 1.0f : -1.0f;
        const float liftCurve = std::pow(depthUnit, 0.82f);

        point.y -= minimumDimension * build * (0.018f + liftCurve * 0.062f + sectionProgress * 0.028f);
        point.z += minimumDimension * build * (0.060f + depthUnit * 0.120f + metrics.buildTension * 0.035f);
        point.x *= 1.0f + build * (0.050f + metrics.stereoWidth * 0.040f);

        point.x *= 1.0f - drop * (0.090f + metrics.bass * 0.060f);
        point.y += minimumDimension * drop * (0.020f + metrics.bass * 0.030f);
        point.z -= minimumDimension * drop * (0.145f + metrics.bass * 0.105f + (1.0f - depthUnit) * 0.050f);

        if (groove > 0.015f) {
            const Vec3 snapped{
                std::round(point.x / cell) * cell,
                point.y,
                std::round(point.z / (cell * 1.42f)) * (cell * 1.42f)
            };
            point = mix(point, snapped, clamp01(groove * (0.30f + metrics.barConfidence * 0.32f)));
            point.y += std::sin(beatStep * 0.44f + seed * 0.6f) * minimumDimension * groove * 0.018f;
            point.z += std::sin(beatStep * 0.82f + seed) * minimumDimension * groove * 0.045f;
        }

        point.x *= 1.0f + breakdown * (0.145f + metrics.stereoWidth * 0.160f);
        point.y += std::sin(phase * 0.022f + seed) * minimumDimension * breakdown * 0.034f;
        point.z += minimumDimension * breakdown * (0.135f + depthUnit * 0.250f + metrics.stereoWidth * 0.055f);

        point.x *= 1.0f + release * (0.095f + metrics.phraseConfidence * 0.055f);
        point.y -= minimumDimension * release * (0.024f + depthUnit * 0.050f);
        point.z += minimumDimension * release * (0.080f + depthUnit * 0.145f);
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        Object3D& object = objects[i];
        const Vec3 originalPosition = object.position;
        const Vec3 originalTarget = object.target;
        const Vec3 originalScale = object.scale;
        const float originalGlow = object.glow;
        const float originalAlpha = object.color.a;
        const float seed = static_cast<float>(i + 1U) * 0.431f +
                           static_cast<float>(static_cast<int>(object.kind)) * 0.257f;
        const float depthUnit = wrapUnit(std::sin(seed * 1.73f) * 0.5f + 0.5f);
        const bool explicitRole = isExplicitMusicalPartRole(object.musicRole);
        const bool connector = object.kind == Object3DKind::Link ||
                               object.musicRole == Object3DRole::Bridge ||
                               object.musicRole == Object3DRole::Convergence;
        ...
        object.rotation.x += build * (0.035f + depthUnit * 0.030f) -
                             drop * (0.040f + metrics.bass * 0.020f) +
                             release * 0.026f;
        object.rotation.y += std::sin(seed + phase * 0.015f) * (breakdown * 0.060f + release * 0.040f);
        object.rotation.z += std::sin(seed * 0.8f) * build * 0.038f +
                             std::round(metrics.beatPhase * 8.0f) * groove * 0.018f +
                             drop * 0.052f;
        ...
        object.scale.x *= 1.0f + breakdown * 0.060f + release * 0.070f - drop * 0.030f;
        object.scale.y *= 1.0f + build * 0.050f + drop * (0.105f + metrics.bass * 0.050f);
        object.scale.z *= 1.0f + build * 0.042f + drop * 0.120f + release * 0.050f;
        if (object.kind == Object3DKind::DepthPlane || object.kind == Object3DKind::WaveSurface) {
            object.scale.x *= 1.0f + breakdown * 0.12f + release * 0.10f;
        ...
        const float glowLift = (build * 0.082f +
                                ...
                                release * 0.078f) *
                               rolePresence;
        object.glow += glowLift;
        object.color.a = clamp01(object.color.a *
                                 (1.0f +
                                  explicitRole * active * 0.082f -
                                  connector * active * (0.050f - drop * 0.024f) +
                                  build * 0.026f +
                                  drop * 0.052f +
                                  release * 0.032f));

        depthMotion += std::fabs(object.position.z - originalPosition.z) * invDimension;
        materialShift += std::fabs(object.glow - originalGlow) * 0.55f +
    ...
    const float invCount = 1.0f / static_cast<float>(objects.size());
    stats.depthMotion = clamp01(depthMotion * invCount * 2.0f);
    stats.materialShift = clamp01(materialShift * invCount * 2.20f);
    stats.transform = clamp01(active * 0.47f +
                              transformMass * invCount * 1.22f +
                              stats.depthMotion * 0.26f +
                              stats.materialShift * 0.18f);
    return stats;
}

void applyEmotionalSectionRoleChoreography3D(std::vector<Object3D>& objects,
                                             const SectionNarrative3D& narrative,
                                             const MusicRoleScene3D& role,
                                             const AudioMetrics& metrics,
                                             const VisualSettings& settings,
                                             ...
                                             double time)
{
    if (objects.empty() || narrative.intensity <= 0.035f) {
        return;
    }
    if (settings.interactiveField && settings.interactionDepth > 0.82f) {
        return;
    ...
    const float response = response3DOf(settings);
    const float build = narrative.build * (0.78f + clarity * 0.22f);
    const float drop = narrative.drop * (0.74f + response * 0.26f);
    const float groove = narrative.groove * (0.72f + stability * 0.28f);
    const float breakdown = narrative.breakdown * (0.74f + clarity * 0.18f + metrics.stereoWidth * 0.08f);
    const float release = narrative.release * (0.78f + stability * 0.12f + clarity * 0.10f);
    const float harmonic = clamp01(metrics.harmonicEnergy * 0.60f +
                                   metrics.keyConfidence * 0.28f +
    ...
    const float beat = metrics.beat ? clamp01(0.08f + metrics.beatConfidence * 0.92f)
                                    : metrics.beatConfidence * 0.42f;
    const float sectionProgress = clamp01(metrics.sectionProgress);
    const float anticipationCurve = smootherStep(0.16f, 0.92f, sectionProgress);
    const float recoveryCurve = smootherStep(0.42f, 0.94f, sectionProgress);
    const float activeRoles = activeRoleCount3D(role);
    const float authority = clamp01(role.separation * 0.50f +
                                    std::min(activeRoles / 7.0f, 1.0f) * 0.18f +
                                    clarity * 0.18f +
                                    stability * 0.10f);
    const float baseBlend = std::clamp(0.055f + authority * 0.12f + narrative.intensity * 0.08f,
                                       0.050f,
                                       0.26f);
    ...
    for (std::size_t i = 0; i < objects.size(); ++i) {
        ...
        if (!explicitRole) {
            if (bridgeRole || convergenceRole) {
                const float event = clamp01(drop * 0.64f + release * 0.28f + role.convergence * 0.38f);
                object.color.a *= std::clamp(0.22f + event * 0.70f, 0.16f, convergenceRole ? 0.96f : 0.72f);
                object.glow *= std::clamp(0.28f + event * 0.72f, 0.18f, 0.94f);
                object.scale = scale(object.scale, std::clamp(0.72f + event * 0.34f, 0.64f, 1.10f));
            }
            continue;
        }
        ...
        const float seed = static_cast<float>(i + 1U) * 0.493f +
                           static_cast<float>(ordinal + 1) * 0.281f +
                           static_cast<float>(static_cast<int>(object.kind)) * 0.137f;
        const float side = std::sin(seed * 2.7f) >= 0.0f ? 1.0f : -1.0f;
        const float lane = static_cast<float>((ordinal % 7) - 3) / 3.0f;
        const float localBlend = baseBlend * std::clamp(0.70f + strength * 0.36f, 0.62f, 1.08f);
        const Vec3 before = object.position;
        Vec3 target = object.position;
        Vec3 scaleBias{1.0f, 1.0f, 1.0f};
        Vec3 rotationBias{};
        ...
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            target.y += minimumDimension * (build * 0.018f + drop * (0.060f + metrics.bass * 0.052f) - release * 0.020f);
            target.z -= minimumDimension * (build * (0.030f + anticipationCurve * 0.034f) +
                                            drop * (0.220f + metrics.bass * 0.180f) -
                                            release * 0.070f);
            target.x *= 1.0f - drop * (0.028f + metrics.bass * 0.034f);
            scaleBias = Vec3{1.0f - drop * 0.050f,
                             1.0f + build * 0.050f + drop * (0.150f + metrics.bass * 0.070f),
                             1.0f + drop * (0.180f + metrics.bass * 0.120f)};
            glowLift = drop * (0.10f + metrics.bass * 0.12f);
            break;
        case MusicalRoleDistrict3D::Drums: {
            const float step = std::round((metrics.beatPhase + lane * 0.12f) * 8.0f);
            target.x += std::sin(step * 0.70f + lane) * minimumDimension * groove * strength * 0.032f;
            target.y += minimumDimension * beat * (groove * 0.024f + drop * 0.032f);
            target.z += minimumDimension * (groove * 0.050f + drop * 0.090f) *
                        ((ordinal + static_cast<int>(step)) % 2 == 0 ? 1.0f : -0.45f);
            scaleBias = Vec3{0.96f, 1.0f + beat * (groove * 0.10f + drop * 0.12f), 0.98f};
            rotationBias.z += std::round(metrics.beatPhase * 8.0f) * 0.012f;
            glowLift = beat * (groove * 0.060f + drop * 0.080f);
            break;
        }
        case MusicalRoleDistrict3D::Melody:
            target.y -= minimumDimension * (build * (0.070f + harmonic * 0.030f + anticipationCurve * 0.040f) +
                                            release * (0.060f + recoveryCurve * 0.050f));
            target.x += std::cos(phase * 0.020f + seed) * minimumDimension *
                        (build * 0.035f + release * 0.060f + harmonic * 0.018f);
            target.z += minimumDimension * (build * (0.080f + harmonic * 0.055f) +
                                            release * (0.110f + harmonic * 0.080f) +
                                            drop * 0.020f);
            scaleBias = Vec3{0.98f, 0.96f, 1.0f + harmonic * (build * 0.12f + release * 0.16f)};
            rotationBias.y += std::sin(seed + phase * 0.016f) * (build + release) * 0.08f;
            glowLift = harmonic * (build * 0.070f + release * 0.090f);
            break;
        case MusicalRoleDistrict3D::Harmony:
            target.x *= 1.0f + build * 0.040f + breakdown * (0.130f + metrics.stereoWidth * 0.080f) + release * 0.090f;
            target.y -= minimumDimension * (build * 0.026f + release * 0.042f);
            target.z += minimumDimension * (build * 0.050f + breakdown * 0.140f + release * (0.145f + harmonic * 0.060f));
            scaleBias = Vec3{1.0f + harmonic * (breakdown * 0.12f + release * 0.13f),
                             0.98f,
                             1.0f + metrics.phraseConfidence * (build * 0.08f + release * 0.12f)};
            glowLift = harmonic * (build * 0.050f + breakdown * 0.050f + release * 0.080f);
            break;
        case MusicalRoleDistrict3D::Space:
            target.x *= 1.0f + breakdown * (0.210f + metrics.stereoWidth * 0.180f) +
                              release * (0.095f + metrics.stereoWidth * 0.060f) -
                              drop * 0.035f;
            target.y += std::sin(phase * 0.014f + seed) * minimumDimension * (breakdown * 0.050f + release * 0.024f);
            target.z += minimumDimension * (build * 0.045f +
                                            breakdown * (0.720f + metrics.stereoWidth * 0.300f) +
                                            release * 0.160f -
                                            drop * 0.040f);
            scaleBias = Vec3{1.0f + metrics.stereoWidth * (breakdown * 0.16f + release * 0.08f),
                             0.98f,
                             1.0f + role.space * (breakdown * 0.14f + release * 0.10f)};
            alphaScale = 1.0f - drop * 0.10f + breakdown * 0.04f;
            glowLift = breakdown * 0.060f + release * 0.040f;
            break;
        case MusicalRoleDistrict3D::Fracture:
            target.x += side * minimumDimension * (drop * (0.075f + transient * 0.075f) +
                                                   groove * 0.020f +
                                                   breakdown * 0.038f);
            target.y += std::sin(seed * 3.0f + metrics.beatPhase * kPi) * minimumDimension *
                        (drop * 0.034f + transient * 0.026f);
            target.z += side * minimumDimension * (drop * (0.070f + transient * 0.090f) +
                                                   build * 0.020f);
            scaleBias = Vec3{1.0f + transient * drop * 0.14f, 0.94f + transient * 0.04f, 0.96f};
            rotationBias.y += side * transient * (0.10f + drop * 0.18f);
            rotationBias.z += side * drop * 0.10f;
            glowLift = transient * (drop * 0.12f + groove * 0.04f);
            break;
        case MusicalRoleDistrict3D::Shadow:
            target.y += minimumDimension * (breakdown * (0.045f + role.shadow * 0.040f) +
                                            drop * 0.022f);
            target.z += minimumDimension * (breakdown * (0.120f + role.shadow * 0.080f) -
                                            drop * 0.030f +
                                            release * 0.035f);
            target.x *= 1.0f - drop * 0.020f;
            scaleBias = Vec3{0.96f, 1.0f + breakdown * 0.12f + role.shadow * 0.08f, 0.98f};
            rotationBias.x -= object.rotation.x * breakdown * 0.16f;
            rotationBias.z -= object.rotation.z * breakdown * 0.20f;
            glowLift = breakdown * 0.040f + drop * 0.035f;
            break;
        }

        object.position = mix(object.position, target, localBlend);
        object.scale = multiply(object.scale, mix(Vec3{1.0f, 1.0f, 1.0f}, scaleBias, localBlend * 0.78f));
        object.rotation = add(object.rotation, scale(rotationBias, localBlend));
        object.glow += glowLift * localBlend * (0.80f + response * 0.20f);
        object.color.a = clamp01(object.color.a * std::clamp(alphaScale, 0.78f, 1.10f));
        const Vec3 delta = subtract(object.position, before);
        object.velocity = add(object.velocity, delta);
        if (object.kind == Object3DKind::Link) {
            object.target = add(object.target, scale(delta, 0.62f));
        }
    }
}
                         ...
                         double time)
{
    ...
    if (arc.impact > 0.05f) {
        const int shocks = scaledCount(7 + static_cast<int>(std::round(arc.impact * 8.0f)),
                                       density * (0.40f + arc.impact * 0.38f));
    ...
    if (arc.recovery > 0.05f) {
        const int ribbons = scaledCount(7 + static_cast<int>(std::round(arc.recovery * 8.0f)),
                                        density * (0.36f + arc.recovery * 0.34f));
        std::vector<Vec3> anchors;
                                       ...
                                       float minimumDimension)
{
    ...
    const float dim = 1.0f - separationYield * convergenceRelief * 0.70f;
    const float pushBack = minimumDimension * separationYield * convergenceRelief *
                           (0.22f + role.space * 0.18f + role.shadow * 0.09f);
    const float count = static_cast<float>(objects.size());
    for (std::size_t i = 0; i < objects.size(); ++i) {
        Object3D& object = objects[i];
        const float unit = count > 1.0f ? static_cast<float>(i) / (count - 1.0f) : 0.0f;
        ...
        object.color.a *= std::clamp(dim + (object.kind == Object3DKind::DepthPlane ? 0.035f : 0.0f), 0.26f, 0.78f);
        object.glow *= std::clamp(0.44f + role.convergence * 0.16f, 0.34f, 0.76f);
        if (object.kind != Object3DKind::Column && object.kind != Object3DKind::TunnelRib) {
            object.scale = scale(object.scale, 0.76f + role.convergence * 0.08f);
        }
    }
}

void addMusicalRoleOrchestraMotifs3D(std::vector<Object3D>& objects,
                                     const MusicRoleScene3D& role,
                                     const SceneInterpretation& intent,
                                     const AudioMetrics& metrics,
                                     const VisualSettings& settings,
                                     ...
                                     double time)
{
    ...
    const float beat = metrics.beat ? clamp01(0.08f + metrics.beatConfidence * 0.92f)
                                    : metrics.beatConfidence * 0.42f;
    const float downbeat = metrics.downbeat ? std::max(0.34f, metrics.downbeatConfidence)
                                            : metrics.downbeatConfidence * 0.30f;
    const float harmonic = clamp01(metrics.harmonicEnergy * 0.64f +
                                   metrics.keyConfidence * 0.26f +
                                   averageChromaEnergy(metrics) * 0.18f);
    const float transient = transientEnergy3D(metrics);
    const float clarity = patternClarityOf(settings);
    const float stability = motionStabilityOf(settings);
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.48f +
                                            metrics.dropIntensity * 0.28f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.24f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.14f : 0.0f));
    const float soloAuthority = clamp01(role.separation * 0.58f +
                                        std::min(activeMass / 3.8f, 1.0f) * 0.18f +
                                        std::min(activeCount / 7.0f, 1.0f) * 0.10f +
                                        clarity * 0.10f +
                                        personality * 0.12f);
    const float isolation = clamp01(soloAuthority *
                                    (0.74f + clarity * 0.16f + stability * 0.10f) *
                                    (1.0f - earnedConvergence * 0.52f - role.convergence * 0.30f));
    const float motifScale = 0.90f + personality * 0.12f + response * 0.045f;
    const float stereoSpread = 0.74f + metrics.stereoWidth * 0.58f;
    const Vec3 convergence = roleConvergencePoint3D(role, minimumDimension);
    std::array<std::vector<Vec3>, 7> anchors;

    const auto roleIndex = [](MusicalRoleDistrict3D district) {
        return static_cast<std::size_t>(district);
    };
    const auto roleVelocity = [&](MusicalRoleDistrict3D district, float strength, float seed) {
        const float voice = clamp01(strength * (0.56f + isolation * 0.44f));
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return Vec3{0.0f,
                        minimumDimension * voice * (0.008f + metrics.bass * 0.024f + downbeat * 0.012f),
                        -minimumDimension * voice * (0.026f + metrics.bass * 0.050f + metrics.dropIntensity * 0.038f)};
        case MusicalRoleDistrict3D::Drums:
            return Vec3{std::round(std::sin(seed + metrics.beatPhase * kPi) * 3.0f) *
                            minimumDimension * voice * beat * 0.014f,
                        minimumDimension * voice * beat * 0.012f,
                        minimumDimension * voice * beat * 0.034f};
        case MusicalRoleDistrict3D::Melody:
            return Vec3{std::cos(seed + metrics.phrasePhase * kPi) * minimumDimension * voice * harmonic * 0.026f,
                        -minimumDimension * voice * harmonic * 0.038f,
                        std::sin(seed * 0.72f + phase * 0.018f) * minimumDimension * voice * harmonic * 0.032f};
        case MusicalRoleDistrict3D::Harmony:
            return Vec3{std::sin(seed * 0.52f) * minimumDimension * voice * harmonic * 0.018f,
                        -minimumDimension * voice * harmonic * 0.014f,
                        minimumDimension * voice * (0.026f + metrics.phraseConfidence * 0.028f)};
        case MusicalRoleDistrict3D::Space:
            return Vec3{std::sin(seed + phase * 0.012f) * minimumDimension * voice * metrics.stereoWidth * 0.018f,
                        std::cos(seed * 0.45f + phase * 0.010f) * minimumDimension * voice * 0.014f,
                        minimumDimension * voice * (0.040f + metrics.stereoWidth * 0.052f)};
        case MusicalRoleDistrict3D::Fracture:
            return Vec3{std::sin(seed * 3.2f) * minimumDimension * voice * (0.044f + transient * 0.060f),
                        std::cos(seed * 2.1f) * minimumDimension * voice * transient * 0.020f,
                        std::cos(seed * 2.7f) * minimumDimension * voice * (0.036f + transient * 0.052f)};
        case MusicalRoleDistrict3D::Shadow:
            return Vec3{0.0f,
                        minimumDimension * voice * (0.014f + role.shadow * 0.040f),
                        minimumDimension * voice * (0.028f + intent.dark * 0.036f + role.shadow * 0.028f)};
        }
        return Vec3{};
    };
    const auto voiceTint = [&](MusicalRoleDistrict3D district, ColorRGBA source, float unit) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return mix(source, ColorRGBA{1.0f, 0.22f, 0.10f, source.a}, 0.34f + metrics.bass * 0.18f);
        case MusicalRoleDistrict3D::Drums:
            return mix(source, ColorRGBA{0.05f, 0.92f, 1.0f, source.a}, 0.34f + beat * 0.12f);
        case MusicalRoleDistrict3D::Melody:
            return mix(source, ColorRGBA{1.0f, 0.20f, 0.86f, source.a}, 0.34f + harmonic * 0.14f);
        case MusicalRoleDistrict3D::Harmony:
            return mix(source, ColorRGBA{0.34f, 0.76f, 1.0f, source.a}, 0.30f + harmonic * 0.12f);
        case MusicalRoleDistrict3D::Space:
            return mix(source, ColorRGBA{0.22f, 0.38f, 1.0f, source.a}, 0.30f + metrics.stereoWidth * 0.14f);
        case MusicalRoleDistrict3D::Fracture:
            return mix(source, ColorRGBA{1.0f, 1.0f, 0.08f, source.a}, 0.36f + transient * 0.16f);
        case MusicalRoleDistrict3D::Shadow:
            return mix(source, ColorRGBA{0.66f, 0.68f, 0.82f, source.a}, 0.34f + role.shadow * 0.12f);
        }
        (void)unit;
        return source;
    };
    const auto placeInDistrict = [&](MusicalRoleDistrict3D district, Vec3 localOffset, float merge) {
        const float strength = roleDistrictStrength(role, district);
        const Vec3 center = roleDistrictCenter3D(district, role, intent, metrics, settings, minimumDimension, time);
        const float readableMerge = clamp01(merge *
                                            (0.06f + earnedConvergence * 0.18f + role.convergence * 0.16f) *
                                            (1.0f - isolation * 0.70f));
        const Vec3 districtBias = scale(roleDistrictUnitOffset(district),
                                        minimumDimension * isolation * (0.050f + strength * 0.035f));
        return mix(add(add(center, localOffset), districtBias), convergence, readableMerge);
    };
    const auto pushMotif = [&](MusicalRoleDistrict3D district,
                               Object3DKind kind,
                               Vec3 localOffset,
                               Vec3 scaleValue,
                               Vec3 rotation,
                               ...
                               float merge = 0.0f) {
        Object3D object = makeObject3D(kind,
                                       placeInDistrict(district, localOffset, merge),
                                       scale(scaleValue, motifScale),
                                       rotation,
                                       voiceTint(district, color, unit),
                                       glow);
        object.musicRole = objectRoleForDistrict(district);
        object.velocity = roleVelocity(district, strength, unit * 9.37f + phase);
        const float voicePresence = clamp01(strength * 0.54f + isolation * 0.36f + soloAuthority * 0.10f);
        object.color.a = clamp01(object.color.a * (1.10f + voicePresence * 0.38f));
        object.glow += voicePresence * (0.08f + response * 0.055f);
        if (district == MusicalRoleDistrict3D::Space || district == MusicalRoleDistrict3D::Harmony) {
            object.glow *= 0.76f;
        } else if (district == MusicalRoleDistrict3D::Fracture || district == MusicalRoleDistrict3D::Drums) {
            object.glow *= 1.10f;
        }
        anchors[roleIndex(district)].push_back(object.position);
        objects.push_back(object);
    };
    const auto pushIntraRoleLink = [&](MusicalRoleDistrict3D district, Vec3 from, Vec3 to, ColorRGBA color, float glow) {
        Object3D link = makeObject3D(Object3DKind::Link,
                                     from,
                                     Vec3{1.0f, 1.0f, 1.0f},
                                     Vec3{},
                                     ...
                                     glow);
        link.target = to;
        link.musicRole = objectRoleForDistrict(district);
        link.velocity = roleVelocity(district, 0.42f + roleDistrictStrength(role, district) * 0.24f, from.x * 0.006f + to.z * 0.004f);
        objects.push_back(link);
    };

    const auto active = [](float strength) {
        return strength > 0.10f;
    };

    if (active(role.bass)) {
        const int ribs = scaledCount(4 + static_cast<int>(std::round(role.bass * 4.0f)),
                                     density * (0.34f + role.bass * 0.22f + isolation * 0.18f));
        for (int i = 0; i < ribs; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, ribs - 1));
            const float compression = std::pow(1.0f - unit, 1.6f) * (metrics.bass * 0.16f + metrics.dropIntensity * 0.22f);
            const float radius = minimumDimension * (0.16f + unit * 0.13f + role.bass * 0.08f - compression * 0.035f);
            pushMotif(MusicalRoleDistrict3D::Bass,
                      Object3DKind::TunnelRib,
                      Vec3{0.0f,
                           minimumDimension * (0.020f + compression * 0.18f),
                           minimumDimension * (-0.28f + unit * 0.72f - compression)},
                      Vec3{radius,
                           radius * (0.42f + metrics.lowMid * 0.18f),
                           0.70f + role.bass * 0.74f + metrics.dropIntensity * 0.22f},
                      Vec3{compression * 0.18f, phase * 0.012f, metrics.beatPhase * kPi * 0.20f + unit},
                      withAlpha(colors[1], 0.24f + role.bass * 0.24f),
                      0.30f + role.bass * 0.68f + metrics.dropIntensity * 0.34f,
                      role.bass,
                      unit,
                      role.bass * 0.10f);
        }
        const int pistons = scaledCount(3, density * (0.36f + role.bass * 0.24f));
        for (int i = 0; i < pistons; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, pistons - 1));
            const float lane = unit - 0.5f;
            pushMotif(MusicalRoleDistrict3D::Bass,
                      Object3DKind::Column,
                      Vec3{lane * minimumDimension * 0.34f,
                           minimumDimension * (0.10f + metrics.bass * 0.07f),
                           minimumDimension * (-0.20f + unit * 0.26f - metrics.dropIntensity * 0.08f)},
                      Vec3{minimumDimension * (0.026f + role.bass * 0.018f),
                           minimumDimension * (0.22f + role.bass * 0.24f + metrics.dropIntensity * 0.08f),
                           minimumDimension * (0.032f + role.bass * 0.018f)},
                      Vec3{0.04f, lane * (0.14f + role.bass * 0.06f), phase * 0.012f},
                      withAlpha(mix(colors[1], colors[3], 0.24f), 0.22f + role.bass * 0.24f),
                      0.24f + role.bass * response * 0.56f,
                      role.bass,
                      unit);
        }
    }

    if (active(role.drums)) {
        const int lanes = scaledCount(3, density * (0.40f + role.drums * 0.18f));
        const int steps = scaledCount(4 + static_cast<int>(std::round(role.drums * 4.0f)),
                                      density * (0.38f + role.drums * 0.22f));
        const int activeStep = std::clamp(static_cast<int>(std::floor(clamp01(metrics.barPhase) * static_cast<float>(steps))),
                                          0,
                                          std::max(0, steps - 1));
        for (int lane = 0; lane < lanes; ++lane) {
            Vec3 previous{};
            ...
            const float laneUnit = lanes > 1 ? static_cast<float>(lane) / static_cast<float>(lanes - 1) : 0.0f;
            for (int step = 0; step < steps; ++step) {
                const float stepUnit = steps > 1 ? static_cast<float>(step) / static_cast<float>(steps - 1) : 0.0f;
                const float pulse = step == activeStep ? std::max(beat, 0.34f) : beat * 0.22f;
                const Vec3 offset{
                    (laneUnit - 0.5f) * minimumDimension * 0.46f * stereoSpread,
                    (laneUnit - 0.5f) * minimumDimension * 0.065f,
                    minimumDimension * (-0.12f + stepUnit * 0.58f + pulse * 0.06f)
                };
                pushMotif(MusicalRoleDistrict3D::Drums,
                          (step + lane) % 3 == 0 ? Object3DKind::Cage : Object3DKind::Column,
                          offset,
                          Vec3{minimumDimension * (0.012f + pulse * 0.010f),
                               minimumDimension * (0.060f + role.drums * 0.070f + pulse * 0.070f),
                               minimumDimension * 0.012f},
                          Vec3{0.0f, laneUnit * 0.18f, std::round(metrics.beatPhase * 8.0f) * (kPi / 8.0f)},
                          withAlpha(colors[(lane + step) % 5], 0.17f + role.drums * 0.18f + pulse * 0.20f),
                          0.16f + role.drums * 0.42f + pulse * 0.34f,
                          role.drums,
                          stepUnit);
                const Vec3 current = anchors[roleIndex(MusicalRoleDistrict3D::Drums)].back();
                if (havePrevious && (step % 2 == 0 || step == activeStep)) {
                    pushIntraRoleLink(MusicalRoleDistrict3D::Drums,
                                      previous,
                                      current,
                                      withAlpha(colors[lane % 5], 0.050f + beat * 0.060f),
                                      0.055f + beat * 0.12f);
                }
                previous = current;
                havePrevious = true;
    ...
    if (active(role.melody)) {
        const int notes = scaledCount(7 + static_cast<int>(std::round(role.melody * 7.0f)),
                                      density * (0.42f + role.melody * 0.28f + harmonic * 0.10f));
        Vec3 previous{};
        bool havePrevious = false;
        for (int i = 0; i < notes; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, notes - 1));
            const float chroma = chromaAt(metrics, static_cast<std::size_t>(i + std::max(0, metrics.keyIndex)));
            const float angle = unit * 2.55f * kPi + metrics.phrasePhase * kPi + harmonic * 0.34f;
            const Vec3 offset{
                std::cos(angle) * minimumDimension * (0.12f + chroma * 0.14f) * stereoSpread,
                -minimumDimension * (0.12f + unit * 0.12f + chroma * 0.05f),
                minimumDimension * (0.04f + unit * 0.55f + chroma * 0.22f)
            };
            pushMotif(MusicalRoleDistrict3D::Melody,
                      i % 3 == 0 ? Object3DKind::Ribbon : (i % 2 == 0 ? Object3DKind::Shard : Object3DKind::Node),
                      offset,
                      Vec3{minimumDimension * (0.020f + chroma * 0.036f),
                           minimumDimension * (0.034f + role.melody * 0.046f + harmonic * 0.020f),
                           minimumDimension * (0.026f + chroma * 0.022f)},
                      Vec3{angle * 0.16f, phase * 0.034f + chroma * kPi, harmonic * kPi},
                      withAlpha(colors[(i + 2) % 5], 0.24f + role.melody * 0.24f + chroma * 0.16f),
                      0.20f + role.melody * 0.52f + harmonic * 0.34f,
                      role.melody,
                      unit,
                      role.melody * 0.08f);
            const Vec3 current = anchors[roleIndex(MusicalRoleDistrict3D::Melody)].back();
            if (havePrevious && i % 2 == 0) {
                pushIntraRoleLink(MusicalRoleDistrict3D::Melody,
                                  previous,
                                  current,
                                  withAlpha(colors[(i + 2) % 5], 0.060f + harmonic * 0.070f),
                                  0.070f + harmonic * 0.16f);
            }
            previous = current;
            havePrevious = true;
        }
    ...
    if (active(role.harmony)) {
        const int chambers = scaledCount(4 + static_cast<int>(std::round(role.harmony * 4.0f)),
                                         density * (0.36f + role.harmony * 0.22f + harmonic * 0.08f));
        for (int i = 0; i < chambers; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, chambers - 1));
            const float side = i % 2 == 0 ? -1.0f : 1.0f;
            pushMotif(MusicalRoleDistrict3D::Harmony,
                      i % 3 == 0 ? Object3DKind::DepthPlane : Object3DKind::Cage,
                      Vec3{side * minimumDimension * (0.08f + unit * 0.18f) * stereoSpread,
                           -minimumDimension * (0.030f + harmonic * 0.040f),
                           minimumDimension * (0.10f + unit * 0.50f + metrics.phraseConfidence * 0.08f)},
                      Vec3{minimumDimension * (0.080f + role.harmony * 0.060f + unit * 0.026f),
                           minimumDimension * (0.052f + harmonic * 0.040f),
                           minimumDimension * (0.090f + harmonic * 0.050f)},
                      Vec3{0.32f + unit * 0.10f, phase * 0.012f + side * 0.08f, harmonic * 0.46f},
                      withAlpha(colors[3], 0.16f + role.harmony * 0.22f + harmonic * 0.10f),
                      0.15f + role.harmony * 0.42f + harmonic * 0.24f,
                      role.harmony,
                      unit);
        }
    }

    if (active(role.space)) {
        const int shells = scaledCount(4 + static_cast<int>(std::round(role.space * 4.0f)),
                                       density * (0.38f + role.space * 0.24f + metrics.stereoWidth * 0.10f));
        for (int i = 0; i < shells; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, shells - 1));
            const float angle = unit * 2.0f * kPi + phase * (0.010f + metrics.stereoWidth * 0.012f);
            pushMotif(MusicalRoleDistrict3D::Space,
                      i % 3 == 0 ? Object3DKind::WaveSurface : (i % 2 == 0 ? Object3DKind::DepthPlane : Object3DKind::Orbiter),
                      Vec3{std::cos(angle) * minimumDimension * (0.10f + unit * 0.18f + metrics.stereoWidth * 0.10f) * stereoSpread,
                           std::sin(angle * 0.66f) * minimumDimension * (0.05f + role.space * 0.05f),
                           minimumDimension * (0.24f + unit * 0.86f + metrics.stereoWidth * 0.18f)},
                      Vec3{minimumDimension * (0.14f + unit * 0.090f + metrics.stereoWidth * 0.070f),
                           minimumDimension * (0.080f + role.space * 0.070f),
                           minimumDimension * (0.016f + unit * 0.014f)},
                      Vec3{0.46f + unit * 0.16f, angle * 0.12f, phase * 0.008f + unit},
                      withAlpha(colors[(i + 4) % 5], 0.12f + role.space * 0.20f + metrics.stereoWidth * 0.08f),
                      0.08f + role.space * 0.20f + metrics.stereoWidth * 0.10f,
                      role.space,
                      unit);
        }
    }

    if (active(role.fracture)) {
        const int cuts = scaledCount(5 + static_cast<int>(std::round(role.fracture * 6.0f)),
                                     density * (0.40f + role.fracture * 0.30f + transient * 0.10f));
        const float cutStep = std::floor(clamp01(metrics.beatPhase) * 8.0f);
        for (int i = 0; i < cuts; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, cuts - 1));
            const float stagger = static_cast<float>((i * 5 + 2) % 11) / 10.0f;
            const float side = (i + static_cast<int>(cutStep)) % 2 == 0 ? -1.0f : 1.0f;
            pushMotif(MusicalRoleDistrict3D::Fracture,
                      i % 2 == 0 ? Object3DKind::Plate : Object3DKind::Shard,
                      Vec3{side * minimumDimension * (0.10f + unit * 0.34f) * stereoSpread,
                           (stagger - 0.5f) * minimumDimension * (0.22f + transient * 0.08f),
                           minimumDimension * (-0.14f + stagger * 0.64f + side * transient * 0.08f)},
                      Vec3{minimumDimension * (0.034f + transient * 0.030f),
                           minimumDimension * (0.090f + role.fracture * 0.070f + metrics.onset * 0.050f),
                           minimumDimension * (0.014f + transient * 0.022f)},
                      Vec3{0.48f + stagger * 0.34f, side * (0.18f + transient * 0.24f), cutStep * (kPi / 8.0f) + unit},
                      withAlpha(colors[2], 0.22f + role.fracture * 0.24f + transient * 0.16f),
                      0.20f + role.fracture * 0.54f + transient * 0.42f,
                      role.fracture,
                      unit);
        }
    }

    if (active(role.shadow)) {
        const int monoliths = scaledCount(3 + static_cast<int>(std::round(role.shadow * 4.0f)),
                                          density * (0.34f + role.shadow * 0.24f));
        for (int i = 0; i < monoliths; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, monoliths - 1));
            const float lane = unit - 0.5f;
            pushMotif(MusicalRoleDistrict3D::Shadow,
                      Object3DKind::Column,
                      Vec3{lane * minimumDimension * (0.34f + metrics.stereoWidth * 0.10f),
                           minimumDimension * (0.12f + role.shadow * 0.08f),
                           minimumDimension * (0.02f + unit * 0.48f + intent.dark * 0.12f)},
                      Vec3{minimumDimension * (0.028f + role.shadow * 0.020f),
                           minimumDimension * (0.30f + role.shadow * 0.32f + intent.minimal * 0.08f),
                           minimumDimension * (0.036f + role.shadow * 0.024f)},
                      Vec3{0.040f + intent.tension * 0.070f, lane * 0.10f, phase * 0.004f},
                      withAlpha(mix(colors[4], colors[3], 0.28f), 0.18f + role.shadow * 0.24f),
                      0.12f + role.shadow * 0.42f,
                      role.shadow,
                      unit);
            if (i == 0 || i == monoliths - 1 || role.shadow > 0.54f) {
                pushMotif(MusicalRoleDistrict3D::Shadow,
                          Object3DKind::Anchor,
                          Vec3{lane * minimumDimension * 0.16f,
                               minimumDimension * (0.04f + role.shadow * 0.04f),
                               minimumDimension * (0.18f + unit * 0.36f)},
                          Vec3{minimumDimension * (0.016f + role.shadow * 0.014f),
                               minimumDimension * (0.016f + role.shadow * 0.014f),
                               minimumDimension * 0.016f},
                          Vec3{0.0f, phase * 0.008f + lane * 0.08f, phase * 0.014f},
                          withAlpha(colors[4], 0.24f + role.shadow * 0.18f),
                          0.16f + role.shadow * 0.34f,
                          role.shadow,
                          unit);
            }
        }
...
void addMusicalRoleInstrumentScore3D(std::vector<Object3D>& objects,
                                     const MusicRoleScene3D& role,
                                     const SceneInterpretation& intent,
                                     const AudioMetrics& metrics,
                                     const VisualSettings& settings,
                                     ...
                                     double time)
{
    ...
    const float activeMass = role.bass + role.drums + role.melody + role.harmony +
                             role.space + role.fracture + role.shadow;
    if (activeCount < 1.3f && metrics.rms < 0.04f) {
        return;
    }
    ...
    const float downbeat = metrics.downbeat ? std::max(0.36f, metrics.downbeatConfidence)
                                            : metrics.downbeatConfidence * 0.32f;
    const float rhythmDrive = clamp01(metrics.beatConfidence * 0.58f +
                                      metrics.barConfidence * 0.26f +
                                      downbeat * 0.16f);
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.52f +
                                            metrics.dropIntensity * 0.32f +
                                            (metrics.phraseBoundary ? metrics.phraseConfidence * 0.28f : 0.0f) +
                                            (metrics.downbeat ? metrics.downbeatConfidence * 0.16f : 0.0f));
    const float roleAuthority = clamp01(role.separation * 0.58f +
                                        std::min(activeMass / 3.8f, 1.0f) * 0.18f +
                                        std::min(activeCount / 7.0f, 1.0f) * 0.12f +
                                        patternClarityOf(settings) * 0.12f +
                                        scenePersonalityOf(settings) * 0.10f);
    const float separationHold = clamp01(roleAuthority *
                                         (0.78f + motionStabilityOf(settings) * 0.12f) *
                                         (1.0f - earnedConvergence * 0.34f - role.convergence * 0.24f));
    const float convergencePermission = clamp01(role.convergence * (0.20f + earnedConvergence * 0.68f) +
                                                earnedConvergence * 0.22f);
    const float stereoSpread = 0.74f + metrics.stereoWidth * 0.64f;
    const float scaleGain = 0.92f + personality * 0.14f + response * 0.06f;
    const Vec3 convergencePoint = roleConvergencePoint3D(role, minimumDimension);
    ...
    const auto strengthOf = [&](MusicalRoleDistrict3D district) {
        return clamp01(roleDistrictStrength(role, district));
    };
    const auto active = [](float strength) {
        return strength > 0.11f;
    };
    const auto roleColor = [&](MusicalRoleDistrict3D district, float alpha) {
        ColorRGBA color = withAlpha(colors[0], alpha);
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            color = ColorRGBA{1.0f, 0.20f, 0.08f, alpha};
            break;
        case MusicalRoleDistrict3D::Drums:
            color = ColorRGBA{0.04f, 0.94f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Melody:
            color = ColorRGBA{1.0f, 0.18f, 0.86f, alpha};
            break;
        case MusicalRoleDistrict3D::Harmony:
            color = ColorRGBA{0.34f, 0.78f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Space:
            color = ColorRGBA{0.22f, 0.38f, 1.0f, alpha};
            break;
        case MusicalRoleDistrict3D::Fracture:
            color = ColorRGBA{1.0f, 0.96f, 0.10f, alpha};
            break;
        case MusicalRoleDistrict3D::Shadow:
            color = ColorRGBA{0.68f, 0.70f, 0.84f, alpha};
            break;
        }
        return shiftedHue(color, settings.hueShift);
    };
    const auto audioDrive = [&](MusicalRoleDistrict3D district) {
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return clamp01(metrics.bass * 0.64f + metrics.lowMid * 0.22f + metrics.dropIntensity * 0.28f + downbeat * 0.10f);
        case MusicalRoleDistrict3D::Drums:
            return clamp01(metrics.beatConfidence * 0.54f + metrics.barConfidence * 0.28f + downbeat * 0.18f);
        case MusicalRoleDistrict3D::Melody:
            return clamp01(harmonic * 0.72f + metrics.melodyRole * 0.18f + metrics.phraseIntensity * 0.10f);
        case MusicalRoleDistrict3D::Harmony:
            return clamp01(harmonic * 0.58f + metrics.harmonyRole * 0.22f + metrics.phraseConfidence * 0.20f);
        case MusicalRoleDistrict3D::Space:
            return clamp01(metrics.stereoWidth * 0.72f + role.space * 0.22f + intent.spacious * 0.12f);
        case MusicalRoleDistrict3D::Fracture:
            return clamp01(transient * 0.82f + metrics.fractureRole * 0.18f);
        case MusicalRoleDistrict3D::Shadow:
            return clamp01(role.shadow * 0.60f + intent.dark * 0.22f + (1.0f - metrics.treble) * 0.18f);
        }
        return 0.0f;
    };
    const auto roleVelocity = [&](MusicalRoleDistrict3D district, float strength, float drive, float seed) {
        const float voice = clamp01(strength * (0.62f + separationHold * 0.32f) + drive * 0.18f);
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            return Vec3{0.0f,
                        minimumDimension * voice * (0.010f + drive * 0.030f),
                        -minimumDimension * voice * (0.034f + drive * 0.070f + metrics.dropIntensity * 0.038f)};
        case MusicalRoleDistrict3D::Drums:
            return Vec3{std::round(std::sin(seed + metrics.beatPhase * kPi) * 3.0f) *
                            minimumDimension * voice * rhythmDrive * 0.030f,
                        minimumDimension * voice * rhythmDrive * 0.024f,
                        minimumDimension * voice * (0.010f + rhythmDrive * 0.070f)};
        case MusicalRoleDistrict3D::Melody:
            return Vec3{std::cos(seed + metrics.phrasePhase * kPi) * minimumDimension * voice * harmonic * 0.030f,
                        -minimumDimension * voice * harmonic * 0.044f,
                        std::sin(seed * 0.74f + phase * 0.022f) * minimumDimension * voice * harmonic * 0.040f};
        case MusicalRoleDistrict3D::Harmony:
            return Vec3{std::sin(seed * 0.52f) * minimumDimension * voice * harmonic * 0.020f,
                        -minimumDimension * voice * harmonic * 0.018f,
                        minimumDimension * voice * (0.030f + metrics.phraseConfidence * 0.034f)};
        case MusicalRoleDistrict3D::Space:
            return Vec3{std::sin(seed + phase * 0.012f) * minimumDimension * voice * metrics.stereoWidth * 0.022f,
                        std::cos(seed * 0.45f + phase * 0.010f) * minimumDimension * voice * 0.016f,
                        minimumDimension * voice * (0.048f + metrics.stereoWidth * 0.060f)};
        case MusicalRoleDistrict3D::Fracture:
            return Vec3{std::sin(seed * 3.2f) * minimumDimension * voice * (0.050f + transient * 0.070f),
                        std::cos(seed * 2.1f) * minimumDimension * voice * transient * 0.024f,
                        std::cos(seed * 2.7f) * minimumDimension * voice * (0.040f + transient * 0.060f)};
        case MusicalRoleDistrict3D::Shadow:
            return Vec3{0.0f,
                        minimumDimension * voice * (0.016f + role.shadow * 0.044f),
                        minimumDimension * voice * (0.034f + intent.dark * 0.040f + role.shadow * 0.030f)};
        }
        return Vec3{};
    };
    const auto place = [&](MusicalRoleDistrict3D district, Vec3 local, float mergeBias) {
        const float strength = strengthOf(district);
        const Vec3 center = roleDistrictCenter3D(district, role, intent, metrics, settings, minimumDimension, time);
        const Vec3 districtBias = scale(roleDistrictUnitOffset(district),
                                        minimumDimension * separationHold * (0.060f + strength * 0.052f));
        const float merge = clamp01(mergeBias * convergencePermission *
                                    (0.18f + earnedConvergence * 0.42f) *
                                    (1.0f - separationHold * 0.48f));
        return mix(add(add(center, local), districtBias), convergencePoint, merge);
    };
    const auto pushInstrument = [&](MusicalRoleDistrict3D district,
                                    Object3DKind kind,
                                    Vec3 local,
                                    Vec3 scaleValue,
                                    Vec3 rotation,
        ...
        const float drive = audioDrive(district);
        Object3D object = makeObject3D(kind,
                                       place(district, local, mergeBias),
                                       scale(scaleValue, scaleGain),
                                       rotation,
                                       roleColor(district, alpha),
                                       glow);
        object.musicRole = objectRoleForDistrict(district);
        object.velocity = roleVelocity(district, strength, drive, local.x * 0.006f + local.z * 0.004f + phase);
        const float presence = clamp01(strength * 0.48f + drive * 0.30f + separationHold * 0.22f);
        object.color.a = clamp01(object.color.a * (1.06f + presence * 0.36f));
        object.glow += presence * (0.08f + response * 0.06f);
        switch (district) {
        case MusicalRoleDistrict3D::Bass:
            object.scale.y *= 1.0f + drive * strength * 0.22f;
            object.scale.z *= 1.0f + drive * strength * 0.18f;
            object.rotation.z *= 0.48f;
            break;
        case MusicalRoleDistrict3D::Drums:
            object.rotation.x = std::round(object.rotation.x * 12.0f) / 12.0f;
            object.rotation.z = std::round(object.rotation.z * 16.0f) / 16.0f;
            object.scale.y *= 1.0f + rhythmDrive * strength * 0.28f;
            object.glow += rhythmDrive * strength * 0.12f;
            break;
        case MusicalRoleDistrict3D::Melody:
            object.scale.z *= 1.0f + harmonic * strength * 0.16f;
            object.rotation.y += harmonic * strength * 0.12f;
            break;
        case MusicalRoleDistrict3D::Harmony:
            object.scale.x *= 1.0f + harmonic * strength * 0.14f;
            object.scale.z *= 1.0f + metrics.phraseConfidence * strength * 0.12f;
            break;
        case MusicalRoleDistrict3D::Space:
            object.scale.x *= 1.0f + metrics.stereoWidth * strength * 0.18f;
            object.color.a = clamp01(object.color.a * 0.94f);
            break;
        case MusicalRoleDistrict3D::Fracture:
            object.rotation.y += transient * strength * 0.20f;
            object.glow += transient * strength * 0.18f;
            break;
        case MusicalRoleDistrict3D::Shadow:
            object.scale.y *= 1.0f + role.shadow * strength * 0.22f;
            object.rotation.z *= 0.28f;
            object.glow *= 0.82f;
            break;
        }
        if (anchor) {
            anchors[slot(district)].push_back(object.position);
        }
        const Vec3 position = object.position;
        objects.push_back(object);
        return position;
    };
    const auto pushRoleLink = [&](MusicalRoleDistrict3D district, Vec3 from, Vec3 to, float alpha, float glow) {
        Object3D link = makeObject3D(Object3DKind::Link,
                                     from,
                                     Vec3{1.0f, 1.0f, 1.0f},
                                     Vec3{},
        ...
        link.target = to;
        link.musicRole = objectRoleForDistrict(district);
        link.velocity = roleVelocity(district, strengthOf(district), audioDrive(district), from.x * 0.005f + to.z * 0.003f);
        objects.push_back(link);
    };

    if (active(role.bass)) {
        const int ribs = scaledCount(5 + static_cast<int>(std::round(role.bass * 5.0f)),
                                     density * (0.42f + role.bass * 0.26f + metrics.bass * 0.12f));
        Vec3 previous{};
        bool havePrevious = false;
        for (int i = 0; i < ribs; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, ribs - 1));
            const float compression = std::pow(1.0f - unit, 1.55f) * (metrics.bass * 0.16f + metrics.dropIntensity * 0.24f);
            const float radius = minimumDimension * (0.15f + unit * 0.14f + role.bass * 0.09f - compression * 0.035f);
            const Vec3 current = pushInstrument(MusicalRoleDistrict3D::Bass,
                                                Object3DKind::TunnelRib,
                                                Vec3{0.0f,
                                                     minimumDimension * (0.030f + compression * 0.18f),
                                                     minimumDimension * (-0.34f + unit * 0.74f - compression)},
                                                Vec3{radius,
                                                     radius * (0.42f + metrics.lowMid * 0.18f),
                                                     0.72f + role.bass * 0.74f + metrics.dropIntensity * 0.26f},
                                                Vec3{compression * 0.20f, phase * 0.010f, metrics.beatPhase * kPi * 0.22f + unit},
                                                0.23f + role.bass * 0.22f,
                                                0.28f + role.bass * 0.64f + metrics.dropIntensity * 0.34f,
                                                0.14f);
            if (havePrevious && i % 2 == 0) {
                pushRoleLink(MusicalRoleDistrict3D::Bass, previous, current, 0.045f + metrics.bass * 0.035f, 0.050f + metrics.bass * 0.10f);
            }
            previous = current;
            havePrevious = true;
        }
        const int weights = scaledCount(3 + static_cast<int>(std::round(role.bass * 3.0f)),
                                        density * (0.36f + role.bass * 0.22f));
        for (int i = 0; i < weights; ++i) {
            const float unit = weights > 1 ? static_cast<float>(i) / static_cast<float>(weights - 1) : 0.0f;
            const float lane = unit - 0.5f;
            pushInstrument(MusicalRoleDistrict3D::Bass,
                           Object3DKind::Column,
                           Vec3{lane * minimumDimension * (0.30f + metrics.stereoWidth * 0.10f),
                                minimumDimension * (0.12f + metrics.bass * 0.08f),
                                minimumDimension * (-0.28f + unit * 0.28f - metrics.dropIntensity * 0.08f)},
                           Vec3{minimumDimension * (0.030f + role.bass * 0.020f),
                                minimumDimension * (0.24f + role.bass * 0.26f + metrics.dropIntensity * 0.08f),
                                minimumDimension * (0.036f + role.bass * 0.020f)},
                           Vec3{0.04f, lane * (0.16f + role.bass * 0.08f), phase * 0.010f},
                           0.20f + role.bass * 0.24f,
                           0.20f + role.bass * response * 0.52f,
                           0.10f);
        }
    }

    if (active(role.drums)) {
        const int lanes = scaledCount(3, density * (0.38f + role.drums * 0.20f));
        const int steps = scaledCount(5 + static_cast<int>(std::round(role.drums * 5.0f)),
                                      density * (0.36f + role.drums * 0.22f + metrics.beatConfidence * 0.08f));
        const int activeStep = std::clamp(static_cast<int>(std::floor(clamp01(metrics.barPhase) * static_cast<float>(steps))),
                                          0,
                                          std::max(0, steps - 1));
        for (int lane = 0; lane < lanes; ++lane) {
            ...
            for (int step = 0; step < steps; ++step) {
                const float stepUnit = steps > 1 ? static_cast<float>(step) / static_cast<float>(steps - 1) : 0.0f;
                const float pulse = step == activeStep ? (0.14f + rhythmDrive * 0.86f) : rhythmDrive * 0.20f;
                const Vec3 current = pushInstrument(MusicalRoleDistrict3D::Drums,
                                                    (step + lane) % 3 == 0 ? Object3DKind::Cage :
                                                        ((step + lane) % 2 == 0 ? Object3DKind::Plate : Object3DKind::Column),
                                                    Vec3{(laneUnit - 0.5f) * minimumDimension * 0.48f * stereoSpread,
                                                         (laneUnit - 0.5f) * minimumDimension * 0.070f,
                                                         minimumDimension * (-0.14f + stepUnit * 0.62f + pulse * 0.060f)},
                                                    Vec3{minimumDimension * (0.014f + pulse * 0.012f),
                                                         minimumDimension * (0.064f + role.drums * 0.074f + pulse * 0.072f),
                                                         minimumDimension * 0.014f},
                                                    Vec3{0.0f, laneUnit * 0.18f, std::round(metrics.beatPhase * 8.0f) * (kPi / 8.0f)},
                                                    0.16f + role.drums * 0.18f + pulse * 0.18f,
                                                    0.15f + role.drums * 0.40f + pulse * 0.34f,
                                                    0.08f);
                if (havePrevious && (step % 2 == 0 || step == activeStep)) {
                    pushRoleLink(MusicalRoleDistrict3D::Drums, previous, current, 0.040f + beat * 0.050f, 0.050f + beat * 0.12f);
                }
                previous = current;
                havePrevious = true;
            }
    ...
    if (active(role.melody)) {
        const int notes = scaledCount(8 + static_cast<int>(std::round(role.melody * 7.0f)),
                                      density * (0.40f + role.melody * 0.28f + harmonic * 0.12f));
        Vec3 previous{};
        bool havePrevious = false;
        for (int i = 0; i < notes; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, notes - 1));
            const float chroma = chromaAt(metrics, static_cast<std::size_t>(i + std::max(0, metrics.keyIndex)));
            const float angle = unit * 2.8f * kPi + metrics.phrasePhase * kPi + harmonic * 0.42f + phase * 0.010f;
            const Vec3 current = pushInstrument(MusicalRoleDistrict3D::Melody,
                                                i % 3 == 0 ? Object3DKind::Ribbon : (i % 2 == 0 ? Object3DKind::Shard : Object3DKind::Node),
                                                Vec3{std::cos(angle) * minimumDimension * (0.12f + chroma * 0.16f) * stereoSpread,
                                                     -minimumDimension * (0.14f + unit * 0.13f + chroma * 0.055f),
                                                     minimumDimension * (0.02f + unit * 0.58f + chroma * 0.24f)},
                                                Vec3{minimumDimension * (0.020f + chroma * 0.038f),
                                                     minimumDimension * (0.034f + role.melody * 0.048f + harmonic * 0.022f),
                                                     minimumDimension * (0.026f + chroma * 0.024f)},
                                                Vec3{angle * 0.16f, phase * 0.034f + chroma * kPi, harmonic * kPi},
                                                0.22f + role.melody * 0.22f + chroma * 0.16f,
                                                0.20f + role.melody * 0.52f + harmonic * 0.32f,
                                                0.10f);
            if (havePrevious && i % 2 == 0) {
                pushRoleLink(MusicalRoleDistrict3D::Melody, previous, current, 0.052f + harmonic * 0.060f, 0.060f + harmonic * 0.15f);
            }
            previous = current;
            havePrevious = true;
        }
    ...
    if (active(role.harmony)) {
        const int chordPairs = scaledCount(4 + static_cast<int>(std::round(role.harmony * 4.0f)),
                                           density * (0.34f + role.harmony * 0.24f + harmonic * 0.08f));
        for (int i = 0; i < chordPairs; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, chordPairs - 1));
            const float span = minimumDimension * (0.12f + unit * 0.17f + metrics.stereoWidth * 0.06f) * stereoSpread;
            const Vec3 left = pushInstrument(MusicalRoleDistrict3D::Harmony,
                                             i % 3 == 0 ? Object3DKind::DepthPlane : Object3DKind::Cage,
                                             Vec3{-span,
                                                  -minimumDimension * (0.040f + harmonic * 0.044f),
                                                  minimumDimension * (0.12f + unit * 0.48f + metrics.phraseConfidence * 0.10f)},
                                             Vec3{minimumDimension * (0.082f + role.harmony * 0.064f + unit * 0.026f),
                                                  minimumDimension * (0.052f + harmonic * 0.042f),
                                                  minimumDimension * (0.092f + harmonic * 0.052f)},
                                             Vec3{0.32f + unit * 0.10f, phase * 0.012f - 0.08f, harmonic * 0.46f},
                                             0.15f + role.harmony * 0.22f + harmonic * 0.10f,
                                             0.14f + role.harmony * 0.40f + harmonic * 0.24f,
                                             0.12f);
            const Vec3 right = pushInstrument(MusicalRoleDistrict3D::Harmony,
                                              Object3DKind::WaveSurface,
                                              Vec3{span,
                                                   -minimumDimension * (0.040f + harmonic * 0.044f),
                                                   minimumDimension * (0.16f + unit * 0.48f + metrics.phraseConfidence * 0.10f)},
                                              Vec3{minimumDimension * (0.16f + role.harmony * 0.10f),
                                                   minimumDimension * (0.060f + harmonic * 0.040f),
                                                   minimumDimension * (0.022f + role.harmony * 0.018f)},
                                              Vec3{0.38f + unit * 0.16f, phase * 0.010f + 0.08f, metrics.phrasePhase * kPi + unit},
                                              0.12f + role.harmony * 0.20f + harmonic * 0.08f,
                                              0.12f + role.harmony * 0.32f + harmonic * 0.20f,
                                              0.10f);
            pushRoleLink(MusicalRoleDistrict3D::Harmony, left, right, 0.050f + harmonic * 0.055f, 0.055f + harmonic * 0.12f);
        }
    }

    if (active(role.space)) {
        if (!settings.interactiveField && role.space > 0.48f && metrics.stereoWidth > 0.55f) {
            const Vec3 nearPortal = pushInstrument(MusicalRoleDistrict3D::Space,
                                                   Object3DKind::DepthPlane,
                                                   Vec3{-minimumDimension * (0.08f + metrics.stereoWidth * 0.10f),
                                                        -minimumDimension * (0.02f + role.space * 0.020f),
                                                        -minimumDimension * (0.10f + metrics.stereoWidth * 0.06f)},
                                                   Vec3{minimumDimension * (0.20f + metrics.stereoWidth * 0.12f),
                                                        minimumDimension * (0.10f + role.space * 0.06f),
                                                        minimumDimension * 0.018f},
                                                   Vec3{0.48f, phase * 0.006f, -0.18f},
                                                   0.10f + role.space * 0.16f,
                                                   0.06f + role.space * 0.18f,
                                                   0.02f);
            const Vec3 farPortal = pushInstrument(MusicalRoleDistrict3D::Space,
                                                  Object3DKind::WaveSurface,
                                                  Vec3{minimumDimension * (0.10f + metrics.stereoWidth * 0.16f),
                                                       minimumDimension * (0.02f + role.space * 0.030f),
                                                       minimumDimension * (0.58f + metrics.stereoWidth * 1.10f + role.space * 0.20f)},
                                                  Vec3{minimumDimension * (0.28f + metrics.stereoWidth * 0.18f),
                                                       minimumDimension * (0.13f + role.space * 0.07f),
                                                       minimumDimension * (0.020f + metrics.stereoWidth * 0.018f)},
                                                  Vec3{0.54f, phase * 0.004f, 0.22f},
                                                  0.09f + role.space * 0.14f + metrics.stereoWidth * 0.06f,
                                                  0.06f + role.space * 0.16f + metrics.stereoWidth * 0.08f,
                                                  0.02f);
            pushRoleLink(MusicalRoleDistrict3D::Space,
                         nearPortal,
                         farPortal,
                         0.034f + metrics.stereoWidth * 0.040f,
                         0.040f + metrics.stereoWidth * 0.08f);
        }

        const int rooms = scaledCount(5 + static_cast<int>(std::round(role.space * 5.0f)),
                                      density * (0.36f + role.space * 0.24f + metrics.stereoWidth * 0.12f));
        for (int i = 0; i < rooms; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, rooms - 1));
            const float angle = unit * 2.0f * kPi + phase * (0.008f + metrics.stereoWidth * 0.010f);
            pushInstrument(MusicalRoleDistrict3D::Space,
                           i % 3 == 0 ? Object3DKind::DepthPlane : (i % 2 == 0 ? Object3DKind::WaveSurface : Object3DKind::Orbiter),
                           Vec3{std::cos(angle) * minimumDimension * (0.12f + unit * 0.20f + metrics.stereoWidth * 0.10f) * stereoSpread,
                                std::sin(angle * 0.66f) * minimumDimension * (0.05f + role.space * 0.05f),
                                minimumDimension * (0.22f +
                                                    unit * (0.82f + metrics.stereoWidth * 0.46f) +
                                                    metrics.stereoWidth * 0.12f)},
                           Vec3{minimumDimension * (0.15f + unit * 0.095f + metrics.stereoWidth * 0.074f),
                                minimumDimension * (0.082f + role.space * 0.074f),
                                minimumDimension * (0.016f + unit * 0.015f)},
                           Vec3{0.46f + unit * 0.16f, angle * 0.12f, phase * 0.008f + unit},
                           0.11f + role.space * 0.18f + metrics.stereoWidth * 0.08f,
                           0.08f + role.space * 0.20f + metrics.stereoWidth * 0.10f,
                           0.08f);
        }
    }
    ...
    if (active(role.fracture)) {
        const int cuts = scaledCount(6 + static_cast<int>(std::round(role.fracture * 6.0f)),
                                     density * (0.38f + role.fracture * 0.30f + transient * 0.12f));
        const float cutStep = std::floor(clamp01(metrics.beatPhase) * 8.0f);
        for (int i = 0; i < cuts; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, cuts - 1));
            const float stagger = static_cast<float>((i * 5 + 2) % 11) / 10.0f;
            const float side = (i + static_cast<int>(cutStep)) % 2 == 0 ? -1.0f : 1.0f;
            pushInstrument(MusicalRoleDistrict3D::Fracture,
                           i % 2 == 0 ? Object3DKind::Plate : Object3DKind::Shard,
                           Vec3{side * minimumDimension * (0.12f + unit * 0.36f) * stereoSpread,
                                (stagger - 0.5f) * minimumDimension * (0.24f + transient * 0.09f),
                                minimumDimension * (-0.16f + stagger * 0.66f + side * transient * 0.09f)},
                           Vec3{minimumDimension * (0.034f + transient * 0.032f),
                                minimumDimension * (0.094f + role.fracture * 0.074f + metrics.onset * 0.052f),
                                minimumDimension * (0.014f + transient * 0.024f)},
                           Vec3{0.48f + stagger * 0.34f, side * (0.18f + transient * 0.24f), cutStep * (kPi / 8.0f) + unit},
                           0.20f + role.fracture * 0.24f + transient * 0.16f,
                           0.20f + role.fracture * 0.54f + transient * 0.42f,
                           0.08f);
        }
    }
    ...
    if (active(role.shadow)) {
        const int monoliths = scaledCount(4 + static_cast<int>(std::round(role.shadow * 4.0f)),
                                          density * (0.34f + role.shadow * 0.24f));
        for (int i = 0; i < monoliths; ++i) {
            const float unit = monoliths > 1 ? static_cast<float>(i) / static_cast<float>(monoliths - 1) : 0.0f;
            const float lane = unit - 0.5f;
            pushInstrument(MusicalRoleDistrict3D::Shadow,
                           Object3DKind::Column,
                           Vec3{lane * minimumDimension * (0.36f + metrics.stereoWidth * 0.10f),
                                minimumDimension * (0.14f + role.shadow * 0.09f),
                                minimumDimension * (0.02f + unit * 0.50f + intent.dark * 0.12f)},
                           Vec3{minimumDimension * (0.030f + role.shadow * 0.020f),
                                minimumDimension * (0.32f + role.shadow * 0.34f + intent.minimal * 0.08f),
                                minimumDimension * (0.038f + role.shadow * 0.024f)},
                           Vec3{0.040f + intent.tension * 0.070f, lane * 0.10f, phase * 0.004f},
                           0.17f + role.shadow * 0.22f,
            ...
            if (i == 0 || i == monoliths - 1 || role.shadow > 0.54f) {
                pushInstrument(MusicalRoleDistrict3D::Shadow,
                               Object3DKind::Anchor,
                               Vec3{lane * minimumDimension * 0.16f,
                                    minimumDimension * (0.045f + role.shadow * 0.040f),
                                    minimumDimension * (0.18f + unit * 0.38f)},
                               Vec3{minimumDimension * (0.018f + role.shadow * 0.014f),
                                    minimumDimension * (0.018f + role.shadow * 0.014f),
                                    minimumDimension * 0.018f},
                               Vec3{0.0f, phase * 0.008f + lane * 0.08f, phase * 0.014f},
                               0.22f + role.shadow * 0.16f,
                               0.16f + role.shadow * 0.32f,
                               0.06f);
            }
        }
    }

    if (convergencePermission > 0.22f && activeCount >= 2.0f) {
        Object3D knot = makeObject3D(Object3DKind::Anchor,
                                     convergencePoint,
                                     Vec3{minimumDimension * (0.020f + convergencePermission * 0.030f),
                                          minimumDimension * (0.020f + convergencePermission * 0.030f),
                                          minimumDimension * (0.020f + convergencePermission * 0.030f)},
                                     Vec3{phase * 0.010f, metrics.phrasePhase * kPi, phase * 0.014f},
                                     withAlpha(shiftedHue(mix(colors[2], colors[3], 0.46f), settings.hueShift),
                                               0.12f + convergencePermission * 0.24f),
                                     0.18f + convergencePermission * 0.58f);
        knot.musicRole = Object3DRole::Convergence;
        objects.push_back(knot);
        ...
        int links = 0;
        for (MusicalRoleDistrict3D district : districts) {
            const auto& roleAnchors = anchors[slot(district)];
            if (roleAnchors.empty() || strengthOf(district) <= 0.16f) {
                continue;
            }

            const Vec3 from = roleAnchors[roleAnchors.size() / 2U];
            Object3D bridge = makeObject3D(Object3DKind::Link,
                                           from,
                                           Vec3{1.0f, 1.0f, 1.0f},
                                           Vec3{},
                                           roleColor(district, 0.050f + convergencePermission * 0.075f),
                                           0.055f + convergencePermission * 0.16f);
            bridge.target = mix(convergencePoint,
                                add(convergencePoint, scale(roleDistrictUnitOffset(district),
                                                            minimumDimension * (0.070f + strengthOf(district) * 0.045f))),
                                0.34f);
            bridge.musicRole = Object3DRole::Bridge;
            bridge.velocity = scale(subtract(bridge.target, bridge.position), 0.020f + convergencePermission * 0.018f);
        ...
        const int petals = scaledCount(3 + static_cast<int>(std::round(convergencePermission * 4.0f)),
                                       density * (0.22f + convergencePermission * 0.18f));
        for (int i = 0; i < petals; ++i) {
            const float unit = static_cast<float>(i) / static_cast<float>(std::max(1, petals - 1));
            const float angle = unit * 2.0f * kPi + phase * 0.012f;
            Object3D petal = makeObject3D(i % 2 == 0 ? Object3DKind::WaveSurface : Object3DKind::Ribbon,
                                          add(convergencePoint,
                                              Vec3{std::cos(angle) * minimumDimension * convergencePermission * 0.16f,
                                                   std::sin(angle * 0.7f) * minimumDimension * convergencePermission * 0.08f,
                                                   std::sin(angle) * minimumDimension * convergencePermission * 0.14f}),
                                          Vec3{minimumDimension * (0.12f + convergencePermission * 0.12f),
                                               minimumDimension * (0.052f + convergencePermission * 0.070f),
                                               minimumDimension * (0.020f + convergencePermission * 0.018f)},
                                          Vec3{0.38f + unit * 0.18f, angle * 0.18f, phase * 0.012f + unit},
                                          withAlpha(shiftedHue(mix(colors[2], colors[0], unit), settings.hueShift),
                                                    0.10f + convergencePermission * 0.16f),
                                          0.12f + convergencePermission * 0.30f);
            petal.musicRole = Object3DRole::Convergence;
            petal.velocity = scale(subtract(convergencePoint, petal.position), 0.018f + convergencePermission * 0.020f);
            objects.push_back(petal);
        }
    }
}
                                    ...
                                    double time)
{
    ...
    density *= std::clamp(0.92f + clarity * 0.08f - roleFocus * 0.05f - role.convergence * 0.02f,
                          0.82f,
                          1.04f);
    const float earnedConvergence = clamp01(metrics.convergenceRole * 0.52f +
                                            phrasePull * 0.34f +
    ...
    const auto districtOffset = [&](RoleDistrict district) {
        switch (district) {
        case RoleDistrict::Bass:
            return Vec3{0.00f, 0.36f, -0.88f};
        case RoleDistrict::Drums:
            return Vec3{-0.96f, 0.02f, -0.22f};
        case RoleDistrict::Melody:
            return Vec3{0.92f, -0.46f, 0.24f};
        case RoleDistrict::Harmony:
            return Vec3{0.40f, -0.28f, 0.72f};
        case RoleDistrict::Space:
            return Vec3{-0.18f, -0.04f, 1.14f};
        case RoleDistrict::Fracture:
            return Vec3{1.10f, 0.20f, -0.44f};
        case RoleDistrict::Shadow:
            return Vec3{-0.82f, 0.40f, 0.58f};
        }
        return Vec3{};
    };
    ...
    const auto pushRoleObject = [&](Object3D object,
        ...
        object.color.a = clamp01(object.color.a * (1.08f + rolePresence * 0.42f));
        object.glow *= 1.0f + rolePresence * 0.24f;
        if (object.kind == Object3DKind::DepthPlane ||
            object.kind == Object3DKind::WaveSurface ||
            object.kind == Object3DKind::Plate ||
            object.kind == Object3DKind::TunnelRib) {
            object.scale = scale(object.scale, 1.0f + rolePresence * 0.12f);
        }
        if (anchor) {
    ...
    if (role.melody > 0.08f) {
        ...
        const std::size_t melodyLinkStride =
            earnedConvergence > 0.36f || convergence > 0.30f ? 1U : (partIsolation > 0.52f ? 3U : 2U);
        for (std::size_t i = 1; i < melodyAnchors.size(); ++i) {
            if (i % melodyLinkStride != 0U && i + 1U != melodyAnchors.size()) {
                continue;
            }
    ...
    const auto pushRoleBridge = [&](const std::vector<Vec3>& fromAnchors,
                                    ...
                                    float relationship,
                                    int offset) {
        const float bridgeReadiness = clamp01(earnedConvergence * 0.70f +
                                              convergence * 0.24f +
                                              metrics.convergenceRole * 0.20f);
        relationship *= std::clamp(0.10f + bridgeReadiness * 0.88f + convergence * 0.10f,
                                   0.08f,
                                   1.02f);
        const float bridgeThreshold = std::clamp(0.62f +
                                                     districtHold * 0.34f +
                                                     partIsolation * 0.24f -
                                                     bridgeReadiness * 0.30f,
                                                 0.54f,
                                                 0.94f);
        ...
        const float bridgePull = clamp01(convergence * 0.16f + earnedConvergence * 0.10f);
        const Vec3 from = mix(fromAnchors[fromIndex], convergencePoint, bridgePull * 0.08f);
        const Vec3 to = mix(toAnchors[toIndex], convergencePoint, bridgePull * 0.08f);
        pushLink(from,
                 to,
                 withAlpha(color,
                           0.004f +
                               relationship * (0.008f + bridgeReadiness * 0.026f) +
                               convergence * 0.020f),
                 0.008f + relationship * (0.022f + bridgeReadiness * 0.050f) + convergence * 0.060f);

        if (convergence > 0.38f && bridgeReadiness > 0.34f && relationship > bridgeThreshold + 0.08f) {
            ...
            Object3D surface = makeObject3D((offset % 2 == 0) ? Object3DKind::WaveSurface : Object3DKind::Plate,
                                            mix(mix(from, to, 0.5f), convergencePoint, convergence * 0.10f),
                                            ...
                                            withAlpha(color,
                                                      0.030f +
                                                          relationship * 0.045f +
                                                          bridgeReadiness * 0.040f +
                                                          convergence * 0.055f),
                                            0.05f + relationship * 0.12f + bridgeReadiness * 0.10f + convergence * 0.20f);
    ...
    if (convergence > 0.34f && earnedConvergence > 0.24f) {
        Object3D knot = makeObject3D(Object3DKind::Cage,
                                     convergencePoint,
        ...
        const auto connectRole = [&](const std::vector<Vec3>& anchors, ColorRGBA color, int stride) {
            for (std::size_t i = 0; i < anchors.size(); i += static_cast<std::size_t>(std::max(1, stride))) {
                pushLink(anchors[i],
                         mix(anchors[i], convergencePoint, 0.30f + convergence * 0.20f),
                         color,
                         0.040f + earnedConvergence * 0.070f + convergence * 0.20f,
                         Object3DRole::Convergence);
            }
        };
...
void addQuietSceneScaffold3D(std::vector<Object3D>& objects,
                             const SceneInterpretation& intent,
                             const AudioMetrics& metrics,
                             const VisualSettings& settings,
                             const std::array<ColorRGBA, 5>& colors,
                             ...
                             double time)
{
    const bool trueSilence = metrics.style == AudioStyle::Silence &&
                             ...
                             metrics.peak < 0.055f &&
                             metrics.beatConfidence < 0.08f;
    const float quietness = trueSilence
                                ? 1.0f
                                : clamp01(1.0f - smootherStep(0.025f, 0.145f, metrics.rms) +
                                          intent.calm * 0.22f +
                                          intent.minimal * 0.10f);
    const float depthPresence = depth3DOf(settings);
    if (quietness <= 0.08f || depthPresence <= 0.01f) {
        return;
    }
    ...
    for (int i = 0; i < rooms; ++i) {
        ...
        chamberAnchors.push_back(pushQuietObject(Object3DKind::DepthPlane,
                                                 ...
                                                 withAlpha(mix(colors[4], colors[2], 0.20f + unit * 0.38f),
                                                           alphaBase + unit * 0.014f),
                                                 glowBase + unit * 0.012f,
                                                 Vec3{0.0f,
                                                      0.0f,
                                                      minimumDimension * motionTrace * 0.004f}));
    }

    for (std::size_t i = 1U; i < chamberAnchors.size(); ++i) {
        Object3D link = makeObject3D(Object3DKind::Link,
                                     chamberAnchors[i - 1U],
                                     Vec3{1.0f, 1.0f, 1.0f},
                                     Vec3{},
                                     withAlpha(colors[3], 0.035f + quietness * 0.025f + audibleTrace * 0.018f),
                                     0.035f + quietness * 0.045f);
        link.target = chamberAnchors[i];
        link.musicRole = Object3DRole::Harmony;
        objects.push_back(link);
    }
    ...
    for (int i = 0; i < arcs; ++i) {
        ...
        pushQuietObject(i % 3 == 0 ? Object3DKind::WaveSurface : Object3DKind::Orbiter,
                        ...
                        withAlpha(mix(colors[(i + 2) % 5], colors[4], 0.38f + layer * 0.20f),
                                  alphaBase + (i % 3 == 0 ? 0.030f : 0.055f)),
                        glowBase + (i % 3 == 0 ? 0.060f : 0.080f) + audibleTrace * 0.040f,
                        Vec3{std::sin(angle) * minimumDimension * motionTrace * 0.004f,
                             0.0f,
    ...
    for (int i = 0; i < markers; ++i) {
        ...
        pushQuietObject(Object3DKind::Column,
                        Object3DRole::Shadow,
                        Vec3{side * minimumDimension * (0.24f + metrics.stereoWidth * 0.06f),
                             minimumDimension * (0.11f + quietness * 0.045f),
                             minimumDimension * (-0.28f + unit * 0.80f)},
                        Vec3{minimumDimension * (0.012f + quietness * 0.008f),
                             minimumDimension * (0.11f + quietness * 0.11f + audibleTrace * 0.040f),
                             minimumDimension * (0.014f + quietness * 0.008f)},
                        Vec3{0.020f,
                             side * 0.08f,
                             slowPhase * 0.20f},
                      ...
                      double time)
{
    ...
    const float personality = scenePersonalityOf(settings);
    const float response = musicResponse3D(metrics, settings);
    const SongSceneIdentity candidateSongIdentity = songSceneIdentityFor(intent, metrics, settings.mode);
    const SongIdentitySelection3D songIdentitySelection =
    ...
    addSongIdentitySetPieces3D(objects,
                               ...
                               response,
                               time);
    addSongIdentityHeroArchitecture3D(objects,
                                      songIdentity,
                                      intent,
                                      metrics,
                                      colors,
    ...
    addEnvironmentContextObjects3D(objects,
                                   ...
                                   response,
                                   time);
    addQuietSceneScaffold3D(objects,
                            intent,
                            metrics,
                            settings,
                            colors,
    ...
    addMusicalRoleOrchestraMotifs3D(objects,
                                    roleScene,
                                    intent,
                                    metrics,
                                    settings,
                                    ...
                                    response,
                                    time);
    addMusicalRoleInstrumentScore3D(objects,
                                    roleScene,
                                    intent,
                                    metrics,
                                    settings,
    ...
    applyMusicalScorePartition3D(objects, roleScene, intent, metrics, settings, minimumDimension, time);
    applyMusicalDepthScoreStaging3D(objects, roleScene, intent, metrics, settings, minimumDimension, time);
    applyMusicalRoleRepresentativeClarity3D(objects,
                                            roleScene,
                                            intent,
                                            metrics,
                                            settings,
    ...
    applyEmotionalSectionRoleChoreography3D(objects,
                                            sectionNarrative,
                                            roleScene,
                                            metrics,
                                            settings,
                                            minimumDimension,
                                            time);
    applyMusicalCounterpointConvergence3D(objects,
                                          roleScene,
                                          sectionNarrative,
                                          metrics,
                                          settings,
                                          ...
                                          minimumDimension,
                                          time);
    addSongIdentitySceneGrammarFinalizer3D(objects,
                                           songIdentity,
                                           intent,
                                           metrics,
                                           settings,
    ...
    applySongIdentitySignatureAccents3D(objects,
                                        songIdentity,
                                        intent,
                                        roleScene,
                                        metrics,
    ...
    frame.sectionTransform3D = sectionTransform.transform;
    frame.sectionDepthMotion3D = sectionTransform.depthMotion;
    frame.sectionMaterialShift3D = sectionTransform.materialShift;
    ...
    frame.songIdentityName = songSceneIdentityName(songIdentity);
    frame.songIdentityConfidence3D = songIdentitySelection.confidence;
    frame.songIdentityContinuity3D = songIdentitySelection.continuity;
    ...
    frame.sceneRoleLegibility3D = roleStats.roleLegibility;
    frame.sceneRoleMotionContrast3D = roleStats.roleMotionContrast;
    frame.sceneMusicalStructure3D = roleStats.musicalStructure;
