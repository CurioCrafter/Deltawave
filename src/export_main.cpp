...
{
    std::cout
        << "VisualizerExport --input song.wav --output frames [options]\n\n"
        << "Options:\n"
        << "  --input FILE       WAV, or Windows Media Foundation-supported audio on Windows\n"
        << "  --width N          Output width, default 1280\n"
        << "  --height N         Output height, default 720\n"
        << "  --fps N            Frames per second, default 60\n"
        << "  --seconds N        Export only the first N seconds\n"
        << "  --mp4 FILE         Encode frames to a shareable H.264 MP4 with FFmpeg\n"
        << "  --style-profile FILE  Load an adaptive style profile before analysis\n"
        << "  --sync-profile FILE   Load an adaptive beat/section sync profile before analysis\n"
        << "  --share           Write index.html and share_manifest.json beside the frames\n"
        ...
        << "  --auto-scene       Let audio style/BPM/drop metrics adapt mode, palette, speed, and intensity\n"
        << "  --environment      Enable time-of-day environmental visual influence\n"
...
void printCuratedLooks()
{
    std::cout << "Built-in looks:\n";
    for (const viz::VisualPreset& preset : viz::curatedPresets()) {
        std::cout << "  " << preset.name
                  ...
                  << (preset.settings.autoScene ? " | auto-scene" : "")
                  << "\n";
    }
}
...
{
    viz::OfflineExportOptions options;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--input" && nextValue(i, argc, argv, value)) {
            options.inputAudio = value;
        ...
        } else if (arg == "--mp4" && nextValue(i, argc, argv, value)) {
            options.outputVideo = value;
    ...
    if (options.inputAudio.empty() || options.outputDirectory.empty()) {
        printUsage();
        return 2;
    }

    viz::OfflineExportResult result;
    std::string error;
    if (!viz::exportAudioToFrames(options, result, error)) {
        std::cerr << "Export failed: " << error << "\n";
        return 1;
    }

    std::cout << "Exported " << result.framesWritten << " frames to "
              << result.outputDirectory.string() << "\n";
    std::cout << "Peak RMS " << result.peakRms
              << " BPM " << result.estimatedBpm
              << " Beats " << result.beatsDetected << "\n";
    std::cout << "Downbeats " << result.trackSummary.downbeatsDetected
              << " BarLock " << std::fixed << std::setprecision(3) << result.trackSummary.averageBarConfidence
              << " PhraseBoundaries " << result.trackSummary.phraseBoundariesDetected
              << " PhraseLock " << result.trackSummary.averagePhraseConfidence
              << " BuildTension " << result.trackSummary.peakBuildTension
              << " DropPeak " << result.trackSummary.peakDropIntensity
              << " Style " << std::string(viz::toString(result.trackSummary.dominantStyle))
              << " (" << result.trackSummary.dominantStyleConfidence << ")\n";
    if (result.videoEncoded) {
        std::cout << "Encoded MP4 " << result.outputVideo.string()
                  << " (" << result.videoBytes << " bytes)\n";
    }
    if (result.sharePackageGenerated) {
        std::cout << "Share page " << result.sharePage.string() << "\n";
        std::cout << "Share manifest " << result.shareManifest.string() << "\n";
        if (!result.previewImage.empty()) {
            std::cout << "Preview image " << result.previewImage.string()
                      << " (" << result.previewFramesUsed << " frames)\n";
        }
    }
    return 0;
}
